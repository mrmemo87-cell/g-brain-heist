...
const expectPattern = (content, pattern, message) => {
...
test('reward RPCs are locked down from authenticated clients', () => {
    ...
    expectPattern(applyRewardSql, /auth\.role\(\).*service_role|service_role.*auth\.role\(\)/is, 'rpc_apply_reward_delta must enforce service_role runtime guard');
    expectPattern(applyRewardSql, /revoke execute on function public\.rpc_apply_reward_delta\([^\)]*\) from authenticated/is, 'rpc_apply_reward_delta must revoke execute from authenticated');
    expectPattern(levelupSql, /auth\.role\(\).*service_role|service_role.*auth\.role\(\)/is, 'rpc_grant_levelup_rewards must enforce service_role runtime guard');
    expectPattern(levelupSql, /revoke execute on function public\.rpc_grant_levelup_rewards\([^\)]*\) from authenticated/is, 'rpc_grant_levelup_rewards must revoke execute from authenticated');
});
...
test('clan territory reward claim is client-direct and uses authenticated actor context', () => {
    ...
    assert.doesNotMatch(bhApi, /"clan-territory\/claim-reward"\s*:/i, 'bh_api must not expose clan-territory/claim-reward route');
    assert.doesNotMatch(studentViewTs, /supabase\.functions\.invoke\(\s*['"]bh_api['"]/i, 'clan territory reward flow must not invoke bh_api edge function');
    expectPattern(studentViewTs, /supabase\.rpc\(\s*['"]rpc_claim_clan_territory_reward['"]/i, 'student view must call rpc_claim_clan_territory_reward directly');
    expectPattern(clanRewardRpcMigration, /v_user_id\s+uuid\s*:=\s*auth\.uid\(\)/i, 'clan territory claim rpc must derive actor from auth.uid()');
    assert.doesNotMatch(clanRewardRpcMigration, /p_user_id/i, 'clan territory claim rpc must not accept caller-provided user id');
    expectPattern(clanRewardRpcMigration, /grant execute on function public\.rpc_claim_clan_territory_reward\(text, text, int, int, int, text, text\) to authenticated;/i, 'clan territory claim rpc must be invokable by authenticated clients');
});
test('task reward claim is client-direct rpc and does not mutate rewards in TypeScript', () => {
    ...
    assert.doesNotMatch(bhApi, /"tasks\/claim"\s*:/i, 'bh_api must not expose tasks/claim route');
    assert.doesNotMatch(gameService, /supabase\.functions\.invoke\(\s*['"]bh_api['"]/i, 'task claim flow must not invoke bh_api edge function');
    expectPattern(gameService, /supabase\.rpc\(\s*['"]rpc_claim_task_reward['"]/i, 'task claim flow must call rpc_claim_task_reward directly');
    assert.doesNotMatch(taskClaimFn, /\.from\(['"]users['"]\)\.update\(/i, 'task_claim function must not manually mutate user rewards in TypeScript');
    expectPattern(taskClaimMigration, /v_user_id\s+uuid\s*:=\s*auth\.uid\(\)/i, 'task claim rpc must derive actor from auth.uid()');
    assert.doesNotMatch(taskClaimMigration, /if\s+coalesce\(auth\.role\(\),\s*''\)\s*<>\s*'service_role'/i, 'task claim rpc must not enforce service_role-only runtime guard');
    expectPattern(taskClaimMigration, /grant execute on function public\.rpc_claim_task_reward\(text, text, int, int, int, text\) to authenticated;/i, 'task claim rpc must be invokable by authenticated clients');
    expectPattern(taskClaimMigration, /v_event_id\s*:=\s*'task:'\s*\|\|\s*p_task_id\s*\|\|\s*':'\s*\|\|\s*v_period_key/i, 'task claim rpc must force canonical event id instead of trusting caller input');
});
