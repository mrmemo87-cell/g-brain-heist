...
import { createInitialStudentWritingState, runDailyWritingPracticeFlow, runInitialWritingAssessmentFlow, runMonthlyWritingReviewFlow, runWeeklyWritingReviewFlow, } from '../src/lib/brains_heist/writingWorkflowOrchestrator.js';
...
test('daily task submission updates state correctly', () => {
    ...
    const daily = runDailyWritingPracticeFlow({
        student_id: 'stu-2',
        daily_task: task,
        student_submission: 'This essay describes the event, explains why it mattered for students, and gives one suggestion for the next term.',
...
test('repeated success changes adaptation trend', () => {
    const initial = runInitialWritingAssessmentFlow({
        ...
        student_response: 'This report describes the event, explains why it mattered, and provides one recommendation for future improvement.',
    });
    let state = initial.updated_writing_state;
    for (let i = 0; i < 3; i += 1) {
        const task = {
        ...
        const result = runDailyWritingPracticeFlow({
            student_id: 'stu-4',
            daily_task: task,
            student_submission: 'This report describes the event clearly, explains why it mattered, and gives one practical recommendation.',
            writing_state: state,
...
test('monthly review builds proper report from accumulated history', () => {
    ...
    const mar = runInitialWritingAssessmentFlow({
        ...
        prompt_text: prompt,
        target_word_count: 120,
        student_response: 'This essay describes the event, explains why it mattered, and gives one clear suggestion for next time.',
        current_state: state,
