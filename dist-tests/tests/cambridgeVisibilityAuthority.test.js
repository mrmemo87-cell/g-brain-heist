...
const migration = fs.readFileSync(path.resolve(process.cwd(), 'supabase/migrations/20260722033000_correct_cambridge_visibility_authority.sql'), 'utf8');
...
test('student Cambridge visibility derives identity and roster server-side', () => {
    ...
    assert.doesNotMatch(migration.match(/CREATE OR REPLACE FUNCTION public\.get_visible_cambridge_tests_for_student[\s\S]*?\$\$;/)?.[0] || '', /WHERE[\s\S]*p_school_id|WHERE[\s\S]*p_student_grade/);
