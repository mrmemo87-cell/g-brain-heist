...
import { CAMBRIDGE_LISTENING_TEST_1_ANSWER_KEY, CAMBRIDGE_LISTENING_TEST_1_QUESTIONS, CAMBRIDGE_LISTENING_TEST_1_SECTIONS, isCambridgeAnswerCorrect, parseCambridgeResponses, } from '../components/cambridgeListeningReview.js';
...
test('Listening Test 1 report reads nested responses and matches the stored 6/30 result', () => {
    ...
    Object.entries(CAMBRIDGE_LISTENING_TEST_1_ANSWER_KEY).forEach(([question, expected]) => {
        ...
        if (!response)
            unanswered += 1;
        if (isCambridgeAnswerCorrect(response, expected))
            correct += 1;
    });
