...
test('historical continuity does not weaken current question integrity or targeted-practice exclusion', () => {
    ...
    assert.match(migration, /not exists \(\s*select 1\s*from public\.student_learning_intervention_practice_assignments practice/i);
});
test('official assignment summaries preserve mapping-superseded outcomes only through immutable item evidence', () => {
    const viewStart = migration.indexOf('create or replace view private.student_verified_assignment_summaries');
