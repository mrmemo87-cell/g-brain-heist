...
const listTeacherAllocationsService = adminService.slice(adminService.indexOf('export async function listTeacherAllocations'), adminService.indexOf('export async function allocateTeacherToClassSubject'));
...
test('teacher allocations carry and render teacher identity independently of eligibility list', () => {
    ...
    assert.match(teachersTab, /allocation\.teacher_name|Allocation needs staff-status review/);
});
