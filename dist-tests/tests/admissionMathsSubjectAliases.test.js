...
function normalizeManagedSubject(subject) {
...
function resolveManagedVersion(grade, subject, pools) {
    const managedSubject = normalizeManagedSubject(subject);
    if (grade === null || managedSubject === undefined)
        return undefined;
    return pools
...
test('Grade 6 Maths v2 content generates the required safe 25-question distribution', () => {
    ...
    const selected = Object.entries(required).flatMap(([difficulty, count]) => grade6Maths.questions
        .filter((question) => question.difficulty === difficulty && question.content_version === 'adm-bank-v2-g6-maths')
        .slice(0, count));
