...
test('schema v2 verifies all visual bytes, metadata and question links', () => {
    for (const [packageDirectory, expectedAssets] of [['2026-7-0', 24], ['2026-8-0', 24], ['2026-10-0', 32], ['2026-11-0', 36]]) {
        ...
        assert.equal(new Set(manifest.assets.map((asset) => asset.assetId)).size, expectedAssets);
        assert.ok(manifest.assets.every((asset) => asset.publicPath.includes(asset.sha256.slice(0, 12))));
        const visualQuestions = manifest.files.flatMap((file) => {
            const source = JSON.parse(readFileSync(path.join('content/verified-question-packages', packageDirectory, file), 'utf8'));
            return source.questions.filter((question) => question.visualAssetId);
        });
