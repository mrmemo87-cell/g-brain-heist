...
test('Grade 6 Science official bank v1 has intended strand distribution', () => {
    ...
    for (const question of grade6.questions)
        byStrand.set(question.strand, (byStrand.get(question.strand) ?? 0) + 1);
...
function byShortId(id) {
    const question = grade6.questions.find((item) => item.external_id.endsWith(id));
...
test('simulated 25-question Grade 6 Science generation prefers unique subskills before shortage fallback', () => {
    ...
    for (let seed = 1; seed <= 30; seed += 1) {
        ...
        const selected = [];
        const selectedSubskills = new Set();
        const selectedStems = new Set();
        const selectedIds = new Set();
        const fallbackChecks = [];
        for (const bucket of blueprint) {
            const candidates = grade6.questions
                .filter((question) => question.question_type === bucket.question_type && question.difficulty === bucket.difficulty)
                .map((question) => ({ question, sort: random() }))
                .sort((a, b) => a.sort - b.sort)
                .map((entry) => entry.question);
            const hasUniqueSubskillCandidate = () => candidates.some((question) => {
            ...
            const pick = (allowRepeatedSubskill) => {
                for (const question of candidates) {
                    const stem = normalizeStem(question.prompt);
                    if (selectedIds.has(question.external_id) || selectedStems.has(stem))
                        continue;
                    if (!allowRepeatedSubskill && selectedSubskills.has(question.subskill))
                        continue;
        ...
        assert.deepEqual(blueprint.map((bucket) => selected.filter((question) => question.question_type === bucket.question_type && question.difficulty === bucket.difficulty).length), blueprint.map((bucket) => bucket.count));
        assert.ok(fallbackChecks.every(Boolean), `seed ${seed} used fallback while a unique-subskill candidate was still available`);
    }
});
const grade6ScienceBlueprintDistribution = {
    mcq: { hard: 2, medium: 13, easy: 10 },
};
const canonical = (value) => value.trim().toLowerCase();
function orderedSqlBuckets(distribution) {
...
function insertionOrderSqlBuckets(distribution) {
    return Object.entries(distribution)
        .flatMap(([question_type, difficulties]) => Object.entries(difficulties)
        .map(([difficulty, count]) => ({ question_type, difficulty, count })));
}
function simulateSqlBatchGeneration(seed, distribution, bucketOrder = orderedSqlBuckets) {
    const random = seededRandom(seed);
    const selected = [];
    const selectedIds = new Set();
    const selectedStems = new Set();
    ...
    for (const bucket of buckets) {
        ...
        for (const question of bucketQuestions) {
            if (!selectedIds.has(question.external_id) && !selectedStems.has(normalizeStem(question.prompt)))
                random();
        }
        const laterDifficultyOverlap = (question) => grade6.questions.some((candidate) => (candidate.question_type === question.question_type
        ...
        const candidates = bucketQuestions
            .map((question) => ({ question, overlap: laterDifficultyOverlap(question), sort: random() }))
            .sort((a, b) => a.overlap - b.overlap || a.sort - b.sort)
            .map((entry) => entry.question);
        const availableUniqueConcepts = new Set(candidates
            .filter((question) => !selectedIds.has(question.external_id) && !selectedStems.has(normalizeStem(question.prompt)) && !selectedCanonical.has(canonical(question.subskill)))
            .map((question) => canonical(question.subskill))).size;
        const selectedBeforeFallback = selected.length;
        const firstPassConcepts = new Set();
        for (const question of candidates) {
            if (selected.filter((item) => item.difficulty === bucket.difficulty).length >= bucket.count)
                break;
            const stem = normalizeStem(question.prompt);
            const concept = canonical(question.subskill);
            if (selectedIds.has(question.external_id) || selectedStems.has(stem) || selectedCanonical.has(concept) || firstPassConcepts.has(concept))
                continue;
            selected.push(question);
            selectedIds.add(question.external_id);
            selectedStems.add(stem);
        ...
        const remaining = bucket.count - selected.filter((item) => item.difficulty === bucket.difficulty).length;
        if (remaining > 0) {
            const fallbackBase = candidates
                .filter((question) => !selectedIds.has(question.external_id) && !selectedStems.has(normalizeStem(question.prompt)))
                .map((question) => ({
                question,
                concept: canonical(question.subskill),
                existing: selected.filter((item) => canonical(item.subskill) === canonical(question.subskill)).length,
                sort: random(),
            }))
                .sort((a, b) => a.concept.localeCompare(b.concept) || a.sort - b.sort);
            const conceptRounds = new Map();
            const byExistingConcept = fallbackBase
                ...
                .sort((a, b) => a.existing - b.existing || a.subskillRound - b.subskillRound || a.sort - b.sort);
            const fallbackQuestions = byExistingConcept.slice(0, remaining).map(({ question }) => question);
            fallbackEvents.push({
                difficulty: bucket.difficulty,
                unavoidable: availableUniqueConcepts < bucket.count,
                repeatedConcepts: fallbackQuestions
                    .map((question) => canonical(question.subskill))
                    .filter((concept) => selected.slice(0, selectedBeforeFallback).some((item) => canonical(item.subskill) === concept)),
            });
...
function strandInterleavedOrder(selected, seed) {
    const random = seededRandom(seed + 10_000);
    const strandRounds = new Map();
    return selected
...
function conceptsForDifficulty(difficulty) {
    return new Set(grade6.questions
        .filter((question) => question.question_type === 'mcq' && question.difficulty === difficulty)
        .map((question) => canonical(question.subskill)));
}
test('Grade 6 Science candidate graph proves 22 canonical concepts is the achievable maximum', () => {
    ...
    for (let seed = 1; seed <= 100; seed += 1) {
        ...
        const selectedConcepts = new Set(selected.map((question) => canonical(question.subskill)));
...
test('simulated fixed SQL batch generation follows ordered difficulty buckets and limits repeats to unavoidable fallback only', () => {
    ...
    for (let seed = 1; seed <= 100; seed += 1) {
        ...
        const byDifficulty = (difficulty) => selected.filter((question) => question.difficulty === difficulty);
        ...
        for (const question of selected)
            conceptCounts.set(canonical(question.subskill), (conceptCounts.get(canonical(question.subskill)) ?? 0) + 1);
        ...
        const conceptsByDifficulty = (difficulty) => new Map();
        const easyConceptCounts = conceptsByDifficulty('easy');
        for (const question of byDifficulty('easy'))
            easyConceptCounts.set(canonical(question.subskill), (easyConceptCounts.get(canonical(question.subskill)) ?? 0) + 1);
        const mediumConceptCounts = conceptsByDifficulty('medium');
        for (const question of byDifficulty('medium'))
            mediumConceptCounts.set(canonical(question.subskill), (mediumConceptCounts.get(canonical(question.subskill)) ?? 0) + 1);
        const hardConceptCounts = conceptsByDifficulty('hard');
        for (const question of byDifficulty('hard'))
            hardConceptCounts.set(canonical(question.subskill), (hardConceptCounts.get(canonical(question.subskill)) ?? 0) + 1);
        ...
        const strandOrder = finalOrder.map((question) => question.strand);
...
test('Grade 6 Science simulation exposes why SQL difficulty buckets must be explicitly ordered', () => {
    ...
    assert.ok(unorderedResult.fallbackEvents.some((event) => event.difficulty === 'medium'), 'expected the hard-first simulation to require medium fallback without explicit SQL difficulty ordering');
});
