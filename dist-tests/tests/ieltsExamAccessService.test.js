...
import { checkIeltsExamModeAdminAccess, resolveIeltsExamModeAdminAccess, } from '../services/ieltsExamAccessService.js';
test('IELTS Exam Mode access trusts canonical school capability and platform administrators', () => {
    assert.deepEqual(resolveIeltsExamModeAdminAccess({ isAuthenticated: true, role: 'school_admin' }), { allowed: false, reason: 'denied' });
    assert.deepEqual(resolveIeltsExamModeAdminAccess({ isAuthenticated: true, role: 'admin' }), { allowed: true, reason: 'platform_admin_role' });
    ...
    assert.deepEqual(resolveIeltsExamModeAdminAccess({ isAuthenticated: true, role: 'school_admin', canAdministerSchool: true }), { allowed: true, reason: 'school_admin_capability' });
});
...
const authenticatedAccessClient = (rpcHandler) => ({
    auth: {
        getUser: async () => ({
            data: { user: { id: 'user-1' } },
            error: null,
...
test('IELTS Exam Mode guard hides previously authorized content during every recheck', () => {
    ...
    const reviewGuard = fs.readFileSync(path.join(process.cwd(), 'components/ielts/IeltsReviewAdminGuard.tsx'), 'utf8');
