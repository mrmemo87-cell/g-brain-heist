...
const { applyCommanderPvpTurn, buildCommanderPvpBattle, } = moduleExports;
...
test('Commander PvP keeps player school powers authoritative', () => {
    ...
    assert.equal(surge.events.filter((event) => event.side === 'player' && event.code === 'chain_surge').length, 2);
    assert.throws(() => applyCommanderPvpTurn(state, { move: 'rot_miasma', targetId: 'enemy_commander' }), /commander_power_locked/);
});
test('Commander PvP fails closed for malformed attacker or defender loadouts', () => {
    for (const [attacker, defender] of [
        ...
        assert.throws(() => buildCommanderPvpBattle(1, attacker, defender, 'Rival'), /invalid_owned_loadout/);
    }
});
