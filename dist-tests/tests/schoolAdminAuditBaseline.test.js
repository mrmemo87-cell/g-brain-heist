...
const membershipCapabilities = readFileSync('supabase/migrations/20260801173000_school_membership_capabilities.sql', 'utf8');
...
test('school administration features stay within one complete portal shell', () => {
    ...
    for (const [id, label] of expectedTabs) {
        assert.match(schoolAdminPortal, new RegExp(`id: '${id}'[^\\n]+label: '${label}'`), `${label} must remain in the school administration navigation`);
        assert.match(schoolAdminPortal, new RegExp(`activeTab === '${id}'`), `${label} must render inside the school administration shell`);
    }
});
