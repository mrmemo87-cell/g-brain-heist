...
- give one suggestion`;
const getTask = (grade, genre, response, studentId) => {
...
test('successful rewrite fixes prior language errors', () => {
    ...
    const result = evaluateWritingPracticeTask({
        ...
        student_submission: 'This essay describes the school event clearly and explains why it mattered for student confidence, teamwork, and responsibility. It also recommends one practical change: schedule structured reflection after each activity so students can review mistakes and improve in the next task.',
        latest_assessment: assessment,
        repeated_error_memory: memory,
        grade: 9,
        genre: 'essay',
...
test('communicative task with wrong tone recommends retry_same_genre', () => {
    const { tasks, assessment, memory } = getTask(10, 'report', 'Hey guys this report was cool. I describe event and explain why it mattered and suggest doing it again.', 'ca');
    const task = tasks.find((item) => ['genre convention task', 'guided writing', 'full exam-style response'].includes(item.task_type)) ?? tasks[0];
    const result = evaluateWritingPracticeTask({
        daily_task: task,
        student_submission: 'Hey guys, this report is awesome and super cool. We should totally do this again because it was fun.',
        latest_assessment: assessment,
        repeated_error_memory: memory,
        grade: 10,
        genre: 'report',
...
test('repeated success can trigger increase_difficulty recommendation', () => {
    ...
    const result = evaluateWritingPracticeTask({
        ...
        student_submission: 'This essay describes the event in detail, explains why it mattered for the school community, and offers one practical suggestion for future planning. The argument is clear, the points are connected logically, and the conclusion reinforces the recommendation with a focused final statement.',
        latest_assessment: a3,
        repeated_error_memory: memory,
        grade: 9,
        genre: 'essay',
