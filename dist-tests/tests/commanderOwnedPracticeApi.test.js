...
const source = ts.transpileModule(readFileSync("supabase/functions/commander_practice/index.ts", "utf8"), {
    compilerOptions: {
...
function setup(options = {}) {
    ...
    new Function("require", "exports", "Deno", "crypto", source)(requireDependency, {}, {
        env: {
            get: (key) => ({
                SUPABASE_URL: "https://test.invalid",
                SUPABASE_SERVICE_ROLE_KEY: "test",
                COMMANDER_PREVIEW_SIGNING_SECRET: "test-owned-loadout-secret-over-thirty-two-characters",
            })[key],
        },
    }, webcrypto);
    return {
        calls,
        request: (body, token = "valid") => handler(new Request("https://test.invalid", {
            method: "POST",
...
test("owned practice fetches only the verified student army and ignores forged browser stats", async () => {
    ...
    const next = await (await api.request({
        action: "turn",
...
test("owned practice denies non-students, banned students, and invalid tokens before loading an army", async () => {
    for (const options of [{ role: "teacher" }, { banned: true }]) {
        ...
        assert.equal((await api.request({ action: "start", loadout: "owned" })).status, 403);
        assert.equal(api.calls.length, 0);
    }
    const api = setup();
    assert.equal((await api.request({ action: "start", loadout: "owned" }, "invalid"))
        .status, 401);
    assert.equal(api.calls.length, 0);
});
