...
const migration = readFileSync('supabase/migrations/20260905053848_canonical_teacher_class_roster_visibility.sql', 'utf8');
const scopeFunction = migration.match(/create or replace function private\.current_teacher_class_ids\(\)[\s\S]*?\$\$;/i)?.[0] || '';
...
test('class_students gives allocated teachers read-only roster access', () => {
    assert.match(migration, /create policy "Teachers can view students in allocated classes"[\s\S]*?for select[\s\S]*?to authenticated[\s\S]*?private\.current_teacher_class_ids\(\)/i);
    assert.match(migration, /drop policy if exists "Teachers can manage students in their classes"/i);
});
test('legacy helper name is only a security-invoker compatibility wrapper', () => {
    assert.match(migration, /create or replace function public\.get_my_teacher_class_ids\(\)[\s\S]*?security invoker[\s\S]*?private\.current_teacher_class_ids\(\)/i);
});
