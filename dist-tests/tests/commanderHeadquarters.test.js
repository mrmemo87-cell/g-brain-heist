...
import { applyPracticeTurn, buildOwnedPracticeBattle, startPracticeBattle, } from "../supabase/functions/commander_practice/engine.js";
import { commanderLevelXp, commanderMissingCoins, commanderTrainingCost, commanderUnitNextRankGain, commanderUnitRankBonus, commanderUnitRankCap, commanderUnitTrainingCost, } from "../src/features/cursedCommander/commanderHeadquartersModel.js";
const loadout = () => ({
    version: 1,
    profileVersion: 4,
    hp: 92,
...
test("owned loadout changes only player army while preserving canonical slots and opponent", () => {
    const input = loadout(), before = structuredClone(input), fixed = startPracticeBattle(7), owned = buildOwnedPracticeBattle(7, input);
    assert.deepEqual(input, before);
    assert.deepEqual(owned.combatants.map((u) => u.id), fixed.combatants.map((u) => u.id));
    assert.deepEqual(owned.combatants.filter((u) => u.side === "enemy"), fixed.combatants.filter((u) => u.side === "enemy"));
...
test("owned battle resolves configured spells and guard while fixed practice retains baseline", () => {
    ...
    assert.equal(bolt.events.find((e) => e.side === "player" && e.code === "death_bolt")
        ?.amount, 34);
    const guard = applyPracticeTurn(owned, { move: "guard" });
    assert.equal(guard.events.find((e) => e.side === "player" && e.code === "guard")?.amount, 20); // 40 capacity - 20 existing
    const focus = applyPracticeTurn(owned, {
        move: "focus_target",
        targetId: "enemy_commander",
    });
    assert.equal(focus.events.find((e) => e.side === "player" && e.code === "focus_target" && e.amount)?.amount, 11);
    assert.equal(applyPracticeTurn(startPracticeBattle(9), {
        move: "death_bolt",
        targetId: "enemy_commander",
    }).events.find((e) => e.code === "death_bolt")?.amount, 26);
    assert.equal(owned.combatants[0].hp, 92, "turn calculation must not mutate the snapshot");
});
test("school powers unlock from equipped recruits and share the Commander power core", () => {
    ...
    assert.throws(() => applyPracticeTurn(storm, { move: "rot_miasma", targetId: "enemy_commander" }), /commander_power_locked/);
    ...
    assert.equal(miasma.events.filter((event) => event.side === "player" && event.code === "rot_miasma").length, 3, "Rot should pressure every living enemy");
...
test("goal prices show only missing Coins and training follows the configured curve", () => {
    ...
    assert.equal(commanderTrainingCost(1, { trainingBase: 25, trainingGrowth: 1.15 }), 25);
    assert.equal(commanderTrainingCost(3, { trainingBase: 25, trainingGrowth: 1.15 }), 33);
