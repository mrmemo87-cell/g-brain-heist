...
function assertHeartbeatQueriesAreExecuted(source, label, expectedCalls) {
    let cursor = 0;
    let calls = 0;
    while (true) {
        ...
        assert.ok(isAwaited || isThenConsumed, `${label} heartbeat #${calls} constructs the Supabase RPC query without executing it`);
...
test('presence heartbeats consume Supabase RPC builders instead of treating them like Promises', () => {
    assert.doesNotMatch(app, /supabase\.rpc\('rpc_touch_last_seen'\)\.catch\s*\(/, 'Supabase query builders expose then(), not catch(), at runtime');
