...
import { canEnterClanTerritoryOfficialRoom, normalizeClanTerritoryClassCode, normalizeClanTerritoryClassCodes, } from '../src/features/clanTerritory/clanTerritoryEligibility.js';
const migration = readFileSync('supabase/migrations/20260726014520_clan_territory_canonical_student_context.sql', 'utf8');
...
test('official arena engine accepts canonical class arrays and keeps legacy batch compatibility', () => {
    assert.match(engine, /canEnterClanTerritoryOfficialRoom\(\s*state\.officialClassCodes,\s*player\.classCodes,\s*player\.batch,/);
