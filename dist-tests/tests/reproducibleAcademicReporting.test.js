...
test('reports cover student class grade subject and school targets', () => {
    for (const type of ['student', 'class', 'grade', 'subject', 'school'])
...
test('governed reports and live student printouts use their report-safe contracts', () => {
    for (const fn of ['rpc_academic_reporting_context', 'rpc_generate_academic_report_snapshot', 'rpc_get_academic_report_snapshot', 'rpc_finalize_academic_report_snapshot'])
        assert.match(service, new RegExp(fn, 'i'));
...
test('report UI exposes year term class grade subject and audience controls', () => {
    for (const label of ['School year', 'Reporting period', 'Class', 'Grade', 'Subject', 'Audience'])
        assert.match(builder, new RegExp(label, 'i'));
