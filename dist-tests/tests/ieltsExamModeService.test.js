...
import { rpcIeltsExamWhoami, rpcIeltsStartAttempt, rpcIeltsAutosaveAttempt, rpcIeltsSubmitAttempt, rpcIeltsLogIncident, rpcIeltsExamMonitoring, rpcIeltsPauseExam, rpcIeltsResumeExam, rpcIeltsExtendAttempt, rpcIeltsForceSubmitAttempt, rpcIeltsVoidAttempt, rpcIeltsCreateExamEvent, rpcIeltsCreateExamForm, rpcIeltsAssignExamToClass, rpcIeltsAssignExamToStudents, rpcIeltsListManageableExams, rpcIeltsGetExamAdminDetail, validateExamJsonText, payloadContainsAnswerKey, sanitizePublicFormPayload, } from '../services/ieltsExamModeService.js';
...
test('IELTS exam service wraps RPC names and parameters', async () => {
    ...
    const client = createClient((name, params) => {
        ...
        if (name === 'rpc_ielts_start_attempt')
            return { attempt_id: 'attempt-1', lock_token: 'lock' };
        if (name === 'rpc_ielts_autosave_attempt')
            return { attempt_id: 'attempt-1', section: 'reading', draft_version: 2 };
        if (name === 'rpc_ielts_submit_attempt')
            return { submission_id: 'submission-1', attempt_id: 'attempt-1', status: 'submitted' };
        if (name === 'rpc_ielts_log_incident')
...
test('IELTS exam monitoring and emergency service wrappers map RPC parameters', async () => {
    ...
    const client = createClient((name, params) => {
        calls.push({ name, params });
        if (name === 'rpc_ielts_exam_monitoring')
            return [{ student_id: 'student-1', attempt_id: 'attempt-1', status: 'in_progress' }];
        return { ok: true };
    });
...
test('IELTS exam manager service wrappers map create and assignment RPC parameters', async () => {
    ...
    const client = createClient((name, params) => {
        ...
        if (name === 'rpc_ielts_list_manageable_exams')
            return [];
        if (name === 'rpc_ielts_get_exam_admin_detail')
            return { exam: { id: 'exam-1' }, forms: [], classes: [], students: [], assignments: [] };
...
test('IELTS start attempt pgcrypto repair migration keeps lock tokens secure and scoped', () => {
    const migration = fs.readFileSync(path.join(process.cwd(), 'supabase/migrations/20260518150000_fix_ielts_start_attempt_pgcrypto.sql'), 'utf8');
    assert.match(migration, /create\s+extension\s+if\s+not\s+exists\s+pgcrypto\s+with\s+schema\s+extensions\s*;/i, 'repair migration must ensure pgcrypto exists in the extensions schema');
    assert.match(migration, /create\s+or\s+replace\s+function\s+public\.rpc_ielts_start_attempt\(p_assignment_id uuid\)[\s\S]*security\s+definer[\s\S]*set\s+search_path\s*=\s*public/i, 'start attempt repair must preserve SECURITY DEFINER and the public search_path');
    assert.match(migration, /encode\(extensions\.gen_random_bytes\(32\),\s*'hex'\)/i, 'new attempts must generate high-entropy lock tokens through extensions.gen_random_bytes');
    assert.match(migration, /coalesce\(v_attempt\.lock_token,\s*encode\(extensions\.gen_random_bytes\(32\),\s*'hex'\)\)/i, 'resumed not_started attempts must only generate a replacement token when one does not exist');
    assert.doesNotMatch(migration, /(^|[^.])\bgen_random_bytes\s*\(/i, 'repair migration must not call gen_random_bytes without the extensions schema');
    assert.doesNotMatch(migration, /answer_key/i, 'start attempt repair must not expose answer keys');
    assert.doesNotMatch(migration, /rpc_is_ielts_admin|ielts_teachers|is_ielts_admin/i, 'start attempt repair must not reintroduce legacy IELTS admin dependencies');
});
