...
function normalizeAdmissionQuestionStem(value) {
    return String(value ?? '')
        .toLowerCase()
        .replace(/[“”]/g, '"')
        .replace(/[‘’]/g, "'")
...
const g7Passages = passages.passages.filter((p) => p.grade_level === 7 && p.content_version === 'adm-bank-v1-g7-english');
const g7Rubrics = rubrics.rubrics.filter((r) => r.grade_level === 7 && r.content_version === 'adm-bank-v1-g7-english');
...
function assertPlacement(records, foundation, target, stretch) {
...
function assertBalancedMcqAnswers(records) {
...
test('Grade 7 English official bank v1 has required counts and links', () => {
    ...
    const passageIds = new Set(g7Passages.map((p) => p.external_id));
    const rubricIds = new Set(g7Rubrics.map((r) => r.external_id));
    for (const question of english.questions.filter((q) => q.question_type === 'reading_comprehension'))
        assert.ok(passageIds.has(question.passage_external_id));
    for (const question of english.questions.filter((q) => String(q.question_type).includes('writing')))
        assert.ok(rubricIds.has(question.rubric_external_id));
...
test('Grade 7 Maths official bank v1 has required counts and distributions', () => {
    ...
    for (const question of maths.questions)
...
test('Grade 7 official bank v1 uses numeric smallint stage fields', () => {
    ...
    for (const record of allRecords) {
        ...
        if ('stage' in record)
            assert.equal(typeof record.stage, 'number');
    }
});
