...
import { isAuthCallbackPath, resolvePostAuthPath, shouldUseGlobalAuthLoader, isSafeIeltsReturnPath, readIeltsAuthIntent, consumeIeltsAuthIntent, IELTS_AUTH_INTENT_KEY, } from '../src/lib/authFlowGuards.js';
...
test('IELTS auth intent is consumed only after a valid IELTS path is available', () => {
    const values = new Map([[IELTS_AUTH_INTENT_KEY, '/ielts/trial-test-2']]);
    const storage = {
...
test('invalid IELTS auth intent is ignored and not consumed as a redirect target', () => {
    const values = new Map([[IELTS_AUTH_INTENT_KEY, '/dashboard']]);
    const storage = {
        getItem: (key) => values.get(key) ?? null,
        removeItem: (key) => values.delete(key),
    };
