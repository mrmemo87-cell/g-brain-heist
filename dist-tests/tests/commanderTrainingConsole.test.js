...
test('Commander training console ships lightweight presentation assets without owning game data', () => {
    ...
    for (const asset of trainingAssets) {
        assert.equal(existsSync(join(root, 'public/commander-training', asset)), true, `${asset} must ship with the Commander training console`);
        assert.ok(css.includes(`/commander-training/${asset}`), `${asset} must be referenced by the Training presentation`);
    }
