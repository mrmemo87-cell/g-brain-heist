...
test('monthly report persistence and retrieval', () => {
    ...
    submitInitialWritingAssessment({
        ...
        prompt_text: prompt,
        target_word_count: 120,
        student_response: 'This essay describes the event clearly, explains why it mattered, and gives one practical suggestion.',
...
test('seeded pilot-readiness data produces expected analytics risk states', () => {
    ...
    const analytics = getWritingAnalyticsDashboard();
    assert.strictEqual(analytics.ok, true);
    assert.ok(analytics.data.pilot_readiness.overused_prompts.length >= 1, `expected overused prompts at threshold ${WRITING_PILOT_GUARDRAILS.prompt_overuse_threshold}`);
});
