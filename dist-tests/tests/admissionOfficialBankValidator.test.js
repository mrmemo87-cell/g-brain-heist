...
function makeInvalidSeedDir() {
    ...
    for (const dir of ['english', 'maths', 'science', 'shared'])
    ...
    for (const subject of ['english', 'maths', 'science']) {
        for (const grade of [5, 6, 7, 8])
            writeJson(path.join(root, subject, `grade_${grade}.json`), empty);
    }
...
test('official admission bank validator reports Grade 7 English longest-answer bias if it is reintroduced', () => {
    ...
    for (const subject of ['english', 'maths', 'science']) {
        for (const grade of [5, 6, 7, 8]) {
            ...
            if (subject === 'english' && grade === 7) {
                for (const question of source.questions.filter((q) => Array.isArray(q.options))) {
                    const correctIndex = question.correct_index;
                    question.options = question.options.map((option, optionIndex) => optionIndex === correctIndex ? `${option} with a uniquely identifying extra phrase` : option.replace(/ with .*/, ''));
    ...
    const output = `${result.stdout}\n${result.stderr}`;
    assert.notEqual(result.status, 0);
    assert.match(output, /grade 7 english official bank has correct option as uniquely longest|grade_7\.json: questions\[\d+\].*extreme correct-option length imbalance/);
});
test('official admission bank validator hard-fails visible placeholder box glyphs', () => {
    ...
    for (const subject of ['english', 'maths', 'science']) {
        for (const grade of [5, 6, 7, 8]) {
            const source = JSON.parse(readFileSync(path.join(seedRoot, subject, `grade_${grade}.json`), 'utf8'));
            if (subject === 'maths' && grade === 6)
                source.questions[0].prompt = 'What is the missing number? 640 ÷ □ = 8';
            writeJson(path.join(root, subject, `grade_${grade}.json`), source);
        }
    }
    const result = runValidator(root);
...
function linkedOfficialBankFixture(overrides = {}) {
