...
const codeSegment = (value) => String(value ?? '')
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-|-$/g, '');
...
const runValidator = (taxonomyPath) => spawnSync(process.execPath, ['scripts/validate-verified-question-package.mjs', packageDirectory], {
    cwd: process.cwd(),
    encoding: 'utf8',
