...
async function loadValidator() {
    return await import(pathToFileURL(path.resolve(validatorPath)).href);
}
async function loadImporter() {
    return await import(pathToFileURL(path.resolve(importerPath)).href);
}
...
test('every proposal is hash-bound, uniquely keyed, and remains human in_review', async () => {
    const { loadAndValidateTaxonomyReviewBatch } = await loadValidator();
    const result = loadAndValidateTaxonomyReviewBatch();
    const proposalKeys = new Set(result.proposals.map((proposal) => proposal.proposalKey));
    const proposalHashes = new Set(result.proposals.map((proposal) => proposal.proposalHash));
    assert.equal(proposalKeys.size, 958);
    assert.equal(proposalHashes.size, 958);
    assert.ok(result.proposals.every((proposal) => /^[0-9a-f]{64}$/.test(proposal.proposalHash)));
    assert.ok(result.proposals.every((proposal) => proposal.payload.reviewStatus === 'in_review'));
    assert.ok(result.proposals.every((proposal) => proposal.payload.humanReview === true));
    assert.ok(result.proposals.every((proposal) => proposal.proposalKey
        === `${proposal.taxonomyVersion}:${proposal.sourceQuestionId}`));
    const retired = result.proposals.filter((proposal) => proposal.sourceLifecycleStatus === 'retired');
    assert.equal(retired.length, 8);
    assert.deepEqual(new Set(retired.map((proposal) => proposal.sourceQuestionId)), retiredQuestionIds);
    assert.equal(result.proposals.filter((proposal) => proposal.sourceLifecycleStatus === 'verified').length, 950);
});
...
test('importer dry-run validates locally and creates the exact atomic RPC envelope', async () => {
    ...
    assert.ok(envelope.proposals.every((proposal) => proposal.payload.reviewStatus === 'in_review'));
    assert.ok(envelope.proposals.every((proposal) => proposal.payload.humanReview === true));
...
test('chunked staging plan is deterministic, bounded, complete, and much smaller than the atomic body', async () => {
    ...
    assert.equal(plan.chunks.reduce((sum, chunk) => sum + chunk.proposals.length, 0), 958);
    assert.equal(new Set(plan.chunks.flatMap((chunk) => chunk.proposals.map((proposal) => proposal.proposalKey))).size, 958);
    assert.ok(plan.chunks.every((chunk, index) => chunk.chunkIndex === index));
    assert.ok(plan.chunks.every((chunk) => chunk.proposals.length <= 75));
    assert.ok(plan.chunks.every((chunk) => /^[0-9a-f]{64}$/.test(chunk.chunkChecksum)));
...
test('chunked importer stages every immutable chunk then performs one transactional finalize', async () => {
    ...
    assert.deepEqual(calls.slice(1, 14).map((call) => call.rpc), Array(13).fill('rpc_stage_verified_question_taxonomy_review_chunk'));
    assert.deepEqual(calls.slice(1, 14).map((call) => call.args.p_chunk_index), Array.from({ length: 13 }, (_, index) => index));
