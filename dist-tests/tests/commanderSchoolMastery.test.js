...
import { applyPracticeTurn, buildOwnedPracticeBattle, } from "../supabase/functions/commander_practice/engine.js";
import * as practiceEngine from "../supabase/functions/commander_practice/engine.js";
import { commanderSchoolMasteryCost, commanderSchoolMasteryDoctrine, commanderSchoolMasteryNextLevel, commanderSchoolMasteryRankCap, } from "../src/features/cursedCommander/commanderHeadquartersModel.js";
...
const pvpExports = {};
new Function("require", "exports", pvpCompiled)((specifier) => {
    if (specifier === "../commander_practice/engine.ts")
        return practiceEngine;
    throw new Error(`Unexpected Commander PvP engine import: ${specifier}`);
}, pvpExports);
const { buildCommanderPvpBattle } = pvpExports;
...
test("trusted practice loadouts apply Void and Storm Mastery without changing power ownership", () => {
    ...
    assert.equal(bolt.events.find((event) => event.side === "player" && event.code === "death_bolt")?.amount, 40, "Void Rank 2 adds 6 to a 34-damage Death Bolt");
    ...
    assert.throws(() => applyPracticeTurn(mastered, { move: "rot_miasma", targetId: "enemy_commander" }), /commander_power_locked/);
});
