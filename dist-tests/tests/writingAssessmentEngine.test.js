...
import { assessWritingExam, createEmptyErrorMemory, generateMonthlyComparison, GENRE_EXPECTATION_CONFIG, storeAttemptInErrorMemory, } from '../src/lib/brains_heist/writingAssessment.js';
...
test('supports all required genres with valid JSON output skeleton', () => {
    ...
    for (const genre of genres) {
        const result = assessWritingExam({
            ...
            genre: genre,
            targetWordCount: 120,
            studentResponse: 'Dear readers, first I describe the main event in detail. Then I explain why it mattered to our community and school. Therefore, my recommendation is to improve planning in future events with clear student roles.',
        });
...
test('grade 8-9 uses unified 4-subscale scoring and still exposes hidden coaching signals', () => {
    const result = assessWritingExam({
        ...
        genre: 'article',
        targetWordCount: 100,
        studentResponse: 'First, I describe the main event. Then I explain why it mattered for readers in our school community. Finally, I add one recommendation for future events.',
    });
    assert.strictEqual(result.score_mode, 'B1B2_4_scale');
...
test('under-length but relevant response gets under_length tag while preserving content recognition', () => {
    const result = assessWritingExam({
        ...
        genre: 'email',
        targetWordCount: 120,
        studentResponse: 'Dear Teacher, the main event was the science fair final. It mattered because students collaborated and learned from mistakes. I recommend a longer preparation week. Regards, Ali',
    });
...
test('good language but missing one content point reduces content band', () => {
    const result = assessWritingExam({
        ...
        genre: 'essay',
        targetWordCount: 140,
        studentResponse: 'This essay describes the main event in the school debate final and explains why it mattered for student confidence. Furthermore, the discussion encouraged critical thinking and respectful disagreement. In conclusion, this event clearly benefited the school community.',
    });
...
test('strong response with wrong tone/register is penalized for communicative achievement', () => {
    const result = assessWritingExam({
        ...
        genre: 'report',
        targetWordCount: 150,
        studentResponse: 'Hey guys, the event was super awesome and we had a blast. I described the main event clearly, explained why it mattered for everyone, and gave a recommendation to repeat it monthly. You all should totally do it again because it was really cool and fun for everybody.',
    });
...
test('repeated error memory and monthly comparison work across attempts', () => {
    ...
    const febResult = assessWritingExam({
        ...
        genre: 'essay',
        targetWordCount: 160,
        studentResponse: 'This essay describes the main event. However, it did not fully succeed. In conclusion, my recommendation is clear for the school audience.',
    });
