...
import { applyPracticeTurn, startPracticeBattle, } from '../supabase/functions/commander_practice/engine.js';
...
test('practice combat rejects forged target ids and preserves source state', () => {
    ...
    assert.throws(() => applyPracticeTurn(original, { move: 'death_bolt', targetId: 'player_commander' }), /invalid_target/);
...
test('death bolt has a server-enforced cooldown', () => {
    ...
    assert.throws(() => applyPracticeTurn(afterBolt, { move: 'death_bolt', targetId: 'enemy_commander' }), /death_bolt_on_cooldown/);
});
