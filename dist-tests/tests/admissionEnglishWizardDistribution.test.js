...
test('Grade 5 English official bank has passage-backed reading comprehension questions', () => {
    ...
    const passageByExternalId = new Map(passageSeed.passages.map((passage) => [passage.external_id, passage]));
    const reading = seed.questions.filter((q) => q.question_type === 'reading_comprehension');
    assert.ok(reading.length > 0);
    assert.ok(reading.every((q) => typeof q.passage_external_id === 'string' && q.passage_external_id.trim().length > 0));
    assert.ok(reading.every((q) => {
        const passage = passageByExternalId.get(q.passage_external_id);
