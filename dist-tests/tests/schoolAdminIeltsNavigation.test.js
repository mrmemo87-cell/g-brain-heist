...
import { buildSchoolAdminNavigationUrl, isValidSchoolAdminIeltsMonitorExamId, isValidSchoolAdminIeltsReviewAttemptId, isValidSchoolAdminIeltsRouteExamId, parseSchoolAdminNavigation, schoolAdminIeltsUrl, } from '../src/lib/schoolAdminIeltsNavigation.js';
...
test('school admin IELTS location state restores a validated tab and review detail', () => {
    assert.deepEqual(parseSchoolAdminNavigation('?adminTab=ielts&ieltsTab=ielts-reviews&reviewSkill=writing&reviewAttempt=attempt-42'), {
        adminTab: 'ielts',
...
test('building navigation URLs preserves unrelated safe query state and clears stale IELTS detail', () => {
    assert.equal(buildSchoolAdminNavigationUrl({ adminTab: 'members', ieltsTab: 'ielts-reviews', review: { skill: 'speaking', attemptId: 'abc' }, monitorExamId: null }, '/?campaign=school&adminTab=ielts&ieltsTab=ielts-reviews&reviewSkill=speaking&reviewAttempt=abc'), '/?campaign=school&adminTab=members&view=school_admin');
});
test('direct IELTS admin destinations are represented by fixed enum state instead of return URLs', () => {
    assert.equal(schoolAdminIeltsUrl('ielts-reviews', { skill: 'speaking', attemptId: 'attempt/7' }), '/?view=school_admin&adminTab=ielts&ieltsTab=ielts-reviews&reviewSkill=speaking&reviewAttempt=attempt%2F7');
});
...
test('exam monitoring restores only a validated UUID inside the IELTS admin shell', () => {
    ...
    assert.equal(parseSchoolAdminNavigation(`?adminTab=ielts&ieltsTab=ielts-exams&monitorExam=${examId}`).monitorExamId, examId);
    assert.equal(parseSchoolAdminNavigation('?adminTab=ielts&ieltsTab=ielts-exams&monitorExam=javascript:alert(1)').monitorExamId, null);
...
test('navigation construction and parsing share bounded detail validation', () => {
    ...
    assert.equal(parseSchoolAdminNavigation(`?adminTab=ielts&ieltsTab=ielts-exams&monitorExam=${genericUuid}`).monitorExamId, genericUuid);
    ...
    const invalidUrl = buildSchoolAdminNavigationUrl({ adminTab: 'ielts', ieltsTab: 'ielts-exams', review: null, monitorExamId: 'not-a-uuid' }, '/?monitorExam=stale');
