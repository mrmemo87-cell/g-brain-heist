...
const migrationPath = 'supabase/migrations/20260824182530_lock_school_head_academic_evidence_authority.sql';
...
test('School Head scored work uses only verified assignment summaries', () => {
    const body = functionBody('private.school_head_authoritative_scored_work', 'private.school_head_authoritative_grade_performance');
...
test('Cambridge score rows fail closed without an authority-qualified observation', () => {
    ...
    assert.match(body, /public\.student_learning_observation_is_qualified\([\s\S]*o\.contributes_to_focus_state[\s\S]*o\.evidence/i);
});
test('grade performance preserves its public contract and delegates to governed evidence', () => {
    const body = functionBody('public.school_head_get_grade_performance', 'private.school_head_authority_qualified_academic_decline');
    ...
    assert.match(migration, /revoke all on function public\.school_head_get_grade_performance\(uuid, integer\)[\s\S]*grant execute[\s\S]*to authenticated/i);
});
test('academic decline uses governed scored work and replaces the legacy signal', () => {
    const decline = functionBody('private.school_head_authority_qualified_academic_decline', 'private.school_head_authoritative_operational_decisions');
    const decisions = functionBody('private.school_head_authoritative_operational_decisions');
...
test('executive snapshot replaces every official academic aggregate before return', () => {
    const body = functionBody('public.school_head_get_executive_snapshot');
    ...
    assert.match(migration, /revoke all on function public\.school_head_get_executive_snapshot_unqualified_legacy_20260824\([\s\S]*from public, anon, authenticated, service_role/i);
});
test('v2 is defined independently of the earlier compatibility rollout', () => {
    const body = functionBody('public.school_head_get_executive_snapshot_v2', 'private.refresh_school_head_decision_alerts');
    ...
    assert.match(body, /not public\.is_school_owner\(p_school_id\)/i);
    assert.match(body, /revoke all[\s\S]*from public, anon, authenticated, service_role[\s\S]*grant execute[\s\S]*to authenticated/i);
});
test('persisted executive averages and alerts use the authoritative helpers', () => {
    ...
    assert.match(migration, /revoke all on function private\.refresh_school_head_decision_alerts\(uuid, integer\)[\s\S]*from public, anon, authenticated, service_role/i);
});
