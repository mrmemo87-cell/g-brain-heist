...
import { rpcIeltsStudentJourney } from '../services/ieltsJourneyService.js';
...
test('IELTS journey SQL is scoped, defensive, and avoids protected answer data', () => {
    const migration = fs.readFileSync(path.join(process.cwd(), 'supabase/migrations/20260519113000_ielts_student_journey_schema_alignment_repair.sql'), 'utf8');
...
test('IELTS journey objective result links use attempt tables and latest student-owned attempts', () => {
    const migration = fs.readFileSync(path.join(process.cwd(), 'supabase/migrations/20260522153000_ielts_journey_objective_result_links.sql'), 'utf8');
...
test('IELTS readiness engine SQL normalizes safe skill readiness defensively', () => {
    const migration = fs.readFileSync(path.join(process.cwd(), 'supabase/migrations/20260518130000_ielts_readiness_engine_foundation.sql'), 'utf8');
