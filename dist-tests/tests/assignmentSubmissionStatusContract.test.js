...
const statusMigration = readFileSync('supabase/migrations/20260815021520_allow_assignment_in_progress_status.sql', 'utf8');
const verifiedAssignmentMigration = readFileSync('supabase/migrations/20260812110000_verified_question_authority.sql', 'utf8');
test('student assignment lifecycle allows the status used by answer persistence', () => {
    ...
    assert.match(statusMigration, /check \(status in \('pending', 'in_progress', 'completed'\)\)/);
    assert.match(verifiedAssignmentMigration, /update public\.student_assignments set status = 'in_progress'/);
});
test('final assignment submission remains server-authoritative over persisted answers', () => {
    assert.match(verifiedAssignmentMigration, /from public\.student_assignment_answers saa/);
