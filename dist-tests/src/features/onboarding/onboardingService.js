...
const normalizeState = (row) => ({
    ...
    core_completed_at: row['core_completed_at'] ?? null,
    first_value_started_at: row['first_value_started_at'] ?? null,
    first_value_completed_at: row['first_value_completed_at'] ?? null,
    metadata: (row['metadata'] && typeof row['metadata'] === 'object' ? row['metadata'] : {}),
    created_at: row['created_at'],
    updated_at: row['updated_at'],
});
const markLegacyTutorialCompleteForFtue = async (userId) => {
...
export const getCurrentOnboardingUserId = async () => {
...
export const updateOnboardingState = async (patch, userId) => {
    const resolvedUserId = userId ?? await getCurrentOnboardingUserId();
    if (!resolvedUserId)
        return null;
...
export const markOnboardingStepComplete = async (step, options = {}, userId) => {
    const resolvedUserId = userId ?? await getCurrentOnboardingUserId();
    if (!resolvedUserId)
        return null;
    const existing = await getOnboardingState(resolvedUserId);
...
export const fetchOnboardingProfile = async (userId) => {
    const resolvedUserId = userId ?? await getCurrentOnboardingUserId();
    if (!resolvedUserId)
        return null;
...
const getSelectedRoleFromOnboardingState = (onboardingState) => {
...
export const readOnboardingResolution = async (options = {}) => {
