import { AlarmLevel, EntryRoute, FinishReason, GamePhase, HeistCondition, QuestionRiskRoute, } from "./lockdownTypes.js";
import { calculateRegionStats } from "./regionCalculator.js";
import { assignSessionClanColor } from "../../utils/clanColors.js";
 ...
 */
const assignParticipantColor = (state, action) => {
    ...
    const usedColors = new Set(existingPlayers
...
const getEntryRouteModifier = (roomSettings, route) => roomSettings.entryRouteModifiers[route ?? EntryRoute.SAFE];
const updateAlarm = (state, delta) => {
...
export const getAlarmLevel = (alarmValue) => {
    if (alarmValue >= 75) {
...
export const createInitialGameState = (roomSettings) => {
    return {
...
const applyLeave = (state, playerId) => {
...
const applyEntryRoute = (state, playerId, route) => {
    const player = state.players[playerId];
    if (!player)
        return state;
    const updatedPlayer = { ...player, entryRoute: route };
    return { ...state, players: { ...state.players, [playerId]: updatedPlayer } };
};
const applyRiskRoute = (state, playerId, route) => {
    const player = state.players[playerId];
    if (!player)
        return state;
    ...
    const updatedPlayer = { ...player, riskRoute: route };
    return { ...state, players: { ...state.players, [playerId]: updatedPlayer } };
};
const applyVoteCondition = (state, condition) => {
...
const applyFinalizeCondition = (state, condition) => {
...
const coinAdjustmentForAnswer = (roomSettings, correct, route, bonusMultiplier) => {
    if (correct) {
...
const alarmDeltaForAnswer = (roomSettings, correct, route) => {
    if (correct)
        return 0;
    if (route === QuestionRiskRoute.ALL_IN)
        return roomSettings.baseAlarmOnWrong * roomSettings.allInAlarmModifier;
    if (route === QuestionRiskRoute.RISKY)
        return roomSettings.baseAlarmOnWrong * roomSettings.riskyAlarmModifier;
    return roomSettings.baseAlarmOnWrong * roomSettings.safeAlarmModifier;
};
const heatGainForRoute = (roomSettings, route) => {
    if (route === QuestionRiskRoute.ALL_IN)
        return roomSettings.allInHeatGain;
    if (route === QuestionRiskRoute.RISKY)
        return roomSettings.riskyHeatGain;
    return roomSettings.safeHeatGain;
};
const effectiveBonusMultiplier = (state) => {
...
const applyAnswer = (state, playerId, correct, chosenRoute) => {
    const player = state.players[playerId];
    if (!player)
        return state;
...
const applyMostWantedCheck = (state, playerId) => {
    const player = state.players[playerId];
    if (!player)
        return state;
    if (player.heat >= state.roomSettings.mostWantedHeat && !player.mostWanted) {
        const updatedPlayer = { ...player, mostWanted: true };
        return { ...state, players: { ...state.players, [playerId]: updatedPlayer } };
    }
    return state;
};
const applyPanicModeIfNeeded = (state) => {
    if (state.panicModeActive || !shouldEnterPanicMode(state)) {
        return state;
    }
    return applyPanicModeTransform(state);
};
const hackCoins = (attacker, target, roomSettings, ruleSet) => {
...
const applyHack = (state, playerId, targetId) => {
    ...
    const { coinsStolen, updatedTargetCoins, updatedAttackerCoins, alarmMultiplier, heatMultiplier } = hackCoins(attacker, target, state.roomSettings, state.ruleSet);
    ...
    const updatedTarget = { ...target, coins: updatedTargetCoins };
    let updatedState = {
        ...state,
        ...alarmUpdated,
        players: { ...state.players, [playerId]: updatedAttacker, [targetId]: updatedTarget },
    };
    updatedState = applyMostWantedCheck(updatedState, playerId);
    return updatedState;
};
const applyScrub = (state, playerId) => {
    const player = state.players[playerId];
    if (!player)
        return state;
    const reducedHeat = Math.max(0, player.heat - state.roomSettings.scrubHeatReduction);
    const updatedPlayer = { ...player, heat: reducedHeat };
    return { ...state, players: { ...state.players, [playerId]: updatedPlayer } };
};
const applyGreed = (state, playerId) => {
    const player = state.players[playerId];
    if (!player)
        return state;
    const coins = player.coins + state.roomSettings.greedCoinBonus;
    const heat = player.heat + state.roomSettings.greedHeatGain;
    const updatedPlayer = { ...player, coins, heat };
    let updatedState = { ...state, players: { ...state.players, [playerId]: updatedPlayer } };
    updatedState = applyMostWantedCheck(updatedState, playerId);
    return updatedState;
};
const applyPostRoundAction = (state, playerId, action, targetId) => {
    if (action === "hack")
        return applyHack(state, playerId, targetId);
    if (action === "scrub")
        return applyScrub(state, playerId);
    return applyGreed(state, playerId);
};
const applyChaos = (state, effect) => {
...
const applyTick = (state, elapsedMs) => {
    const now = Date.now();
...
const totalCoins = (state) => Object.values(state.players).reduce((sum, player) => sum + player.coins, 0);
export const shouldEnterPanicMode = (state) => {
    if (state.panicModeActive)
        return false;
    if (state.alarm >= state.roomSettings.panicAlarmThreshold)
        return true;
    return state.remainingTimeMs <= state.roomSettings.panicTimeThresholdMs;
};
export const applyPanicModeTransform = (state) => {
    const updatedRuleSet = { ...state.ruleSet, safeRouteDisabled: true };
    return { ...state, panicModeActive: true, ruleSet: updatedRuleSet };
};
export const evaluateEndCondition = (state) => {
...
const finalizeIfFinished = (state) => {
