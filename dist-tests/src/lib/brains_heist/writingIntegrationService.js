import { createEmptyErrorMemory, generateWeeklyImprovementPlan, storeAttemptInErrorMemory, } from './writingAssessment.js';
import { generateDailyWritingTasksForWeek } from './writingTaskGenerator.js';
import { createInitialStudentWritingState, runDailyWritingPracticeFlow, runInitialWritingAssessmentFlow, runMonthlyWritingReviewFlow, runWeeklyWritingReviewFlow, } from './writingWorkflowOrchestrator.js';
...
const GENRE_KEYS = ['email', 'article', 'review', 'story', 'essay', 'report', 'paragraph'];
const isKnownGenre = (genre) => GENRE_KEYS.includes(genre);
const isLikelyUuid = (value) => {
    if (typeof value !== 'string')
        return false;
    return /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value.trim());
};
const isReadableStudentLabel = (value) => typeof value === 'string' && value.trim().length > 0 && !isLikelyUuid(value);
const resolveStudentLabel = (studentId, preferredName) => {
    if (isReadableStudentLabel(preferredName))
        return preferredName.trim();
    const profileName = store.profiles.get(studentId)?.student_name;
    if (isReadableStudentLabel(profileName))
        return profileName.trim();
    const username = store.usernamesById[studentId];
    if (isReadableStudentLabel(username))
        return username.trim();
    return 'Student';
};
const buildStateKey = (studentId, genre) => `${studentId}::${genre}`;
const parseStateKey = (key) => {
    const [studentId, rawGenre] = key.split('::');
    if (studentId && rawGenre && isKnownGenre(rawGenre))
        return { studentId, genre: rawGenre };
    return { studentId: key, genre: null };
};
const resolveGenreFromState = (state, fallback) => {
    if (state?.current_genre && isKnownGenre(state.current_genre))
        return state.current_genre;
    if (fallback && isKnownGenre(fallback))
        return fallback;
    return null;
};
const getProfileGenre = (studentId) => {
    const genre = store.profiles.get(studentId)?.current_genre;
    return genre && isKnownGenre(genre) ? genre : 'essay';
};
const getStateForGenre = (studentId, genre) => {
    const resolvedGenre = genre ?? getProfileGenre(studentId);
    return store.states.get(buildStateKey(studentId, resolvedGenre)) ?? null;
};
const setStateForGenre = (studentId, genre, state) => {
...
let lastPersistenceMode = 'runtime-only';
...
const notifyHydrationListeners = () => {
...
const applyFallbackSnapshot = (storage) => {
    if (!storage)
        return false;
    try {
        ...
        store.profiles = new Map(fallback.profiles);
        store.usernamesById = fallback.usernamesById ?? {};
        const loadedStates = new Map();
        fallback.states.forEach(([key, value]) => {
        ...
        store.attempts = (fallback.attempts ?? []);
        store.weeklyPlans = (fallback.weeklyPlans ?? []);
        store.dailyTasks = (fallback.dailyTasks ?? []);
        store.dailySubmissions = (fallback.dailySubmissions ?? []);
        store.dailyEvaluations = (fallback.dailyEvaluations ?? []);
        store.monthlyReports = (fallback.monthlyReports ?? []);
        store.memorySnapshots = (fallback.memorySnapshots ?? []);
        store.promptBank = (fallback.promptBank ?? []);
        store.reviewSignals = (fallback.reviewSignals ?? []);
...
const applyHydratedSnapshot = (parsed, hydrateStartVersion, reason, generation) => {
    ...
    store.profiles = new Map(parsed.profiles);
    store.usernamesById = parsed.usernamesById ?? {};
    const loadedStates = new Map();
    parsed.states.forEach(([key, value]) => {
        const { studentId, genre } = parseStateKey(key);
        const resolvedGenre = resolveGenreFromState(value, genre ?? undefined);
        if (!resolvedGenre) {
            loadedStates.set(key, value);
    ...
    store.weeklyPlans = (parsed.weeklyPlans ?? []);
    store.dailyTasks = (parsed.dailyTasks ?? []);
    store.dailySubmissions = (parsed.dailySubmissions ?? []);
    store.dailyEvaluations = (parsed.dailyEvaluations ?? []);
    store.monthlyReports = (parsed.monthlyReports ?? []);
    store.memorySnapshots = (parsed.memorySnapshots ?? []);
    store.promptBank = (parsed.promptBank ?? []);
    store.reviewSignals = (parsed.reviewSignals ?? []);
    ...
    hydrationInFlight = dbLoad
        ...
        if (!parsed) {
            ...
            void dbLoadPromise
                .then((lateParsed) => {
                if (lateParsed)
                    applyHydratedSnapshot(lateParsed, hydrateStartVersion, 'late-after-timeout', generation);
            })
                .catch((error) => {
...
hydrateStore();
export const getWritingHydrationStatus = () => {
    hydrateStore();
    return hydrationState;
};
export const retryWritingHydration = () => {
...
const clearStudentScopedWritingStore = () => {
...
export const ensureWritingHydrationForStudent = async (studentId) => {
    const expectedStudentId = studentId.trim();
    if (!expectedStudentId)
        return;
    if (hydrationState === 'ready' && hydratedStudentId === expectedStudentId)
        return;
...
export const subscribeToWritingHydrationStatus = (listener) => {
...
export const getWritingPersistenceStatus = () => persistenceStatus;
export const subscribeToWritingPersistenceStatus = (listener) => {
...
const isValidGenre = (genre) => GENRE_KEYS.includes(genre);
...
const weekKey = (isoDate) => isoDate.slice(0, 10);
const WRITING_WEAKNESS_TAGS = [
...
const WRITING_WEAKNESS_TAG_SET = new Set(WRITING_WEAKNESS_TAGS);
const readObjectArray = (value) => Array.isArray(value)
    ? value.filter((item) => Boolean(item && typeof item === 'object' && !Array.isArray(item)))
    : [];
const tokenizeWords = (value) => value.toLowerCase().match(/[a-z]+/g) ?? [];
const levenshteinDistance = (left, right) => {
    ...
    for (let leftIndex = 1; leftIndex <= left.length; leftIndex += 1) {
        ...
        for (let rightIndex = 1; rightIndex <= right.length; rightIndex += 1) {
            ...
            previous[rightIndex] = Math.min(previous[rightIndex] + 1, previous[rightIndex - 1] + 1, diagonal + replacementCost);
            diagonal = above;
        }
    }
    return previous[right.length];
};
const containsLikelySpellingCorrection = (original, better) => {
    ...
    return removed.some((before) => before.length >= 4 && added.some((after) => Math.abs(before.length - after.length) <= 2 && levenshteinDistance(before, after) <= 2));
};
const inferFeedbackWeaknessTagCounts = (feedback) => {
    const counts = new Map();
    const increment = (tag, amount = 1) => {
        if (amount <= 0)
            return;
    ...
    readObjectArray(feedback['grammar_fixes']).forEach((fix) => {
        ...
        if (/agreement|subject.?verb|\bthere is\b.*\bthere are\b|\bthere are\b.*\bthere is\b|\bhas\b.*\bhave\b|\bhave\b.*\bhas\b/i.test(combined))
            increment('agreement_error');
        else if (/tense|past tense|present tense|verb time/i.test(combined))
            increment('tense_error');
        else if (/\barticle\b|\ba\b.*\ban\b|\ban\b.*\ba\b|\bthe\b/i.test(issue))
            increment('article_error');
        else if (/preposition/i.test(issue))
            increment('preposition_error');
        else if (/fragment|incomplete sentence/i.test(issue))
            increment('fragment');
        else if (/run.?on|sentence boundary/i.test(issue))
            increment('run_on');
        else if (/spelling|misspell|typo/i.test(issue) || containsLikelySpellingCorrection(original, better))
            increment('spelling_error');
    });
    ...
    explicitTags.forEach((tag) => {
        if (typeof tag === 'string' && WRITING_WEAKNESS_TAG_SET.has(tag) && !counts.has(tag)) {
            increment(tag);
        }
    });
...
const synchronizeSavedFeedbackWeaknessMemory = (studentId, genre) => {
    let changed = false;
    store.attempts
        .filter((attempt) => attempt.student_id === studentId && attempt.genre === genre)
        .forEach((attempt) => {
        if (!attempt.rich_feedback || typeof attempt.rich_feedback !== 'object' || Array.isArray(attempt.rich_feedback))
            return;
    ...
    const currentWeek = store.weeklyPlans
        ...
        .sort((left, right) => right.created_at.localeCompare(left.created_at))[0];
    if (currentWeek)
        currentWeek.plan = weeklyPlan;
    persistStore();
    return true;
};
const badRequest = (message) => ({ ok: false, error: message });
const ok = (data) => ({ ok: true, data });
export const submitInitialWritingAssessment = (input) => {
    if (!input.student_id?.trim())
        return badRequest('student_id is required.');
    const normalizedGrade = normalizeGrade(input.grade);
    if (normalizedGrade === null)
        return badRequest('grade must be an integer between 1 and 12.');
    if (!isValidGenre(input.genre))
        return badRequest('genre is invalid.');
    const normalizedGenre = input.genre;
    if (!input.prompt_text?.trim())
        return badRequest('prompt_text is required.');
    if (!Number.isFinite(input.target_word_count) || input.target_word_count < 20)
...
export const getStudentWritingState = (studentId, genre) => {
    hydrateStore();
    const resolvedGenre = genre ?? getProfileGenre(studentId);
    ...
    if (!state) {
        ...
        if (!profile)
            return badRequest('student writing state not found.');
...
export const getStudentWritingHubSnapshot = (studentId, genre) => {
    hydrateStore();
...
export const listStudentWritingHistoryByGenre = (studentId) => {
    if (!studentId?.trim())
        return badRequest('student_id is required.');
    hydrateStore();
    ...
    Array.from(dedupedAttempts.values()).forEach((attempt) => {
        ...
        const entry = {
            ...
            grammar_issue_count: Math.max(readObjectArray(feedback?.['grammar_fixes']).length, attempt.assessment?.weakness_tags?.filter((tag) => /agreement|tense|article|preposition|fragment|run_on|spelling/i.test(String(tag))).length ?? 0),
            punctuation_issue_count: Math.max(readObjectArray(feedback?.['punctuation_fixes']).length, attempt.assessment?.weakness_tags?.filter((tag) => /punctuation|capital/i.test(String(tag))).length ?? 0),
            feedback_quick_fixes: Array.isArray(feedback?.['quick_fixes'])
                ? (feedback?.['quick_fixes']).map((item) => ({
...
export const getStudentPromptAttemptCount = (input) => {
    ...
    if (!input.prompt_text?.trim())
        return badRequest('prompt_text is required.');
    hydrateStore();
    const normalizePrompt = (value) => value.replace(/\s+/g, ' ').trim().toLowerCase();
    const targetPrompt = normalizePrompt(input.prompt_text);
    const count = store.attempts.filter((attempt) => attempt.student_id === input.student_id &&
        attempt.genre === input.genre &&
        normalizePrompt(attempt.prompt_text ?? '') === targetPrompt).length;
    return ok({ count });
};
export const persistInitialWritingRichFeedback = (input) => {
    if (!input.student_id?.trim())
        return badRequest('student_id is required.');
    if (!input.rich_feedback || typeof input.rich_feedback !== 'object')
        return badRequest('rich_feedback is required.');
    hydrateStore();
    const attempts = store.attempts
        .filter((attempt) => attempt.student_id === input.student_id &&
        attempt.genre === input.genre &&
    ...
    const targetAttempt = input.attempt_id
        ...
        : attempts[0];
    if (!targetAttempt)
        return badRequest('initial assessment attempt not found.');
    const richFeedbackClone = JSON.parse(JSON.stringify(input.rich_feedback));
    const fingerprint = typeof richFeedbackClone['text_fingerprint'] === 'string' && richFeedbackClone['text_fingerprint'].trim()
...
export const getStudentGenrePathStatuses = (studentId, genres) => {
    hydrateStore();
...
export const getCurrentWeeklyPlan = (studentId, genre) => {
    hydrateStore();
    const state = getStateForGenre(studentId, genre);
    if (!state || !state.active_week_plan)
        return badRequest('active weekly plan not found.');
    return ok(state.active_week_plan);
};
export const getTodayWritingTask = (studentId, genre) => {
    hydrateStore();
    const state = getStateForGenre(studentId, genre);
    if (!state)
        return badRequest('student writing state not found.');
    ...
    if (!nextTask)
        return badRequest('no pending daily task found.');
    return ok(nextTask);
};
export const submitDailyWritingPractice = (input) => {
    if (!input.student_id?.trim())
        return badRequest('student_id is required.');
    const normalizedGenre = input.genre && isValidGenre(input.genre) ? input.genre : getProfileGenre(input.student_id);
    if (!Number.isInteger(input.day_number) || input.day_number <= 0)
        return badRequest('day_number must be a positive integer.');
    if (!input.submission_text?.trim())
        return badRequest('submission_text is required.');
    ...
    if (!task)
        return badRequest('daily task not found for provided day_number.');
...
export const getWeeklyWritingReview = (studentId, genre) => {
    hydrateStore();
    const state = getStateForGenre(studentId, genre);
    if (!state)
        return badRequest('student writing state not found.');
...
export const getMonthlyWritingReport = (studentId, month, genre) => {
    hydrateStore();
    const resolvedGenre = genre ?? getProfileGenre(studentId);
    const state = getStateForGenre(studentId, resolvedGenre);
    if (!state)
    ...
    if (!hasActivity)
        return badRequest('monthly report unavailable until writing activity exists.');
...
const buildRetryInsights = (attempts) => {
    ...
    const retryDepthCounter = new Map();
    const repeatedTagCounter = new Map();
    const samePromptDeltas = [];
    const profiles = new Map();
    let samePromptRetryCount = 0;
    let newPromptRestartCount = 0;
    let cyclesImproved = 0;
    let cyclesNotImproved = 0;
    const ensureProfile = (studentId) => {
    ...
    cycleMap.forEach((items, cycleKey) => {
        ...
        const sorted = [...items].sort((a, b) => {
            const attemptA = Number.isFinite(a.attempt_number) ? a.attempt_number : Number.POSITIVE_INFINITY;
            const attemptB = Number.isFinite(b.attempt_number) ? b.attempt_number : Number.POSITIVE_INFINITY;
    ...
    const studentProfiles = [...profiles.values()].map((profile) => {
        ...
        const deltas = studentCycles
            ...
            if (!sorted.some((attempt) => attempt.retry_kind === 'same_prompt'))
                return null;
...
export const getWritingMonitoringOverview = (month = currentMonthKey()) => {
    hydrateStore();
    ...
    for (const [stateKey, state] of store.states.entries()) {
        ...
        const trend = {
            content: first && last ? Number((last.subscores.content - first.subscores.content).toFixed(2)) : 0,
            communicative_achievement: first && last
                ? Number(((last.subscores.communicative_achievement ?? 0) -
                    (first.subscores.communicative_achievement ?? 0)).toFixed(2))
                : 0,
            organisation: first && last ? Number((last.subscores.organisation - first.subscores.organisation).toFixed(2)) : 0,
            language: first && last ? Number((last.subscores.language - first.subscores.language).toFixed(2)) : 0,
        };
        ...
        const lowCompletionRisk = totalTasks > 0 && completed > 0 && completionRate < WRITING_PILOT_GUARDRAILS.stalled_completion_rate_threshold;
        const stalled = hasSubmissions && (state.adaptation_trend.failure_streak >= WRITING_PILOT_GUARDRAILS.stalled_failure_streak_threshold ||
            lowCompletionRisk ||
            negativeTrendCount >= 2);
        const improving = !stalled &&
                ...
                : stalled
                    ...
                    : improving
                        ...
                        : attemptsCount === 1 && completed === 0
                            ? 'plan_ready'
                            : 'on_track';
        const statusReason = status === 'not_started'
...
export const getWritingCalibrationCase = (studentId, month = currentMonthKey(), genre) => {
    hydrateStore();
    const targetGenre = genre ?? getProfileGenre(studentId);
    ...
    const latestPracticeEvaluations = store.dailyEvaluations
        ...
        .sort((a, b) => b.created_at.localeCompare(a.created_at))
        .slice(0, 3);
    const monthlyReport = store.monthlyReports
...
export const exportStudentMonthlyWritingReport = (studentId, month, genre) => {
    hydrateStore();
    const targetGenre = genre ?? getProfileGenre(studentId);
    const state = getStateForGenre(studentId, targetGenre);
    if (!state)
    ...
    if (!monthly)
        return badRequest('monthly report not found for this student/month.');
...
export const exportTeacherWeeklyClassSummary = (month = currentMonthKey()) => {
    ...
    if (!overview.ok || !overview.data)
        return badRequest('writing monitoring overview unavailable.');
    if (overview.data.student_rows.length === 0)
        return badRequest('no class writing data available to export.');
    const rows = overview.data.student_rows.map((row) => `${row.student_name} (${row.student_id}) | completion ${Math.round(row.completion_rate * 100)}% | latest ${row.latest_score ?? '—'} | status ${row.stalled ? 'stalled' : row.improving ? 'improving' : 'steady'}`);
    const html = `
    <article>
      ...
      <h2>Improving students</h2><p>${safeText(overview.data.student_rows.filter((row) => row.improving).map((row) => row.student_id).join(', ') || 'None')}</p>
    </article>
  `.trim();
    return ok({
...
export const getTeacherExportRowsScoped = async (month = new Date().toISOString().slice(0, 7)) => {
    if (typeof process !== 'undefined' && process.env?.['NODE_ENV'] === 'test') {
        const overview = getWritingMonitoringOverview(month);
        if (!overview.ok || !overview.data)
            return badRequest(overview.error ?? 'No export rows available.');
        return ok(overview.data.student_rows.map((row) => ({
    ...
    try {
        ...
        if (error || !data)
            return badRequest(error?.message ?? 'Unable to load scoped teacher export rows.');
        return ok(data);
    }
    catch (error) {
...
export const exportAdminCalibrationReport = (studentId, month = currentMonthKey()) => {
    const calibration = getWritingCalibrationCase(studentId, month);
    if (!calibration.ok || !calibration.data)
        return badRequest('calibration data unavailable.');
    ...
    <article>
      ...
      <h2>Evaluations</h2><p>${safeText(data.latest_practice_evaluations
    ...
    return ok({
        ...
        pdf_ready: {
            ...
            sections: [
                ...
                {
                    title: 'Evaluations',
                    lines: data.latest_practice_evaluations.map((item) => `Day ${item.task_day_number}: ${item.evaluation.completion_status}/${item.evaluation.recommended_next_action}`),
                },
...
export const getWritingAnalyticsDashboard = (filters = {}) => {
    hydrateStore();
    const students = [...store.states.values()].filter((state) => {
        if (filters.grade && state.grade !== filters.grade)
            return false;
        if (filters.genre && state.current_genre !== filters.genre)
            return false;
        return true;
    });
    if (students.length === 0)
        return badRequest('no writing analytics data available for the selected filters.');
    const weaknessCounter = new Map();
    const studentWeaknessCounts = [];
    const gradeScores = new Map();
    const genreScores = new Map();
    const subscaleImprovement = [];
    ...
    for (const state of students) {
        ...
        attempts.forEach((attempt) => {
            const counts = attempt.feedback_weakness_tag_counts ?? Object.fromEntries((attempt.assessment?.weakness_tags ?? []).map((tag) => [tag, 1]));
    ...
    for (const prompt of store.promptBank) {
        ...
        const avg = matchedAttempts.length > 0
            ? Number((matchedAttempts.reduce((acc, item) => acc + item.assessment.total_score, 0) / matchedAttempts.length).toFixed(2))
            : null;
    ...
    for (const evaluation of store.dailyEvaluations) {
        ...
        if (evaluation.evaluation.detected_weakness_tags.length > 0)
            current.repeatedWeaknessHits += 1;
        current.total += 1;
    ...
    const lowImprovementTags = [...weaknessCounter.entries()]
        ...
        .map(([tag]) => tag);
    const retryInsights = buildRetryInsights(store.attempts.filter((item) => {
    ...
    return ok({
        ...
        task_type_effectiveness: [...taskTypeStats.entries()].map(([task_type, stats]) => ({
            task_type,
            average_target_skill_score: Number((stats.targetScores.reduce((acc, item) => acc + item, 0) / Math.max(1, stats.targetScores.length)).toFixed(2)),
...
const gradeMatchesBand = (grade, band) => {
...
const normalizePromptText = (prompt) => prompt.replace(/\s+/g, ' ').trim().toLowerCase();
const ensureStructuredPromptBankSeeded = () => {
    ...
    STRUCTURED_WRITING_PROMPT_BANK.forEach((prompt) => {
        ...
        if (existingIds.has(prompt.id) || existingPromptTexts.has(normalizedText))
            return;
        const record = {
            id: prompt.id,
            title: prompt.title,
...
const resolvePromptFocusAndContext = (prompt) => {
...
const normalizeDifficultyForState = (grade, stateDifficulty) => {
    const gradeDefault = gradeToDifficultyLevel(grade);
    if (stateDifficulty === 'reduced')
        return 'foundational';
    if (stateDifficulty === 'increased')
        return gradeDefault === 'foundational' ? 'core' : 'stretch';
    return gradeDefault;
};
export const createWritingPrompt = (input) => {
    hydrateStore();
    if (!input.title.trim())
    ...
    if (!isValidGenre(input.genre))
        return badRequest('genre is invalid.');
    if (!input.grade_band.trim())
        return badRequest('grade_band is required.');
    if (!Number.isFinite(input.target_word_count) || input.target_word_count < 20)
        return badRequest('target_word_count must be >= 20.');
    const now = new Date().toISOString();
    const prompt = {
...
export const archiveWritingPrompt = (promptId) => {
    hydrateStore();
    const prompt = store.promptBank.find((item) => item.id === promptId);
    if (!prompt)
        return badRequest('prompt not found.');
...
export const setWritingPromptActiveStatus = (promptId, isActive) => {
    hydrateStore();
    const prompt = store.promptBank.find((item) => item.id === promptId);
    if (!prompt)
        return badRequest('prompt not found.');
    if (prompt.is_archived)
        return badRequest('archived prompts cannot be activated.');
    prompt.is_active = isActive;
    prompt.updated_at = new Date().toISOString();
    persistStore();
    return ok(prompt);
};
export const listWritingPrompts = (filters = {}) => {
    hydrateStore();
    const results = store.promptBank.filter((prompt) => {
        if (filters.genre && prompt.genre !== filters.genre)
            return false;
        if (typeof filters.is_active === 'boolean' && prompt.is_active !== filters.is_active)
            return false;
        if (filters.difficulty_label && prompt.difficulty_label !== filters.difficulty_label)
            return false;
        if (filters.grade && !gradeMatchesBand(filters.grade, prompt.grade_band))
            return false;
        if (filters.prompt_quality_flag && prompt.prompt_quality_flag !== filters.prompt_quality_flag)
            return false;
        return true;
    });
    return ok(results);
...
export const getSmartWritingPromptForStudent = async (input) => {
    ...
    const historicalWeaknessTags = Object.entries(historicalTagCounts)
        .filter((entry) => WRITING_WEAKNESS_TAG_SET.has(entry[0]) && Number(entry[1] ?? 0) > 0)
    ...
    const weaknessFocusTargets = weaknessTags.flatMap((tag) => WEAKNESS_TAG_TO_PROMPT_FOCUS[tag] ?? []);
    ...
    if (!isTestRuntime) {
        try {
            ...
            if (!error && remotePrompt) {
                ...
                const focusTags = Array.isArray(remote?.['focus_tags']) ? remote['focus_tags'] : uniqueFocusTargets;
                const contextTags = Array.isArray(remote?.['context_tags']) ? remote['context_tags'] : [];
                return ok({
            ...
            if (error)
                console.warn('[writingIntegrationService] Student prompt RPC unavailable; using safe fallback.', error.message);
        }
        catch (error) {
    ...
    const recentPromptIds = recentAttempts.map((attempt) => attempt.prompt_id).filter((value) => Boolean(value));
    ...
    const hasCompletedStarterEmailPrompt = store.attempts.some((attempt) => attempt.student_id === input.student_id &&
        attempt.genre === 'email' &&
    ...
    const uniqueCandidates = [...new Map(allCandidates.map((prompt) => [`${prompt.id}|${normalizePromptText(prompt.prompt_text)}`, prompt])).values()];
    ...
    const ranked = candidates
        ...
        const recencyPenalty = Math.max(recentIndex === -1 ? 0 : recentIndex < 2 ? 120 : 50, recentIdIndex === -1 ? 0 : recentIdIndex < 2 ? 120 : 50);
        const sameAsCurrentPenalty = (currentPromptNormalized && currentPromptNormalized === normalizedPrompt) || prompt.id === input.current_prompt_id
            ? 500
            : 0;
        ...
        if (a.prompt.usage_count !== b.prompt.usage_count)
    ...
    if (input.use_ai_polish) {
        ...
        if (rewrite.ok && rewrite.data) {
            const ai = (rewrite.data.result ?? {});
            const polished = ai.rewritten_prompt?.trim();
            if (polished)
                finalPromptText = polished;
        }
    }
    persistStore();
    return ok({
...
export const getNextRotatedPromptForStudent = (input) => {
    ...
    if (candidates.length === 0)
        return badRequest('no active prompt available for the requested student profile.');
    const withSortMeta = candidates
        ...
        .sort((a, b) => {
        if (a.seenRecently !== b.seenRecently)
            return Number(a.seenRecently) - Number(b.seenRecently);
...
export const saveAdminReviewSignal = (input) => {
    ...
    const now = new Date().toISOString();
    const existing = store.reviewSignals.find((item) => item.entity_type === input.entity_type && item.entity_id === input.entity_id && item.student_id === input.student_id);
    if (existing) {
...
export const listAdminReviewSignals = (filters = {}) => {
    hydrateStore();
    return ok(store.reviewSignals.filter((item) => {
        if (filters.status && item.status !== filters.status)
            return false;
        if (filters.entity_type && item.entity_type !== filters.entity_type)
            return false;
        if (filters.student_id && item.student_id !== filters.student_id)
            return false;
        return true;
    }));
};
export const setPromptQualityFlag = (promptId, flag, note) => {
    hydrateStore();
    const prompt = store.promptBank.find((item) => item.id === promptId);
    if (!prompt)
        return badRequest('prompt not found.');
...
export const setCalibrationFollowUpFlag = (studentId, flagged, note) => {
    hydrateStore();
...
export const seedWritingPilotReadinessDemoData = () => {
    ...
    const seededStudents = [
        { id: 'seed-lower-struggling', grade: 6, genre: 'paragraph', response: 'I is go event short', strong: false },
        {
        ...
        { id: 'seed-stalled', grade: 7, genre: 'email', response: 'Bad short text', strong: false },
        {
            id: 'seed-improving',
            grade: 9,
            genre: 'essay',
        ...
        { id: 'seed-low-improvement-tag', grade: 8, genre: 'article', response: 'Tiny weak line', strong: false },
    ];
    createWritingPrompt({
...
export const runWritingPilotVerificationChecklist = () => {
    const seeded = seedWritingPilotReadinessDemoData();
    if (!seeded.ok || !seeded.data)
        return badRequest('failed to seed pilot readiness demo data.');
...
export const recordWritingIntegrityVoid = async (input) => {
    try {
        ...
        if (error || !data)
            return badRequest(error?.message ?? 'Unable to preserve the interrupted attempt.');
...
export const getStudentWritingIntegrityMode = async () => ok({ mode: 'formal', class_id: null, class_name: 'Formal Cambridge-aligned assessment' });
export const setTeacherWritingIntegrityMode = async (input) => {
    if (!input.class_id)
        return badRequest('Select a class before changing its writing mode.');
    return ok({ mode: 'formal', class_id: input.class_id, class_name: 'Formal Cambridge-aligned assessment' });
};
export const mapCalibrationCaseToTeacherReport = (c, month, genre = 'essay') => {
    ...
    return {
        ...
        latest_evaluation: c.latest_practice_evaluations[0]?.evaluation ?? {},
        monthly_summary: c.monthly_report_snapshot?.report ?? {},
...
export const getTeacherAttemptListScoped = async (input) => {
    try {
        ...
        if (error || !data)
            return badRequest(error?.message ?? 'Unable to load teacher attempt list.');
        const seenAttemptIds = new Set();
        const deduplicated = data.filter((attempt) => {
...
export const getTeacherGeneralReportScoped = async (input) => {
    try {
        ...
        if (error || !data)
            return badRequest(error?.message ?? 'Unable to load teacher general report.');
        return ok(data);
    }
    catch (error) {
...
export const getTeacherAttemptReportScoped = async (input) => {
    try {
        ...
        if (error || !data)
            return badRequest(error?.message ?? 'Unable to load teacher attempt report.');
        return ok(data);
    }
    catch (error) {
...
export const getTeacherSavedReportsScoped = async (input) => {
    try {
        ...
        if (error || !data)
            return badRequest(error?.message ?? 'Unable to load saved teacher reports.');
        return ok(data);
    }
    catch (error) {
...
export const saveTeacherReportScoped = async (input) => {
    try {
        ...
        if (error || !data)
            return badRequest(error?.message ?? 'Unable to save teacher report.');
        return ok(data);
    }
    catch (error) {
...
export const getTeacherMonitoringOverviewScoped = async (month = new Date().toISOString().slice(0, 7), filters) => {
    if (isNodeTestRuntime())
        return getWritingMonitoringOverview(month);
    try {
        const { supabase } = await import('../../../services/supabaseClient.js');
        ...
        if (error || !data)
            return badRequest(error?.message ?? 'Unable to load scoped teacher monitoring.');
        return ok(data);
    }
    catch (error) {
...
export const getTeacherAnalyticsDashboardScoped = async (month = new Date().toISOString().slice(0, 7), filters) => {
    if (isNodeTestRuntime())
        return getWritingAnalyticsDashboard({ grade: filters?.grade, genre: filters?.genre });
    try {
        const { supabase } = await import('../../../services/supabaseClient.js');
        ...
        if (error || !data)
            return badRequest(error?.message ?? 'Unable to load scoped teacher analytics.');
        const analytics = data;
        if (!weaknessResult.error && weaknessResult.data) {
            const weaknessData = weaknessResult.data;
...
export const getTeacherCalibrationQueueScoped = async (month = new Date().toISOString().slice(0, 7), limit = 50) => {
    if (isNodeTestRuntime()) {
        const monitoring = getWritingMonitoringOverview(month);
        if (!monitoring.ok || !monitoring.data)
            return badRequest(monitoring.error ?? 'No calibration data available.');
        return ok(monitoring.data.student_rows.map((row) => ({
            student_id: row.student_id,
            student_name: row.student_name,
            grade: row.current_grade,
            latest_score: row.latest_score,
    ...
    try {
        ...
        if (error || !data)
            return badRequest(error?.message ?? 'Unable to load scoped calibration queue.');
        return ok(data);
    }
    catch (error) {
...
export const getTeacherStudentSummaryScoped = async (input) => {
    if (isNodeTestRuntime()) {
        ...
        if (!calibration.ok || !calibration.data)
            return badRequest(calibration.error ?? 'No student summary available.');
        return ok(mapCalibrationCaseToTeacherReport(calibration.data, input.month ?? new Date().toISOString().slice(0, 7), input.genre ?? 'essay'));
    }
    return getTeacherWritingReport(input);
};
export const getWritingPersistenceDiagnostics = () => {
    return ok({
        mode: lastPersistenceMode,
        repository_mode: getWritingRepositoryMode(),
    });
};
export const __resetWritingIntegrationStoreForTests = () => {
    store.profiles.clear();
    store.usernamesById = {};
    store.states.clear();
    store.attempts = [];
    ...
    const storage = getStorage();
    if (storage)
        storage.removeItem(WRITING_STORE_KEY);
};
export const __getWritingIntegrationStoreForTests = () => store;
