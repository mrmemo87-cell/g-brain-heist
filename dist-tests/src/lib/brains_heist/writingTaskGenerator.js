...
const supportLevelForGrade = (grade) => {
    if (grade <= 7)
        return 'high';
    if (grade <= 9)
        return 'medium';
...
const baseWordCountForGrade = (grade) => {
    if (grade <= 7)
        return 70;
    if (grade <= 9)
        return 110;
...
const inferFocusArea = (primaryTarget) => {
    const text = primaryTarget.toLowerCase();
    if (/tone|register|audience|genre|communicative/.test(text))
        return 'communicative';
    if (/paragraph|sequencing|linking|organisation|flow/.test(text))
        return 'organisation';
    if (/content|coverage|under length|missed/.test(text))
        return 'content';
    return 'language';
};
...
const buildCriteria = (tags, genre, includeWordCount) => {
    ...
    if (tags.includes('missed_content_point') || tags.includes('partial_content_coverage'))
        criteria.push('Covers all required content points.');
    if (tags.includes('weak_paragraphing') || tags.includes('poor_sequencing'))
        criteria.push('Uses clear paragraphing and logical sequencing.');
    if (tags.includes('tense_error'))
        criteria.push('Avoids repeated tense errors.');
    if (tags.includes('agreement_error'))
        criteria.push('Maintains correct subject–verb agreement.');
    if (tags.includes('wrong_tone') || tags.includes('weak_register_control'))
        criteria.push('Matches tone and register to task purpose.');
    if (genre === 'email')
        criteria.push('Includes a suitable opening and closing.');
    if (includeWordCount)
        criteria.push('Stays within the target word range.');
...
const blueprintsForFocus = (focus) => {
    if (focus === 'language') {
        return [
...
const withGradeScaffolding = (grade, instructions) => {
    if (grade <= 7)
        return `${instructions} Use the checklist and start with one simple plan sentence.`;
    if (grade <= 9)
        return `${instructions} Use the short checklist before you write your final version.`;
    return `${instructions} Plan first, then check style, structure, and accuracy before submission.`;
};
export const generateDailyWritingTasksForWeek = (input) => {
    ...
    return tasks.map((task, index) => {
        ...
        const expectedWords = task.task_type === 'sentence correction' || task.task_type === 'error spotting' || task.task_type === 'sentence combining'
        ...
        const instructions = withGradeScaffolding(input.grade, `Today, complete a ${task.task_type} practice for ${input.target_genre} writing. Main focus: ${input.weekly_plan.primary_target}. Second focus: ${input.weekly_plan.secondary_target}. Keep this writing style in mind: ${GENRE_FOCUS[input.target_genre]}.`);
