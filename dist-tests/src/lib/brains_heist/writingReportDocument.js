const escapeHtml = (value) => String(value ?? '')
    ...
    .replaceAll('"', '&quot;')
    .replaceAll("'", '&#39;');
export const humanizeWritingTag = (value) => {
    const overrides = {
...
const scoreValue = (value) => value == null || Number.isNaN(Number(value)) ? '—' : `${Number(value)}/20`;
...
export const buildProfessionalWritingReportHtml = (report, options) => {
    ...
    const actions = report.teacher_actions.map((item) => item.replace(/[_]+/g, ' ').replace(/\s+/g, ' ').trim());
...
export const openProfessionalWritingReport = (report, options) => {
