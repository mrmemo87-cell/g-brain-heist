import { TRUSTED_WRITING_EVALUATOR_VERSIONS, WRITING_RUBRIC_VERSION, } from './writingAssessment.js';
...
const allowedStrengthTags = new Set(WRITING_STRENGTH_TAGS);
const asRecord = (value) => value && typeof value === 'object' && !Array.isArray(value) ? value : null;
const asNonEmptyString = (value) => typeof value === 'string' && value.trim().length > 0 ? value.trim() : null;
const asStringArray = (value, minimum = 0) => {
    if (!Array.isArray(value))
        return null;
    const values = value.map(asNonEmptyString).filter((item) => Boolean(item));
    return values.length >= minimum ? values : null;
};
export const normalizeWritingTextForFingerprint = (value) => value
    .toLowerCase()
    .replace(/[“”]/g, '"')
    .replace(/[‘’]/g, "'")
    .replace(/\s+/g, ' ')
    .trim();
export const buildWritingTextFingerprint = (value) => {
...
const normalizeEvidence = (value, studentResponse) => {
    ...
    for (const item of value) {
        ...
        if (!quote || !Number.isInteger(start) || !Number.isInteger(end))
            return null;
        const startChar = Number(start);
        const endChar = Number(end);
        if (startChar < 0 || endChar !== startChar + quote.length)
            return null;
        if (studentResponse.slice(startChar, endChar) !== quote)
            return null;
        spans.push({ quote, start_char: startChar, end_char: endChar });
    }
    return spans.slice(0, 6);
};
const normalizeCriterion = (value, studentResponse) => {
    ...
    if (typeof confidence !== 'number' || !Number.isFinite(confidence) || confidence < 0 || confidence > 1)
        return null;
    if (!descriptorId || !justification || justification.length < 12 || !improvementAction || !evidence)
        return null;
    return {
        score: Number(score),
        confidence,
...
const normalizePromptDefinition = (value, context) => {
    const record = asRecord(value);
    if (!record)
        return null;
    ...
    if (!Number.isInteger(contextGrade) || contextGrade < 1 || contextGrade > 12)
        return null;
    if (grade !== contextGrade || genre !== context.genre || targetWordCount !== context.targetWordCount)
        return null;
    if (promptId !== context.promptId || !validRegister || !validDifficulty)
        return null;
    ...
    if (!promptDefinitionHash || !audience || !purpose)
        return null;
    return {
        prompt_id: promptId,
...
const normalizeFeedback = (value, fingerprint, studentResponse) => {
    const record = asRecord(value);
    if (!record || record['text_fingerprint'] !== fingerprint)
        return null;
    ...
    const strengthEvidence = Array.isArray(record['strength_evidence'])
        ? record['strength_evidence'].flatMap((item) => {
            ...
            if (!allowedStrengthTags.has(tag) || !evidence || !explanation)
                return [];
            if (!Number.isInteger(start) || !Number.isInteger(end) || Number(start) < 0 || Number(end) <= Number(start))
                return [];
            if (studentResponse.slice(Number(start), Number(end)) !== evidence)
                return [];
            return [{ strength_tag: tag, evidence, explanation, start_char: Number(start), end_char: Number(end) }];
        })
        : [];
    ...
    if (!validAlignment || !taskUnderstanding || !submissionRead || !nextMove || !monthlySummary)
        return null;
    if (!strengths || strengthEvidence.length === 0 || !weaknesses || !nextSteps || !whatIsWorking || !whatIsMissing)
        return null;
    const weaknessTags = Array.isArray(record['weakness_tags'])
        ? [...new Set(record['weakness_tags'].map(String).filter((tag) => allowedWeaknessTags.has(tag)))].slice(0, 12)
        : [];
    return {
...
export const normalizeAuthoritativeWritingAssessment = (raw, context) => {
    const payload = asRecord(raw);
    const assessmentRecord = asRecord(payload?.['assessment']);
    if (!payload || !assessmentRecord)
        return { ok: false, error: 'Assessment response is incomplete.' };
    ...
    if (assessmentRecord['rubric_version'] !== WRITING_RUBRIC_VERSION
        || !TRUSTED_WRITING_EVALUATOR_VERSIONS.includes(evaluatorVersion)) {
    ...
    if (!feedback)
        return { ok: false, error: 'Cinematic feedback evidence is incomplete.' };
...
export const isAcademicProfileWritingAssessment = (assessment) => Boolean(assessment
    ...
    && TRUSTED_WRITING_EVALUATOR_VERSIONS.includes(assessment.evaluator_version));
