...
const normalizeComparable = (value) => value.normalize('NFKC').replace(/\s+/g, ' ').trim().toLocaleLowerCase();
const spreadsheetDatePattern = /^(?:20\d{2}[⁄/]\d{1,2}[⁄/]\d{1,2}|\d{1,2}月\d{1,2}日)$/;
const parseDelimitedRows = (input) => {
    ...
    if (quoted)
        throw new Error('The file contains an unclosed quoted value.');
    return rows;
};
...
export const parseTeacherQuestionImport = (input) => {
    const rows = parseDelimitedRows(input);
    if (rows.length < 2)
        throw new Error('Add a header row and at least one question.');
    const headers = rows[0].map(normalizeHeader);
    const column = (...aliases) => aliases.map((alias) => headers.indexOf(alias)).find((index) => index >= 0) ?? -1;
    const columns = {
    ...
    rows.slice(1).forEach((cells, offset) => {
        ...
        const read = (index) => index >= 0 ? (cells[index] || '').trim() : '';
        try {
            ...
            if (!subject)
                throw new Error('Use a supported subject name');
            if (!difficulty)
                throw new Error('difficulty must be easy, medium, or hard');
            if (!questionType)
                throw new Error('question_type must be multiple_choice, true_false, or short_answer');
            if (!questionText)
            ...
            if (questionType === 'multiple_choice') {
                if (rawOptions.length < 2)
                    throw new Error('multiple_choice questions require at least two options');
    ...
    if (questions.length > 500)
        throw new Error('A bulk import can contain at most 500 valid questions. Split this file into smaller batches.');
