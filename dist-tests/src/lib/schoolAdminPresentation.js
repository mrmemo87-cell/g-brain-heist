...
const IELTS_ADMIN_ERROR_MESSAGES = [
...
export function friendlySchoolAdminError(message, fallback) {
    const raw = message instanceof Error ? message.message : String(message ?? '').trim();
    const normalized = raw.toLowerCase();
    if (!raw)
        return fallback;
    if (normalized.includes('classes_school_classcode_uniq')
...
export function friendlyIeltsAdminError(message, fallback = 'The IELTS workspace could not complete this request. Please try again.') {
    const raw = message instanceof Error ? message.message : String(message ?? '').trim();
    const normalized = raw.toLowerCase();
    let knownMessage = null;
    for (const entry of IELTS_ADMIN_ERROR_MESSAGES) {
        if (normalized.includes(entry[0])
