export var AwardId;
(function (AwardId) {
    AwardId["TOP_AGENT"] = "TOP_AGENT";
    AwardId["ACCURACY_SNIPER"] = "ACCURACY_SNIPER";
    AwardId["CHAOS_GREMLIN"] = "CHAOS_GREMLIN";
    AwardId["CLUTCH_HACKER"] = "CLUTCH_HACKER";
    AwardId["SILENT_GHOST"] = "SILENT_GHOST";
})(AwardId || (AwardId = {}));
const MIN_ACCURACY_ANSWERS = 5;
const MIN_SILENT_GHOST_ANSWERS = 3;
function collectEvents(state) {
    const inlineEvents = state.events ?? [];
    const roundEvents = (state.rounds ?? []).flatMap((round) => round.events.map((event) => ({ ...event, round: event.round ?? round.round })));
    const events = [...inlineEvents, ...roundEvents];
    return events.sort((a, b) => (a.round ?? 0) - (b.round ?? 0));
}
function totalRounds(state, events) {
    if (typeof state.totalRounds === 'number') {
        return state.totalRounds;
    }
    const roundFromEvents = events.reduce((max, event) => {
        const roundIndex = event.round ?? 0;
        return roundIndex > max ? roundIndex : max;
    }, 0);
    const roundFromSummary = (state.rounds ?? []).reduce((max, round) => round.round > max ? round.round : max, 0);
    return Math.max(roundFromEvents, roundFromSummary);
}
function fallbackNumber(value, alternative) {
    return typeof value === 'number' ? value : alternative;
}
function summarizePlayer(player, events, finalRound) {
    const playerEvents = events.filter((event) => event.playerId === player.id);
    const correctFromEvents = playerEvents.filter((event) => event.type === 'QUESTION' && event.correct).length;
    const incorrectFromEvents = playerEvents.filter((event) => event.type === 'QUESTION' && !event.correct).length;
    const correctAnswers = fallbackNumber(player.correctAnswers, correctFromEvents);
    const incorrectAnswers = fallbackNumber(player.incorrectAnswers, incorrectFromEvents);
    const answered = correctAnswers + incorrectAnswers;
    const accuracy = answered > 0 ? correctAnswers / answered : 0;
    const hacksUsed = fallbackNumber(player.hacksUsed, playerEvents.filter((event) => event.type === 'HACK').length);
    const alarmContribution = fallbackNumber(player.alarmContribution, playerEvents.reduce((sum, event) => {
        if (event.type === 'ALARM') {
    ...
    const stealEvents = [
        ...(player.steals ?? []),
        ...playerEvents.filter((event) => event.type === 'STEAL' && event.successful),
    ];
    const totalStealValue = stealEvents.reduce((sum, steal) => (steal.successful ? sum + Math.max(steal.amount, 0) : sum), 0);
    const biggestSteal = stealEvents.reduce((max, steal) => (steal.successful && steal.amount > max ? steal.amount : max), 0);
    const assists = fallbackNumber(player.assists, playerEvents.filter((event) => event.type === 'ASSIST').length);
    const roundsPlayed = fallbackNumber(player.roundsPlayed, new Set(playerEvents.map((event) => event.round ?? 0)).size);
...
export function summarizePlayers(state) {
    const events = collectEvents(state);
    const finalRound = totalRounds(state, events);
    return state.players.map((player) => summarizePlayer(player, events, finalRound));
}
export function getTopAgentsByCoins(state, limit) {
...
export function calculateAwards(state) {
    const summaries = summarizePlayers(state);
    ...
    const silentCandidates = summaries
        .filter((summary) => !topCoinIds.has(summary.playerId) &&
        summary.answered >= MIN_SILENT_GHOST_ANSWERS &&
        summary.accuracy >= 0.6)
        .sort((a, b) => {
        if (b.accuracy !== a.accuracy)
            return b.accuracy - a.accuracy;
        if (a.hacksUsed !== b.hacksUsed)
            return a.hacksUsed - b.hacksUsed;
        return b.answered - a.answered;
    });
