...
const roundToHalfBand = (value) => Math.round(value * 2) / 2;
const normalizeComparableAnswer = (value) => String(value ?? '').trim().toLowerCase();
const parsePossibleJsonArray = (value) => {
...
export const doesAnswerMatchCorrectAnswer = (studentAnswer, correctAnswer) => {
...
export const estimateIeltsBandFromPercent = (percentage) => {
...
export const normalizeIeltsRawScore = (input) => {
...
export const toRawScoreResult = (rawScore, totalQuestions) => {
...
export const normalizeIeltsBand = (band) => {
    if (!isFiniteNumber(band)) {
        return null;
    }
    return roundToHalfBand(Math.min(9, Math.max(0, band)));
};
export const buildReadingAttemptPayload = (basePayload, score) => {
...
export const buildListeningAttemptPayload = (basePayload, score) => {
    const normalizedScore = normalizeIeltsRawScore(score);
    return {
        ...basePayload,
        ...(normalizedScore ?? {}),
...
export const buildWritingAttemptPayload = (basePayload, score = {}) => {
...
export const buildSpeakingAttemptPayload = (basePayload, score = {}) => {
    const bandOverall = normalizeIeltsBand(score.bandOverall);
    return {
        ...basePayload,
        ...(bandOverall === null ? {} : { band_overall: bandOverall }),
