...
import { getAcademicReportingContext } from '../../../services/academicReportingService.js';
gsap.registerPlugin(DrawSVGPlugin);
const toAlignmentLabel = (alignment) => {
    const labels = {
        on_task: 'On task',
...
export const buildWritingDashboardSnapshot = (studentId, month, genre) => {
    const stateRes = getStudentWritingState(studentId, genre);
    if (!stateRes.ok || !stateRes.data)
        return { ok: false, error: stateRes.error ?? 'Unable to load writing state.' };
...
const getShellCardStyle = (_theme) => ({
    borderRadius: 20,
...
const getMissionCardStyle = (_theme) => ({
...
const toWordCountLabel = (targetWords) => {
    const range = computeWordCountRange(targetWords);
    return `${range.min}–${range.max} words`;
};
const getHighlightAnimationDurationMs = (segmentLength) => Math.max(140, Math.min(360, segmentLength * 14));
const getWordCounterTone = (typedWords, targetWords) => {
...
const normalizeTextForMatch = (value) => value
    .toLowerCase()
    .replace(/[“”]/g, '"')
    .replace(/[‘’]/g, "'")
    .replace(/\s+/g, ' ')
    .trim();
export const buildTextFingerprint = (value) => {
...
const extractQuotedSnippet = (value) => {
...
const findSafeSnippetRange = (text, snippet) => {
    if (!snippet)
        return null;
    ...
    if (directStart >= 0)
        return { start: directStart, end: directStart + trimmed.length };
    return null;
};
const buildRepairQueue = (ai, fallbackWeaknessTips) => {
    if (!ai) {
    ...
    const queue = [];
    const mapRepairCategory = (stepType) => {
        const normalized = (stepType ?? '').toLowerCase();
        if (normalized.includes('grammar'))
            return 'grammar';
        if (normalized.includes('punct'))
            return 'punctuation';
        if (normalized.includes('style') || normalized.includes('tone'))
            return 'style';
...
const buildAnchorRanges = (text, ai) => {
    if (!ai)
        return [];
    const ranges = [];
    const classifyPolarity = (category, fallback) => {
        const normalized = (category ?? '').toLowerCase();
        if (!normalized)
            return fallback;
        if (normalized.includes('strong') || normalized.includes('strength') || normalized.includes('what_is_working'))
            return 'strong';
        if (normalized.includes('grammar')
    ...
    (ai.highlights ?? []).forEach((item, idx) => {
        if (typeof item.start_char !== 'number' || typeof item.end_char !== 'number')
            return;
        if (!Number.isInteger(item.start_char) || !Number.isInteger(item.end_char))
            return;
        if (item.start_char < 0 || item.end_char <= item.start_char || item.end_char > text.length)
            return;
...
export const evaluateAnchorTrust = (submissionText, feedback) => {
    ...
    const persistedFingerprint = typeof feedback?.text_fingerprint === 'string' && feedback.text_fingerprint.trim()
        ...
        : null;
    if (!feedback)
        return { mode: 'no_feedback', localFingerprint, persistedFingerprint };
    if (!persistedFingerprint || !localFingerprint)
        return { mode: 'missing_fingerprint', localFingerprint, persistedFingerprint };
    if (persistedFingerprint !== localFingerprint)
        return { mode: 'stale_feedback', localFingerprint, persistedFingerprint };
    if (buildAnchorRanges(safeSubmissionText, feedback).length === 0)
        return { mode: 'no_anchors', localFingerprint, persistedFingerprint };
    return { mode: 'trusted', localFingerprint, persistedFingerprint };
};
const hasConflictingAnchorOverlap = (ranges) => {
    if (ranges.length < 2)
        return false;
    ...
    for (let i = 1; i < sorted.length; i += 1) {
        ...
        if (overlapRatio >= 0.4 && prev.polarity !== current.polarity)
            return true;
    }
    return false;
};
...
const ReviewHighlightSpan = ({ index, range, segment, isActive, spotlightMode = false, onMount, }) => {
    ...
    return (_jsxs("span", { ref: (element) => onMount?.(index, element), "data-review-highlight-index": index, className: `review-highlight ${strong ? 'review-highlight--strong' : 'review-highlight--weak'} ${isActive ? 'review-highlight--active' : 'review-highlight--inactive'}`, title: range.reason, style: highlightStyle, children: [_jsx("span", { className: "review-highlight__ink-base", "aria-hidden": "true" }), _jsx("span", { className: "review-highlight__text", children: segment })] }));
};
const renderAnnotatedText = (text, ranges, activeIndex = null, onHighlightMount, spotlightMode = false) => {
    if (!text)
        return text;
...
const getVisualLineRectsForHighlight = (scrollContainer, highlightElement) => {
    if (!scrollContainer || !highlightElement)
        return [];
...
const scrollLineIntoComfortZone = (scrollContainer, line) => {
    if (!scrollContainer || !line)
        return;
...
const describeHighlight = (range, text, ai) => {
    if (!range)
        return { label: 'AI feedback', detail: 'Select a highlight to view detailed guidance.' };
    ...
    const toTeacherDetail = (kind, issue, original, betterVersion) => {
        const simpleIssue = simplifyStudentLanguage(issue || '').trim();
        const quotedOriginal = original?.trim() ? `"${original.trim()}"` : '';
        if (kind === 'grammar') {
            if (quotedOriginal && betterVersion?.trim())
                return `You wrote ${quotedOriginal} — ${simpleIssue || 'this needs a grammar fix'}. Better: "${betterVersion.trim()}".`;
            if (quotedOriginal && simpleIssue)
                return `You wrote ${quotedOriginal} — ${simpleIssue}.`;
            if (simpleIssue)
                return simpleIssue;
        ...
        if (kind === 'punctuation') {
            if (quotedOriginal && betterVersion?.trim())
                return `You wrote ${quotedOriginal} — ${simpleIssue || 'punctuation needs fixing'}. Better: "${betterVersion.trim()}".`;
            if (quotedOriginal && simpleIssue)
                return `You wrote ${quotedOriginal} — ${simpleIssue}.`;
            if (simpleIssue)
                return simpleIssue;
        ...
        if (quotedOriginal && betterVersion?.trim())
            return `You wrote ${quotedOriginal} — ${simpleIssue || 'this can sound more natural'}. Try: "${betterVersion.trim()}".`;
        if (quotedOriginal && simpleIssue)
            return `You wrote ${quotedOriginal} — ${simpleIssue}.`;
        if (simpleIssue)
            return simpleIssue;
    ...
    const isOriginalMatch = (value) => {
        const normalized = normalize(value);
        if (!normalized || !normalizedSnippet)
            return false;
        if (normalizedSnippet === normalized)
            return true;
        if (normalizedSnippet.includes(normalized) || normalized.includes(normalizedSnippet))
            return true;
    ...
    const scoreOriginalMatch = (value) => {
        const normalized = normalize(value);
        if (!normalized || !normalizedSnippet)
            return 0;
        if (normalizedSnippet === normalized)
            return 100;
        if (normalizedSnippet.includes(normalized))
            return Math.min(95, 50 + normalized.length);
        if (normalized.includes(normalizedSnippet))
            return Math.min(90, 40 + normalizedSnippet.length);
    ...
    const pickBestByOriginal = (items) => {
        let best = null;
        let bestScore = 0;
        items.forEach((item) => {
    ...
    const strength = [...(ai.what_is_working ?? []), ...(ai.strengths ?? [])].find((item) => {
        ...
        if (normalizedQuoted && normalizedQuoted === normalizedSnippet)
            return true;
        return normalize(item).includes(normalizedSnippet) && normalizedSnippet.length > 10;
    });
    if (prefersStrength && strength)
        return { label: 'Why this is strong', detail: simplifyStudentLanguage(strength) };
    if (!prefersStrength && grammar)
        return { label: 'Grammar fix', detail: toTeacherDetail('grammar', grammar.issue, grammar.original, grammar.better_version), correction: grammar.better_version };
    if (!prefersStrength && punctuation)
        return { label: 'Punctuation fix', detail: toTeacherDetail('punctuation', punctuation.issue, punctuation.original, punctuation.better_version), correction: punctuation.better_version };
    if (!prefersStrength && phrase)
        return { label: 'Phrase upgrade', detail: toTeacherDetail('phrase', phrase.why_it_helps, phrase.original, phrase.better_version), correction: phrase.better_version };
...
const normalizeComparisonText = (value) => value.replace(/\s+/g, ' ').trim().toLowerCase();
const normalizeForComparison = (text) => text
    .trim()
    .replace(/\s+/g, ' ')
    .replace(/\s+([,.;:!?])/g, '$1')
    .replace(/([,.;:!?])(?=\S)/g, '$1 ')
    .replace(/\s+/g, ' ')
    .toLowerCase();
const stripOuterPunctuation = (text) => text.replace(/^[\s"'“”‘’.,;:!?()[\]{}-]+|[\s"'“”‘’.,;:!?()[\]{}-]+$/g, '');
const isMeaningfullyDifferent = (a, b) => {
...
const validateIssueConsistency = (issue) => {
    if (!issue.originalSentence.trim())
        return { valid: false, reason: 'missing_original_sentence' };
    const consistencyFields = [issue.diagnosis, issue.whyThisIsStronger, issue.coachingNote ?? ''].join(' ').trim();
    if (!consistencyFields)
        return { valid: false, reason: 'missing_explanation_fields' };
...
const getSafeIssueExplanation = (issue) => {
...
const renderPhraseComparison = (text, phrase, tone) => {
    ...
    if (idx < 0) {
        return (_jsxs(_Fragment, { children: [snippet, _jsx("span", { style: { ...toneStyle, borderRadius: 8, padding: '1px 6px', fontWeight: 700, marginLeft: 8 }, children: target })] }));
    }
    const before = snippet.slice(0, idx);
    const match = snippet.slice(idx, idx + target.length);
    const after = snippet.slice(idx + target.length);
    return (_jsxs(_Fragment, { children: [before, _jsx("span", { style: { ...toneStyle, borderRadius: 8, padding: '1px 6px', fontWeight: 700 }, children: match }), after] }));
};
const buildRubricReason = (key, score, weaknessTags, ai) => {
    ...
    if (key === 'content') {
        if (score >= 4)
            return strengths.find((s) => /point|detail|idea|argument|reason/i.test(s)) ? simplifyStudentLanguage(strengths.find((s) => /point|detail|idea|argument|reason/i.test(s))) : 'Strong coverage of the task with clear ideas and supporting detail.';
        if (score >= 3)
            return 'Most task points addressed, but some ideas need more supporting detail.';
        if (tags.includes('missing_content_point') || tags.includes('partial_content_coverage'))
            return 'Key parts of the question were not answered or not developed with reasons.';
        if (score >= 2)
            return 'Some relevant ideas, but important task points are missing or undeveloped.';
        return 'The response does not cover enough of what the task asks for.';
    }
    if (key === 'organisation') {
        if (score >= 4)
            return 'Ideas flow logically with clear connections between sentences.';
        if (score >= 3)
            return 'Generally organized, but some transitions between ideas are missing.';
        if (tags.includes('weak_organisation') || tags.includes('no_paragraph_structure'))
            return 'Ideas are not grouped or connected in a clear order.';
        if (score >= 2)
            return 'Some structure, but the argument or narrative jumps between points.';
    ...
    if (key === 'language') {
        ...
        if (score >= 4)
            return 'Good range of vocabulary and mostly accurate grammar.';
        if (score >= 3)
            return grammarCount > 0 ? `Meaning is clear, but ${grammarCount} language issue${grammarCount > 1 ? 's' : ''} reduce${grammarCount === 1 ? 's' : ''} accuracy.` : 'Meaning is mostly clear, with some grammar or vocabulary issues.';
        if (score >= 2)
            return grammarCount > 2 ? `Several grammar mistakes (${grammarCount} found) make some sentences hard to follow.` : 'Grammar errors reduce clarity in places.';
        return 'Frequent language errors make the meaning difficult to understand.';
    }
    if (key === 'communicative_achievement') {
        if (score >= 4)
            return 'Tone, register, and format match the task well.';
        if (score >= 3)
            return 'Mostly appropriate tone, but some sections feel too informal or too formal.';
        if (tags.includes('wrong_register') || tags.includes('tone_mismatch'))
            return 'The tone or style does not match what the task requires.';
        if (score >= 2)
            return 'The writing communicates meaning, but the tone needs adjustment for this genre.';
        return 'The style and tone do not match the expected format.';
    }
    return null;
};
const SUPPORTED_GENRES = ['essay', 'story', 'article', 'review', 'report', 'email', 'paragraph'];
const defaultPromptByGenre = FALLBACK_PROMPT_BY_GENRE;
const toGenreLabel = (genre) => genre.charAt(0).toUpperCase() + genre.slice(1);
const toAttemptTypeLabel = (attemptType) => attemptType === 'initial_assessment' ? 'Initial assessment' : 'Daily practice';
...
const compactSnippet = (text, max = 180) => {
    const normalized = text.replace(/\s+/g, ' ').trim();
    if (!normalized)
        return 'No writing text saved for this attempt.';
    if (normalized.length <= max)
        return normalized;
    return `${normalized.slice(0, max).trimEnd()}…`;
};
const toGenreStateCopy = (status, day, completed, total) => {
    if (status === 'not_started')
        return 'Not started yet';
    if (status === 'week_complete')
        return 'Week complete';
    if (day)
        return `Day ${day} ready`;
    if (total > 0)
        return `Week active • ${completed}/${total} tasks`;
    return 'Week active';
};
const buildReadableTaskSummary = (promptText) => {
    const normalized = simplifyStudentLanguage(promptText).replace(/\s+/g, ' ').trim();
    if (!normalized)
        return 'Your writing mission will appear here once a task is loaded.';
    return normalized.length > 220 ? `${normalized.slice(0, 217).trimEnd()}…` : normalized;
};
const writingMissionCategoryMeta = {
...
const normalizeMissionText = (value) => (value ?? '').trim().toLowerCase().replace(/\s+/g, ' ');
const scoreQuestMissionForCategory = (mission, categoryMeta) => {
...
const buildWritingMissionRecommendations = (input) => {
    ...
    for (const weaknessTag of input.rankedWeaknessTags) {
        const category = WEAKNESS_TAG_TO_MISSION_CATEGORY[weaknessTag];
...
const normalizePromptForComparison = (prompt) => prompt.replace(/\s+/g, ' ').trim().toLowerCase();
const getProgressTone = (score) => {
    if (score == null)
        return { color: '#64748b', glow: 'rgba(100, 116, 139, 0.35)', label: 'No score yet' };
    if (score >= 4)
        return { color: '#22c55e', glow: 'rgba(34, 197, 94, 0.45)', label: 'Strong' };
    if (score >= 3)
        return { color: '#38bdf8', glow: 'rgba(56, 189, 248, 0.45)', label: 'Building' };
    if (score >= 2)
        return { color: '#f59e0b', glow: 'rgba(245, 158, 11, 0.45)', label: 'In progress' };
    return { color: '#f97316', glow: 'rgba(249, 115, 22, 0.45)', label: 'Needs attention' };
};
const parseSubscaleProgress = (entries) => {
    const values = {
        content: null,
        organisation: null,
        language: null,
    ...
    entries.forEach((entry) => {
        ...
        if (contentMatch)
            values['content'] = Number(contentMatch[1]);
        const organisationMatch = entry.match(/organisation[:\s]*([+\-]?\d+(\.\d+)?)/i);
        if (organisationMatch)
            values['organisation'] = Number(organisationMatch[1]);
        const languageMatch = entry.match(/language[:\s]*([+\-]?\d+(\.\d+)?)/i);
        if (languageMatch)
            values['language'] = Number(languageMatch[1]);
        const achievementMatch = entry.match(/communicative achievement[:\s]*([+\-]?\d+(\.\d+)?)/i);
        if (achievementMatch)
            values['communicative_achievement'] = Number(achievementMatch[1]);
    });
    return values;
};
const taskTypeToFriendlyTitle = (taskType, day) => {
    const map = {
...
const taskTypeToFriendlyInstruction = (taskType) => {
    const map = {
...
const getTaskTypeStudyGuide = (taskType) => {
    return (taskTypeStudyGuideMap[taskType] ?? {
...
const weaknessTagToStudentTip = (tag) => {
    const tipMap = {
...
const estimateWeeklyTargetScoreRange = (baselineScore, supportLevel) => {
    if (baselineScore == null)
        return null;
    const growthBySupport = {
...
const keepHorizontalItemInView = (container, item, behavior = 'smooth') => {
    if (!container || !item)
        return;
...
const getMissionRecommendationKey = (item) => `${item.missionCategory}-${item.title}`;
const WritingHubLegacy = ({ studentId, studentName, grade, genre, month = new Date().toISOString().slice(0, 7), onOpenQuestMission }) => {
    ...
    const [aiFeedbackDetails, setAiFeedbackDetails] = useState(null);
    const [uiNotice, setUiNotice] = useState('');
    const [aiWeeklyFocus, setAiWeeklyFocus] = useState('');
    const [aiCoachingPoints, setAiCoachingPoints] = useState([]);
    const [aiTaskWording, setAiTaskWording] = useState('');
    const [aiMonthlyWording, setAiMonthlyWording] = useState('');
    ...
    const [aiReviewViewMode, setAiReviewViewMode] = useState('full');
    ...
    const [reviewActiveIndex, setReviewActiveIndex] = useState(null);
    const [activeLineMeasure, setActiveLineMeasure] = useState({
        index: null,
        rects: [],
    });
    const [activeRepairId, setActiveRepairId] = useState(null);
    const [repairStatusMessage, setRepairStatusMessage] = useState('');
    const [viewedRepairIds, setViewedRepairIds] = useState([]);
    const closeModalButtonRef = useRef(null);
    const closeProgressDetailsButtonRef = useRef(null);
    const firstRepairButtonRef = useRef(null);
    const practiceTextareaRef = useRef(null);
    const feedbackCardRef = useRef(null);
    const todayMissionCardRef = useRef(null);
    const progressCardRef = useRef(null);
    const preWeekComposeCardRef = useRef(null);
    const preWeekResponseRef = useRef(null);
    const writingPathCarouselRef = useRef(null);
    const reviewEssayPanelRef = useRef(null);
    const writingHubRootRef = useRef(null);
    const highlightSpanRefs = useRef({});
    const activeLineOverlayRefs = useRef([]);
    const reviewAutoplaySessionRef = useRef(null);
    const reviewAnimationStepRef = useRef(null);
    const reviewAnimationForIndexRef = useRef(null);
    const aiPlanAssistRequestIdRef = useRef(0);
    const writingPathButtonRefs = useRef({});
    const missionsCarouselRef = useRef(null);
    ...
    const supportPanelStyle = useMemo(() => (themeMode === 'light'
        ? {
            ...shellCardStyle,
    ...
    const isEmptyWritingState = Boolean(stateRes.ok &&
        stateRes.data &&
    ...
    const submittedHighlightRanges = useMemo(() => buildAnchorRanges(submittedPracticeText, aiFeedbackDetails), [submittedPracticeText, aiFeedbackDetails]);
    const fallbackHighlightRanges = useMemo(() => buildFallbackHighlightRanges(submittedPracticeText, aiFeedbackDetails), [submittedPracticeText, aiFeedbackDetails]);
    const reviewAnchorTrust = useMemo(() => evaluateAnchorTrust(submittedPracticeText, aiFeedbackDetails), [submittedPracticeText, aiFeedbackDetails]);
    const shouldUseFallbackRanges = useMemo(() => reviewAnchorTrust.mode !== 'trusted' || hasConflictingAnchorOverlap(submittedHighlightRanges), [reviewAnchorTrust.mode, submittedHighlightRanges]);
    const reviewScanPlan = useMemo(() => buildBalancedReviewSequence(shouldUseFallbackRanges ? fallbackHighlightRanges : submittedHighlightRanges, 8), [shouldUseFallbackRanges, submittedHighlightRanges, fallbackHighlightRanges]);
    const reviewAnimationTimelineMs = useMemo(() => reviewScanPlan.reduce((total, range) => {
    ...
    }, 0), [reviewScanPlan, submittedPracticeText]);
    const visibleSubmittedHighlightRanges = useMemo(() => reviewScanPlan, [reviewScanPlan]);
    const reviewSessionKey = useMemo(() => `${submittedPracticeText.length}:${visibleSubmittedHighlightRanges
        .map((range) => `${range.start}-${range.end}-${range.reason ?? ''}-${range.sourceCategory ?? ''}`)
        .join('|')}`, [submittedPracticeText, visibleSubmittedHighlightRanges]);
    const activeReviewRange = useMemo(() => (reviewActiveIndex != null ? visibleSubmittedHighlightRanges[reviewActiveIndex] ?? null : null), [visibleSubmittedHighlightRanges, reviewActiveIndex]);
    const activeReviewNote = useMemo(() => describeHighlight(activeReviewRange, submittedPracticeText, aiFeedbackDetails), [activeReviewRange, submittedPracticeText, aiFeedbackDetails]);
    const fullFeedbackStrengths = useMemo(() => (aiFeedbackDetails?.what_is_working ?? aiFeedbackDetails?.strengths ?? []).filter((item) => Boolean(item?.trim())), [aiFeedbackDetails]);
    const fullFeedbackImprovements = useMemo(() => (aiFeedbackDetails?.what_is_missing ?? aiFeedbackDetails?.weaknesses ?? []).filter((item) => Boolean(item?.trim())), [aiFeedbackDetails]);
    const fullFeedbackGrammarFixes = useMemo(() => [
        ...(aiFeedbackDetails?.grammar_fixes ?? []).map((item) => ({ ...item, category: 'Grammar' })),
        ...(aiFeedbackDetails?.punctuation_fixes ?? []).map((item) => ({ ...item, category: 'Punctuation' })),
    ].filter((item) => item.original?.trim() && item.better_version?.trim()), [aiFeedbackDetails]);
    const fullFeedbackPhraseUpgrades = useMemo(() => (aiFeedbackDetails?.natural_phrase_upgrades ?? []).filter((item) => item.original?.trim() && item.better_version?.trim()), [aiFeedbackDetails]);
    const fullFeedbackStyleTone = useMemo(() => (aiFeedbackDetails?.style_tone_feedback ?? []).filter((item) => item.evidence?.trim() || item.suggestion?.trim()), [aiFeedbackDetails]);
    useEffect(() => {
        if (typeof window === 'undefined')
            return;
        window.localStorage.setItem('writing-hub-theme', themeMode);
    }, [themeMode]);
    const handleReviewHighlightMount = (index, element) => {
    ...
    const rankedWeaknessTags = useMemo(() => {
        ...
        Object.entries(repeatedTagCounts).forEach(([tag, count]) => {
            if (typeof count === 'number' && count > 0)
                scoreByTag.set(tag, (scoreByTag.get(tag) ?? 0) + count * 4);
        });
    ...
    const firstAttemptRichFeedback = (hubSnapshot.ok ? hubSnapshot.data?.first_attempt_rich_feedback ?? null : null);
    ...
    const anchorTrustEvaluation = useMemo(() => evaluateAnchorTrust(firstAttemptSubmission, firstAttemptRichFeedback), [firstAttemptSubmission, firstAttemptRichFeedback]);
    ...
    const firstAttemptAnchorRanges = useMemo(() => buildAnchorRanges(firstAttemptSubmission ?? '', firstAttemptFeedbackForAnchors), [firstAttemptSubmission, firstAttemptFeedbackForAnchors]);
    const repairQueue = useMemo(() => buildRepairQueue(firstAttemptFeedbackForRepairQueue, studentFriendlyWeaknesses), [firstAttemptFeedbackForRepairQueue, studentFriendlyWeaknesses.join('|')]);
    const activeRepairItem = useMemo(() => repairQueue.find((item) => item.id === activeRepairId) ?? repairQueue[0] ?? null, [repairQueue, activeRepairId]);
    const studentStrengths = useMemo(() => (firstAttemptRichFeedback?.what_is_working?.length
        ? firstAttemptRichFeedback.what_is_working
        : firstAttemptRichFeedback?.strengths ?? [])
        .map((item) => simplifyStudentLanguage(item))
        .filter(Boolean)
        .slice(0, 2), [firstAttemptRichFeedback]);
    const sessionSeenRepairCount = useMemo(() => {
        if (repairQueue.length === 0)
            return 0;
        const seen = new Set(viewedRepairIds);
        if (activeRepairItem?.id)
            seen.add(activeRepairItem.id);
    ...
    const activeRepairRange = useMemo(() => {
        if (!trustedAnchorMode)
            return null;
        if (!firstAttemptSubmission || !activeRepairItem?.evidenceSnippet)
            return null;
    ...
    const missionRecommendations = useMemo(() => buildWritingMissionRecommendations({ rankedWeaknessTags, questMissions }), [rankedWeaknessTags.join('|'), questMissions]);
    ...
    const communicativeAchievementScore = (() => {
        ...
        if (totalScore == null || content == null || organisation == null || language == null)
            return null;
    ...
    const completionRatio = stateRes.ok && stateRes.data && stateRes.data.active_daily_tasks.length > 0
    ...
    const showMonthlyEvidence = Boolean(monthlyReport.ok &&
        monthlyReport.data &&
    ...
    useEffect(() => {
        ...
        setSelectedMissionKey((current) => {
            if (current && missionRecommendations.some((item) => getMissionRecommendationKey(item) === current))
                return current;
    ...
    useEffect(() => {
        if (!showTaskContextModal)
            return;
        const onEscape = (event) => {
            if (event.key === 'Escape')
                setShowTaskContextModal(false);
        };
    ...
    useEffect(() => {
        if (!showAiReviewModal || reviewActiveIndex == null || activeLineMeasure.rects.length === 0)
            return;
        if (activeLineMeasure.index !== reviewActiveIndex)
            return;
        if (reviewAnimationForIndexRef.current === reviewActiveIndex && reviewAnimationStepRef.current === reviewActiveIndex)
            return;
    ...
    useEffect(() => {
        if (!showTaskContextModal || !activeRepairItem?.id)
            return;
    ...
    useEffect(() => {
        ...
        const loadSmartPrompt = async () => {
            if (initializing)
                return;
            if (!isPreWeek && !isWeekComplete)
                return;
            if (initialResponse.trim().length > 0)
                return;
            const shouldRefreshPrompt = !promptText.trim() || normalizePromptForComparison(promptText) === normalizePromptForComparison(defaultPromptByGenre[activeGenre]);
    ...
    useEffect(() => {
        if (aiFeedbackDetails)
            return;
        if (!firstAttemptRichFeedback)
            return;
        if (!isActiveWeek)
            return;
        ...
        if (fallbackSummary)
            setFeedback((current) => current || fallbackSummary);
    ...
    useEffect(() => {
        ...
        const loadAiPlanAssist = async () => {
            if (!nonCriticalReady)
                return;
            if (!stateRes.ok || !stateRes.data?.latest_assessment || aiBusy)
                return;
            ...
            try {
                ...
                if (cancelled || activeRequestId == null || activeRequestId !== requestId)
                    return;
                if (planAssist.ok && planAssist.data) {
                    const ai = (planAssist.data.result ?? {});
                    if (ai.focus?.trim())
                        setAiWeeklyFocus(ai.focus.trim());
                    if (Array.isArray(ai.coaching_points) && ai.coaching_points.length > 0) {
                        setAiCoachingPoints(ai.coaching_points.slice(0, 3));
                    }
                    if (ai.daily_task?.trim())
                        setAiTaskWording(ai.daily_task.trim());
                }
    ...
    useEffect(() => {
        ...
        void quest_get_missions()
            .then((rows) => {
            if (!cancelled)
                setQuestMissions(Array.isArray(rows) ? rows : []);
        })
            .catch(() => {
            if (!cancelled)
                setQuestMissions([]);
    ...
    const loadRichFeedback = async (submissionText, promptForFeedback, weaknessesForFeedback, source, attemptId) => {
        ...
        if (aiFeedback.ok && aiFeedback.data) {
            const ai = (aiFeedback.data.result ?? {});
            ...
            if (refined)
                setFeedback(refined);
            if (ai.monthly_report_summary?.trim())
                setAiMonthlyWording(ai.monthly_report_summary.trim());
            if (source === 'initial') {
        ...
        return { ok: false, error: aiFeedback.error ?? 'AI analysis unavailable.' };
    };
    const handleStart = async (options) => {
        ...
        try {
            const aiResult = await loadRichFeedback(safeInitialResponse, promptForSubmission, latestWeaknesses, 'initial', result.data?.attempt_id);
            if (aiResult.ok) {
    ...
    const handleChangeWritingType = (nextGenre) => {
        if (nextGenre === activeGenre)
            return;
    ...
    const handleReviewStepNavigation = (direction) => {
    ...
    const renderLoadingSkeleton = () => (_jsxs(_Fragment, { children: [_jsxs("section", { className: "writing-hub-card", style: missionCardStyle, children: [_jsx("p", { style: { ...pillStyle, margin: 0 }, children: "Loading Writing Hub\u2026" }), _jsx("div", { style: { marginTop: 12, height: 18, borderRadius: 10, background: 'rgba(148,163,184,0.35)' } }), _jsx("div", { style: { marginTop: 8, height: 12, borderRadius: 10, background: 'rgba(148,163,184,0.25)' } })] }), _jsxs("section", { className: "writing-hub-card", style: shellCardStyle, children: [_jsx("div", { style: { height: 12, borderRadius: 10, background: 'rgba(148,163,184,0.3)' } }), _jsx("div", { style: { marginTop: 8, height: 100, borderRadius: 12, background: 'rgba(148,163,184,0.2)' } })] })] }));
    return (_jsxs("div", { ref: writingHubRootRef, style: pageStyle, className: `writing-hub-root writing-hub-theme-${themeMode}`, onPasteCapture: handlePasteCapture, onCopyCapture: handleCopyCapture, children: [_jsx("style", { children: `
        .writing-hub-root {
          --hub-bg: #020617;
          --hub-text: #dbe7ff;
          --hub-text-strong: #f8fbff;
      ...
      ` }), initializing ? renderLoadingSkeleton() : (_jsxs(_Fragment, { children: [_jsxs("section", { className: "writing-hub-card", style: missionCardStyle, children: [_jsxs("div", { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }, children: [_jsx("p", { style: { margin: 0, color: headingSubtle, fontWeight: 700, fontSize: 13 }, children: isWeekComplete ? 'Week complete' : isActiveWeek ? 'Week active' : 'First step' }), _jsx("button", { type: "button", onClick: () => setThemeMode((current) => (current === 'dark' ? 'light' : 'dark')), "aria-label": `Switch to ${themeMode === 'dark' ? 'light' : 'dark'} theme`, style: {
                                            borderRadius: 999,
                                            border: '1px solid var(--hub-border-strong)',
                                            background: 'var(--hub-hud-bg)',
                                            color: 'var(--hub-text-strong)',
                                            ...
                                            padding: '6px 10px',
                                            cursor: 'pointer',
                                        }, children: themeMode === 'dark' ? '☀️ Light' : '🌙 Dark' })] }), _jsx("h1", { style: { margin: '6px 0 10px', color: headingColor, fontSize: 30, lineHeight: 1.1 }, children: isPreWeek ? 'Welcome to your Writing Hub' : isWeekComplete ? 'Great work — week finished' : 'Your Writing Hub' }), _jsxs("div", { style: { margin: '0 0 10px', display: 'grid', gap: 8 }, children: [_jsx("label", { style: { color: 'var(--hub-text-accent)', fontSize: 13, fontWeight: 700 }, children: "Choose your writing path" }), _jsx("div", { ref: writingPathCarouselRef, style: {
                                            display: 'flex',
                                            gap: 8,
                                            overflowX: 'auto',
                                            paddingBottom: 4,
                                            ...
                                            scrollbarWidth: 'thin',
                                        }, children: SUPPORTED_GENRES.map((item) => {
                                            ...
                                            return (_jsxs("button", { ref: (node) => {
                                                    ...
                                                    if (!pathRefs)
                                                        return;
                                                    pathRefs[item] = node;
                                                }, type: "button", onClick: () => handleChangeWritingType(item), "aria-pressed": isSelected, "aria-label": `${toGenreLabel(item)} writing path`, style: {
                                                    minWidth: 180,
                                                    flex: '0 0 auto',
                                                    ...
                                                    color: 'var(--hub-text)',
                                                    cursor: 'pointer',
                                                }, children: [_jsxs("div", { style: { fontWeight: 800, color: 'var(--hub-text-strong)', fontSize: 14 }, children: [toGenreLabel(item), " path"] }), _jsx("div", { style: { fontSize: 12, color: 'var(--hub-text-accent)', marginTop: 4 }, children: status ? toGenreStateCopy(status.status, status.current_day, status.completed_tasks_count, status.total_tasks_count) : 'Not started yet' }), status && status.latest_score != null && (_jsxs("div", { style: { fontSize: 11, color: 'var(--hub-text-accent-2)', marginTop: 2 }, children: ["Latest score: ", status.latest_score, "/20"] })), _jsx("div", { style: { marginTop: 6, ...progressTrackStyle, height: 6 }, role: "progressbar", "aria-label": `${toGenreLabel(item)} path progress ${progress}%`, "aria-valuemin": 0, "aria-valuemax": 100, "aria-valuenow": progress, children: _jsx("div", { className: "progress-fill", style: { width: `${progress}%`, height: '100%', background: 'linear-gradient(90deg, #38bdf8 0%, #22d3ee 100%)' } }) })] }, item));
                                        }) }), isWeekComplete && (_jsx("button", { type: "button", onClick: () => void handleStart({ fromWeekComplete: true }), disabled: loading || !promptText.trim() || targetWordCount < 20, style: {
                                            padding: '10px 12px',
                                            borderRadius: 10,
                                        ...
                                        }, children: loading ? 'Starting new week…' : `Start new ${toGenreLabel(activeGenre)} week` }))] }), _jsx("p", { style: { margin: 0, color: 'var(--hub-text)', fontSize: 15 }, children: isPreWeek
                                    ...
                                    : isWeekComplete
                                        ...
                                        : 'Your weekly plan is active. Follow your focus goals and complete today’s task.' }), isPreWeek && (_jsxs("div", { className: "phone-quick-nav", "aria-label": "Quick actions for phone users", children: [_jsx("button", { type: "button", onClick: () => scrollToSection(preWeekResponseRef), children: "Go to task" }), _jsx("button", { type: "button", onClick: () => scrollToSection(preWeekResponseRef), children: "Start writing" }), _jsx("button", { type: "button", onClick: () => preWeekResponseRef.current?.focus(), children: "Focus input" })] })), isActiveWeek && (_jsxs("div", { className: "phone-quick-nav", "aria-label": "Quick actions for phone users", children: [_jsx("button", { type: "button", onClick: () => scrollToSection(todayMissionCardRef), children: "Today task" }), _jsx("button", { type: "button", onClick: () => scrollToSection(feedbackCardRef), children: "Feedback" }), _jsx("button", { type: "button", onClick: () => scrollToSection(progressCardRef), children: "Progress" })] })), showNoWritingState && _jsx("p", { style: { margin: '8px 0 0', color: 'var(--hub-text-accent)', fontSize: 13 }, children: "No writing state yet" }), !isWeekComplete && (_jsxs("div", { style: { marginTop: 12 }, className: "week-stage-wrap", children: [_jsx("div", { className: "week-stage-track", children: _jsx("div", { className: "progress-fill", style: {
                                                width: `${Math.min(100, completionRatio * 100)}%`,
                                                height: '100%',
                                                background: 'linear-gradient(90deg, #38bdf8 0%, #22d3ee 55%, #34d399 100%)',
                                            } }) }), _jsx("div", { className: "week-stage-nodes", children: weeklyPlanStages.map((stage, index) => {
                                            const isCurrent = index === currentStageIndex;
                                            const isDone = index < currentStageIndex;
                                            return (_jsxs("div", { className: "week-stage-node", style: {
                                                ...
                                                }, children: [_jsx("div", { style: { fontSize: 11, fontWeight: 800, color: isDone ? '#86efac' : isCurrent ? '#7dd3fc' : 'var(--hub-subtext)' }, children: isDone ? '✓ Done' : isCurrent ? 'Now' : 'Next' }), _jsx("div", { style: { marginTop: 2, fontSize: 12, fontWeight: 700, color: 'var(--hub-text-soft)' }, children: stage.label })] }, stage.key));
                                        }) })] }))] }), isPreWeek && (_jsxs(_Fragment, { children: [_jsxs("section", { ref: preWeekComposeCardRef, className: "writing-hub-card", style: shellCardStyle, children: [_jsx("h3", { style: { marginTop: 0, marginBottom: 8, fontSize: 20, color: 'var(--hub-text-strong)' }, children: "Your writing task" }), _jsx("p", { style: { margin: '0 0 8px', color: 'var(--hub-text-accent-2)', fontSize: 13, fontWeight: 700 }, children: "Task summary" }), _jsx("p", { style: { margin: '0 0 10px', color: 'var(--hub-text)', fontSize: 15 }, children: buildReadableTaskSummary(promptText) }), _jsx("div", { style: { ...fieldStyle, minHeight: 88, whiteSpace: 'pre-wrap' }, children: promptText })] }), _jsxs("section", { className: "writing-hub-card", style: shellCardStyle, children: [_jsx("h3", { style: { marginTop: 0, marginBottom: 6, fontSize: 20, color: 'var(--hub-text-strong)' }, children: "Write your first response" }), _jsxs("p", { style: { margin: '0 0 8px', color: 'var(--hub-text-accent)', fontSize: 14 }, children: ["Word count target: ", toWordCountLabel(targetWordCount)] }), _jsxs("div", { style: { margin: '0 0 8px', borderRadius: 12, border: `1px solid ${getWordCounterTone(initialResponseWordCount, targetWordCount).glow}`, background: 'var(--hub-muted-surface)', padding: 8 }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center' }, children: [_jsxs("p", { style: { margin: 0, color: 'var(--hub-text-soft)', fontSize: 12, fontWeight: 800 }, children: ["Typed so far: ", initialResponseWordCount, " words"] }), _jsx("span", { style: { borderRadius: 999, padding: '3px 9px', fontSize: 11, fontWeight: 800, color: getWordCounterTone(initialResponseWordCount, targetWordCount).accent, border: `1px solid ${getWordCounterTone(initialResponseWordCount, targetWordCount).glow}` }, children: getWordCounterTone(initialResponseWordCount, targetWordCount).label })] }), _jsx("div", { style: { marginTop: 6, height: 7, borderRadius: 999, background: 'rgba(51,65,85,0.8)', overflow: 'hidden' }, children: _jsx("div", { className: "progress-fill", style: { width: `${getWordCounterTone(initialResponseWordCount, targetWordCount).progress}%`, height: '100%', background: getWordCounterTone(initialResponseWordCount, targetWordCount).track } }) })] }), _jsx("p", { style: { margin: '0 0 8px', color: 'var(--hub-text-accent-2)', fontSize: 12 }, children: "Try to stay close to this range." }), _jsx("textarea", { ref: preWeekResponseRef, value: initialResponse, onChange: (e) => setInitialResponse(e.target.value), placeholder: "Write your first response here. This helps us understand your starting point and build your weekly coaching plan.", className: "touch-friendly-field", style: { ...fieldStyle, minHeight: 130 } }), _jsx("button", { onClick: () => void handleStart(), disabled: loading || !promptText.trim() || !initialResponse.trim() || targetWordCount < 20, className: "writing-primary-button", style: { ...primaryButtonStyle, opacity: loading ? 0.7 : 1, cursor: loading ? 'not-allowed' : 'pointer' }, "aria-label": "Start Writing Week", children: loading ? 'Building your weekly plan…' : 'Submit and build my plan' })] })] })), isActiveWeek && (_jsxs(_Fragment, { children: [_jsxs("div", { className: "dashboard-grid", children: [_jsxs("section", { ref: todayMissionCardRef, className: "writing-hub-card", style: { ...missionCardStyle, borderColor: 'rgba(125, 211, 252, 0.8)' }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginBottom: 10 }, children: [_jsx("p", { style: { ...dashboardSectionTitleStyle, color: 'var(--hub-text-soft)' }, children: "Today" }), _jsx("span", { style: { ...sectionLabelPillStyle, color: 'var(--hub-text-accent)', borderColor: 'rgba(147, 197, 253, 0.55)' }, children: "Action now" })] }), _jsx("h3", { style: { margin: '0 0 10px', fontSize: 24, color: 'var(--hub-text-strong)' }, children: "Today\u2019s writing mission" }), !todayTask.ok || !todayTask.data ? (_jsxs("div", { children: [_jsx("p", { style: { margin: 0, color: 'var(--hub-text-muted)' }, children: "Preparing today\u2019s task\u2026" }), _jsx("p", { style: { margin: '6px 0 0', color: 'var(--hub-text-accent-2)', fontSize: 12 }, children: "If this takes longer, refresh and try again." })] })) : (_jsxs(_Fragment, { children: [_jsx("h4", { style: { margin: '0 0 8px', color: 'var(--hub-text-strong)', fontSize: 18 }, children: taskTypeToFriendlyTitle(todayTask.data.task_type, todayTask.data.day_number) }), _jsx("p", { style: { margin: '0 0 6px', color: 'var(--hub-text-accent-2)', fontSize: 13, fontWeight: 700 }, children: "Today\u2019s goal" }), _jsx("p", { style: { margin: '0 0 8px', color: 'var(--hub-text)', fontSize: 15 }, children: simplifyStudentLanguage(aiTaskWording || taskTypeToFriendlyInstruction(todayTask.data.task_type)) }), _jsx("p", { style: { margin: '0 0 6px', color: 'var(--hub-text-accent-2)', fontSize: 13, fontWeight: 700 }, children: "Focus on this" }), _jsx("p", { style: { margin: '0 0 8px', color: 'var(--hub-text-muted)', fontSize: 14 }, children: "This helps you improve your weekly focus areas and raise your writing score step by step." }), _jsxs("p", { style: { margin: '0 0 8px', color: 'var(--hub-text-accent-2)', fontSize: 13 }, children: ["Word count target: ", toWordCountLabel(todayTask.data.expected_word_count)] }), _jsxs("div", { style: { margin: '0 0 8px', borderRadius: 12, border: `1px solid ${getWordCounterTone(practiceResponseWordCount, todayTask.data.expected_word_count).glow}`, background: 'var(--hub-muted-surface)', padding: 8 }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center' }, children: [_jsxs("p", { style: { margin: 0, color: 'var(--hub-text-soft)', fontSize: 12, fontWeight: 800 }, children: ["Typed so far: ", practiceResponseWordCount, " words"] }), _jsx("span", { style: { borderRadius: 999, padding: '3px 9px', fontSize: 11, fontWeight: 800, color: getWordCounterTone(practiceResponseWordCount, todayTask.data.expected_word_count).accent, border: `1px solid ${getWordCounterTone(practiceResponseWordCount, todayTask.data.expected_word_count).glow}` }, children: getWordCounterTone(practiceResponseWordCount, todayTask.data.expected_word_count).label })] }), _jsx("div", { style: { marginTop: 6, height: 7, borderRadius: 999, background: 'var(--hub-progress-track)', overflow: 'hidden' }, children: _jsx("div", { className: "progress-fill", style: { width: `${getWordCounterTone(practiceResponseWordCount, todayTask.data.expected_word_count).progress}%`, height: '100%', background: getWordCounterTone(practiceResponseWordCount, todayTask.data.expected_word_count).track } }) })] }), _jsx("p", { style: { margin: '0 0 8px', color: 'var(--hub-text-accent-2)', fontSize: 12 }, children: "Try to stay close to this range." }), _jsx("p", { style: { margin: '0 0 6px', color: 'var(--hub-text-accent-2)', fontSize: 13, fontWeight: 700 }, children: "Try to do these 2 things" }), _jsx("ul", { style: { margin: '0 0 10px', paddingLeft: 18, color: 'var(--hub-text-muted)', fontSize: 14 }, children: todayTask.data.success_criteria.slice(0, 2).map((item) => (_jsx("li", { style: { marginBottom: 4 }, children: simplifyStudentLanguage(item) }, item))) }), _jsx("textarea", { ref: practiceTextareaRef, value: practiceResponse, onChange: (e) => setPracticeResponse(e.target.value), placeholder: "Write today\u2019s response here. Focus on today\u2019s goal and your weekly coaching points.", className: "touch-friendly-field", style: { ...fieldStyle, minHeight: 120 } }), _jsx("button", { onClick: () => void handleSubmitPractice(), disabled: loading || !practiceResponse.trim(), className: "writing-primary-button", style: { ...primaryButtonStyle, opacity: loading ? 0.7 : 1, cursor: loading ? 'not-allowed' : 'pointer' }, "aria-label": "Submit Today\u2019s Task", children: loading ? 'Submitting today’s writing…' : 'Submit Today’s Task' })] }))] }), hasTaskToday && (_jsxs("div", { className: "phone-submit-bar", role: "region", "aria-label": "Quick submit bar for phone users", children: [_jsxs("p", { style: { margin: '0 0 8px', color: 'var(--hub-text-accent)', fontSize: 12, fontWeight: 700 }, children: [practiceResponseWordCount, " words typed \u00B7 Keep going and submit when ready."] }), _jsx("button", { type: "button", onClick: () => void handleSubmitPractice(), disabled: loading || !practiceResponse.trim(), className: "writing-primary-button quick-submit-button", style: { ...primaryButtonStyle, opacity: loading ? 0.7 : 1, cursor: loading ? 'not-allowed' : 'pointer' }, children: loading ? 'Submitting…' : 'Quick submit' })] })), _jsxs("section", { ref: feedbackCardRef, className: "writing-hub-card", style: { ...shellCardStyle, borderColor: 'var(--hub-border-strong)', background: 'var(--hub-surface-card)' }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginBottom: 10 }, children: [_jsx("p", { style: { ...dashboardSectionTitleStyle, color: 'var(--hub-text-accent-2)' }, children: "Your Feedback" }), _jsx("span", { style: { ...sectionLabelPillStyle, color: 'var(--hub-text-accent)', borderColor: 'var(--hub-border-strong)' }, children: "Coaching" })] }), _jsx("h3", { style: { marginTop: 0, marginBottom: 6, fontSize: 22, color: 'var(--hub-text-strong)' }, children: "Quick coaching snapshot" }), isAnalyzingRichFeedback ? (_jsxs("div", { className: "analysis-shimmer", role: "status", "aria-live": "polite", style: {
                                                    border: '1px solid var(--hub-border-strong)',
                                                    borderRadius: 12,
                                                    padding: 12,
                                                    background: 'var(--hub-overlay-soft)',
                                                    boxShadow: 'var(--hub-shadow-card)',
                                                }, children: [_jsx("p", { style: { margin: '0 0 6px', color: 'var(--hub-text-accent-2)', fontWeight: 800, fontSize: 16 }, children: "Analyzing your writing\u2026" }), _jsx("p", { style: { margin: '0 0 10px', color: 'var(--hub-text-accent)', fontSize: 14 }, children: ['Reading your answer…', 'Checking task match…', 'Finding grammar fixes…', 'Preparing your next step…'][analysisStageIndex] }), _jsxs("div", { style: { display: 'flex', gap: 6, alignItems: 'center' }, "aria-hidden": "true", children: [_jsx("span", { className: "analysis-dot" }), _jsx("span", { className: "analysis-dot" }), _jsx("span", { className: "analysis-dot" })] })] })) : aiFeedbackDetails ? (_jsxs("div", { style: { display: 'grid', gap: 10 }, children: [_jsx("p", { style: { margin: 0, color: 'var(--hub-text-accent-2)', fontSize: 14 }, children: "AI review is ready. Expand detailed feedback for a clear, full report." }), _jsx("button", { type: "button", onClick: () => {
                                                            setShowDetailedFeedback((prev) => !prev);
                                                        }, className: "writing-primary-button", style: { ...primaryButtonStyle, marginTop: 0, background: 'var(--hub-muted-surface-soft)', color: 'var(--hub-text-strong)' }, children: showDetailedFeedback ? 'Hide detailed feedback' : 'Show detailed feedback' }), showDetailedFeedback && (_jsxs("div", { style: { borderRadius: 14, border: '1px solid #e2e8f0', background: '#f8fafc', padding: '20px 18px', color: '#0f172a', display: 'grid', gap: 20 }, children: [_jsxs("section", { children: [_jsxs("p", { style: { margin: 0, fontSize: 18, fontWeight: 900, color: '#0f172a' }, children: ["Total: ", progressAssessment?.total_score != null ? `${progressAssessment.total_score}/20` : '—'] }), _jsx("p", { style: { margin: '8px 0 0', fontSize: 16, lineHeight: 1.8 }, children: aiFeedbackDetails.task_understanding || toAlignmentLabel(aiFeedbackDetails.alignment) })] }), _jsxs("section", { style: { display: 'grid', gap: 8 }, children: [_jsx("h4", { style: { margin: 0, fontSize: 32, lineHeight: 1.2, fontWeight: 900 }, children: "What\u2019s good" }), _jsx("ul", { style: { margin: 0, paddingLeft: 30, display: 'grid', gap: 8 }, children: (fullFeedbackStrengths.length > 0 ? fullFeedbackStrengths : ['You already have a useful starting point with clear ideas.']).map((item, idx) => (_jsx("li", { style: { fontSize: 20, lineHeight: 1.8, fontWeight: 600 }, children: simplifyStudentLanguage(item) }, `inline-good-${idx}`))) })] }), _jsxs("section", { style: { display: 'grid', gap: 8 }, children: [_jsx("h4", { style: { margin: 0, fontSize: 32, lineHeight: 1.2, fontWeight: 900 }, children: "Main things to improve" }), _jsx("ol", { style: { margin: 0, paddingLeft: 30, display: 'grid', gap: 10 }, children: (fullFeedbackImprovements.length > 0 ? fullFeedbackImprovements : ['Focus on clearer wording and more specific support.']).map((item, idx) => (_jsx("li", { style: { fontSize: 20, lineHeight: 1.8, fontWeight: 700 }, children: simplifyStudentLanguage(item) }, `inline-improve-${idx}`))) })] }), fullFeedbackGrammarFixes.length > 0 && (_jsxs("section", { style: { display: 'grid', gap: 8 }, children: [_jsx("h4", { style: { margin: 0, fontSize: 30, lineHeight: 1.2, fontWeight: 900 }, children: "Grammar" }), _jsx("ul", { style: { margin: 0, paddingLeft: 30, display: 'grid', gap: 12 }, children: fullFeedbackGrammarFixes.slice(0, 6).map((item, idx) => (_jsxs("li", { style: { fontSize: 18, lineHeight: 1.75 }, children: [_jsx("p", { style: { margin: 0, fontWeight: 800 }, children: item.original }), _jsxs("p", { style: { margin: '2px 0 0', fontSize: 18, fontWeight: 800 }, children: ["\u2192 better: ", item.better_version] }), item.issue && _jsx("p", { style: { margin: '4px 0 0', color: '#334155' }, children: item.issue })] }, `inline-grammar-${idx}`))) })] })), (fullFeedbackPhraseUpgrades.length > 0 || fullFeedbackStyleTone.length > 0) && (_jsxs("section", { style: { display: 'grid', gap: 8 }, children: [_jsx("h4", { style: { margin: 0, fontSize: 30, lineHeight: 1.2, fontWeight: 900 }, children: "Word choice / phrasing" }), _jsxs("ul", { style: { margin: 0, paddingLeft: 30, display: 'grid', gap: 12 }, children: [fullFeedbackPhraseUpgrades.slice(0, 4).map((item, idx) => (_jsxs("li", { style: { fontSize: 18, lineHeight: 1.75 }, children: [_jsx("p", { style: { margin: 0, fontWeight: 800 }, children: item.original }), _jsxs("p", { style: { margin: '2px 0 0', fontSize: 18, fontWeight: 800 }, children: ["\u2192 better: ", item.better_version] }), item.why_it_helps && _jsx("p", { style: { margin: '4px 0 0', color: '#334155' }, children: item.why_it_helps })] }, `inline-phrase-${idx}`))), fullFeedbackStyleTone.slice(0, 3).map((item, idx) => (_jsxs("li", { style: { fontSize: 18, lineHeight: 1.75 }, children: [_jsx("p", { style: { margin: 0, fontWeight: 800 }, children: item.evidence || 'Style note' }), item.issue && _jsx("p", { style: { margin: '4px 0 0' }, children: item.issue }), item.suggestion && _jsxs("p", { style: { margin: '2px 0 0', fontWeight: 800 }, children: ["\u2192 better: ", item.suggestion] })] }, `inline-style-${idx}`)))] })] })), _jsxs("section", { style: { borderRadius: 12, border: '1px solid #cbd5e1', background: '#eef2ff', padding: '14px 14px' }, children: [_jsx("h4", { style: { margin: 0, fontSize: 30, lineHeight: 1.2, fontWeight: 900 }, children: "Improvement point" }), _jsx("p", { style: { margin: '8px 0 0', fontSize: 19, lineHeight: 1.8, fontWeight: 600 }, children: aiFeedbackDetails.next_move || (aiFeedbackDetails.next_steps ?? [])[0] || 'Pick one point and rewrite that sentence first.' })] })] })), _jsx("p", { style: { margin: 0, color: 'var(--hub-text-accent-2)', fontSize: 13 }, children: feedback || (completedTasksCount > 0 ? `Great consistency. You completed ${completedTasksCount} task${completedTasksCount === 1 ? '' : 's'} this week.` : 'Great start. Your progress grows every day you submit.') })] })) : (_jsx("p", { style: { margin: 0, color: feedback ? 'var(--hub-text-accent-2)' : 'var(--hub-subtext)', fontSize: 15 }, children: feedback || (completedTasksCount > 0 ? `Great consistency. You completed ${completedTasksCount} task${completedTasksCount === 1 ? '' : 's'} this week.` : 'Great start. Your progress grows every day you submit.') }))] })] }), _jsxs("div", { className: "dashboard-grid", children: [_jsxs("section", { ref: progressCardRef, className: "writing-hub-card", style: { ...shellCardStyle, borderColor: 'var(--hub-border-strong)', background: 'var(--hub-surface-card)' }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginBottom: 10 }, children: [_jsx("p", { style: { ...dashboardSectionTitleStyle, color: 'var(--hub-text-accent-2)' }, children: "Your Progress" }), _jsx("span", { style: { ...sectionLabelPillStyle, color: 'var(--hub-text-accent)', borderColor: 'var(--hub-border-strong)' }, children: "Snapshot" })] }), _jsx("h3", { style: { margin: '0 0 8px', fontSize: 21, color: 'var(--hub-text-strong)' }, children: "Progress at a glance" }), _jsxs("p", { style: { margin: '0 0 8px', color: 'var(--hub-text-soft)', fontSize: 15 }, children: ["Current score: ", _jsx("strong", { children: progressAssessment?.total_score != null ? `${progressAssessment.total_score}/20` : 'Waiting for first score' })] }), _jsxs("p", { style: { margin: '0 0 8px', color: 'var(--hub-text-accent)', fontSize: 14 }, children: ["Weekly focus: ", aiWeeklyFocus || 'We picked your focus areas based on your first writing.'] }), _jsxs("p", { style: { margin: '0 0 8px', color: 'var(--hub-text)', fontSize: 14 }, children: ["What\u2019s improving: ", showWeeklyEvidence
                                                        ? `You completed ${weeklySummary?.completed_tasks} task${weeklySummary?.completed_tasks === 1 ? '' : 's'} this week.`
                                                        : 'You are building consistency. Keep submitting daily tasks.'] }), _jsxs("p", { style: { margin: '0 0 10px', color: 'var(--hub-text-accent-2)', fontSize: 14 }, children: ["What to work on today: ", toStudentLabel(nextWeekInputs?.carry_forward_primary_target ?? weeklyGoals[0] ?? 'Keep building your weekly focus skills.')] }), _jsxs("p", { style: { margin: '0 0 10px', color: 'var(--hub-text-accent-2)', fontSize: 14 }, children: ["Next step: ", toStudentLabel(nextWeekInputs?.carry_forward_primary_target ?? 'Keep building your weekly focus skills.')] }), _jsx("button", { type: "button", onClick: () => setShowProgressDetailsModal(true), style: {
                                                    marginTop: 2,
                                                    padding: '10px 12px',
                                                    width: '100%',
                                                    borderRadius: 10,
                                                    ...
                                                    cursor: 'pointer',
                                                    fontWeight: 700,
                                                }, children: "Open full progress details" })] }), _jsxs("section", { className: "writing-hub-card", style: supportPanelStyle, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, marginBottom: 10 }, children: [_jsx("p", { style: { ...dashboardSectionTitleStyle, color: themeMode === 'light' ? '#7c3aed' : '#c4b5fd' }, children: "More Help" }), _jsx("span", { style: { ...sectionLabelPillStyle, color: themeMode === 'light' ? '#7c3aed' : '#d8b4fe', borderColor: themeMode === 'light' ? 'rgba(167, 139, 250, 0.5)' : 'rgba(192, 132, 252, 0.35)' }, children: "Optional support" })] }), _jsx("h3", { style: { margin: '0 0 8px', fontSize: 21, color: 'var(--hub-text-strong)' }, children: "Deep guides & references" }), missionRecommendations.length > 0 && (_jsxs("div", { style: { marginBottom: 12, borderRadius: 12, border: '1px solid rgba(196, 181, 253, 0.45)', background: themeMode === 'light' ? 'rgba(243, 232, 255, 0.72)' : 'rgba(30, 27, 75, 0.35)', padding: 10 }, children: [_jsx("p", { style: { margin: '0 0 6px', color: themeMode === 'light' ? '#7c3aed' : '#d8b4fe', fontSize: 12, fontWeight: 800, letterSpacing: 0.2 }, children: "RECOMMENDED PRACTICE MISSIONS" }), _jsx("div", { ref: missionsCarouselRef, style: {
                                                            display: 'flex',
                                                            gap: 8,
                                                            overflowX: 'auto',
                                                            paddingBottom: 4,
                                                            scrollSnapType: 'x mandatory',
                                                            scrollbarWidth: 'thin',
                                                        }, children: missionRecommendations.map((item) => (_jsxs("div", { ref: (node) => {
                                                                const missionRefs = missionCardRefs.current;
                                                                if (!missionRefs)
                                                                    return;
                                                                ...
                                                                flex: '0 0 auto',
                                                                scrollSnapAlign: 'start',
                                                            }, children: [_jsx("p", { style: { margin: 0, color: 'var(--hub-text-strong)', fontSize: 14, fontWeight: 700 }, children: item.title }), _jsxs("p", { style: { margin: '3px 0 0', color: 'var(--hub-text-accent)', fontSize: 12 }, children: [item.missionCategoryLabel, item.mission?.difficulty ? ` · ${item.mission.difficulty}` : '', item.source === 'category_fallback' ? ' · Practice card' : ' · Mission'] }), _jsx("p", { style: { margin: '6px 0 0', color: 'var(--hub-text)', fontSize: 13 }, children: item.reason }), _jsx("button", { type: "button", onClick: () => {
                                                                        ...
                                                                        fontSize: 12,
                                                                        cursor: 'pointer',
                                                                    }, children: item.source === 'quest' ? 'Start mission' : 'Practice this skill' })] }, getMissionRecommendationKey(item)))) })] })), _jsxs("p", { style: { margin: '0 0 8px', color: 'var(--hub-text)', fontSize: 14 }, children: ["Target score range: ", estimatedTargetRange ? `${estimatedTargetRange.low}–${estimatedTargetRange.high} / 20` : 'Will appear after first scoring'] }), _jsxs("button", { type: "button", onClick: () => setShowTaskTypeGuide((prev) => !prev), className: "help-node-button help-node-button--guide", style: {
                                                    margin: '0 0 10px',
                                                    padding: '10px 12px',
                                                    borderRadius: 12,
                                                    ...
                                                    alignItems: 'center',
                                                    gap: 8,
                                                }, children: [_jsx("span", { "aria-hidden": "true", children: showTaskTypeGuide ? '▾' : '▸' }), showTaskTypeGuide ? 'Hide detailed task guide' : 'Show detailed task guide'] }), showTaskTypeGuide && todayTask.ok && todayTask.data && (_jsxs("div", { style: { borderRadius: 12, border: '1px solid rgba(147, 197, 253, 0.45)', background: 'var(--hub-muted-surface-soft)', padding: 12, marginBottom: 10 }, children: [_jsx("p", { style: { margin: '0 0 6px', color: 'var(--hub-text-accent-2)', fontSize: 12, fontWeight: 800 }, children: "How to answer this task type" }), _jsxs("p", { style: { margin: '0 0 8px', color: 'var(--hub-text)', fontSize: 14 }, children: [_jsx("strong", { children: "Goal:" }), " ", getTaskTypeStudyGuide(todayTask.data.task_type).objective] }), _jsx("p", { style: { margin: '0 0 4px', color: 'var(--hub-text-accent-2)', fontSize: 12, fontWeight: 700 }, children: "Best structure" }), _jsx("ul", { style: { margin: '0 0 8px', paddingLeft: 18, color: 'var(--hub-text-muted)', fontSize: 13 }, children: getTaskTypeStudyGuide(todayTask.data.task_type).structure.map((item) => (_jsx("li", { style: { marginBottom: 3 }, children: item }, item))) }), _jsx("p", { style: { margin: '0 0 4px', color: 'var(--hub-text-accent-2)', fontSize: 12, fontWeight: 700 }, children: "Check before you submit" }), _jsx("ul", { style: { margin: '0 0 8px', paddingLeft: 18, color: 'var(--hub-text-muted)', fontSize: 13 }, children: getTaskTypeStudyGuide(todayTask.data.task_type).qualityChecks.map((item) => (_jsx("li", { style: { marginBottom: 3 }, children: item }, item))) }), _jsx("p", { style: { margin: '0 0 4px', color: 'var(--hub-text-accent-2)', fontSize: 12, fontWeight: 700 }, children: "Common mistakes to avoid" }), _jsx("ul", { style: { margin: 0, paddingLeft: 18, color: 'var(--hub-text-muted)', fontSize: 13 }, children: getTaskTypeStudyGuide(todayTask.data.task_type).pitfalls.map((item) => (_jsx("li", { style: { marginBottom: 3 }, children: item }, item))) })] })), _jsxs("button", { type: "button", onClick: () => setShowTaskContextModal(true), className: "help-node-button help-node-button--context", style: {
                                                    margin: 0,
                                                    padding: '10px 12px',
                                                    borderRadius: 12,
                                                    ...
                                                    alignItems: 'center',
                                                    gap: 8,
                                                }, children: [_jsx("span", { "aria-hidden": "true", children: "\u25C9" }), "Open task + starter context"] })] })] })] })), isWeekComplete && (_jsxs("section", { className: "writing-hub-card", style: { ...shellCardStyle, borderColor: 'rgba(250, 204, 21, 0.45)', background: 'linear-gradient(175deg,#1f2937 0%, #0b1224 80%)' }, children: [_jsx("h3", { style: { marginTop: 0, marginBottom: 8, fontSize: 22, color: '#fde68a' }, children: "Week complete \uD83C\uDFAF" }), _jsx("p", { style: { margin: '0 0 8px', color: '#fef3c7', fontSize: 15 }, children: "You finished your writing mission. Nice consistency and progress. All tasks submitted for now." }), _jsxs("p", { style: { margin: '0 0 8px', color: 'var(--hub-text-soft)', fontSize: 14 }, children: ["Tasks completed: ", completedTasksCount, "/", totalPlannedTasks] }), _jsxs("p", { style: { margin: '0 0 8px', color: 'var(--hub-text-accent)', fontSize: 14 }, children: ["What improved: ", studentFriendlyWeaknesses.length > 0 ? studentFriendlyWeaknesses.slice(0, 2).join(' · ') : 'Your writing control and task focus improved.'] }), _jsx("button", { onClick: () => void handleStart({ fromWeekComplete: true }), disabled: loading || !promptText.trim() || targetWordCount < 20, className: "writing-primary-button", style: { ...primaryButtonStyle, opacity: loading ? 0.7 : 1, cursor: loading ? 'not-allowed' : 'pointer' }, "aria-label": "Start New Week", children: loading ? 'Starting new week…' : 'Start New Week' })] })), !isActiveWeek && (_jsxs("details", { className: "writing-hub-card", style: { ...shellCardStyle, borderColor: 'rgba(148, 163, 184, 0.28)' }, children: [_jsx("summary", { style: { cursor: 'pointer', fontWeight: 700, color: 'var(--hub-text)' }, children: "View your progress details" }), _jsxs("div", { style: { marginTop: 10, display: 'grid', gap: 8 }, children: [_jsx("div", { className: "focus-grid", children: subscaleCards.map((item) => {
                                            ...
                                            return (_jsxs("div", { style: {
                                                    ...
                                                    display: 'grid',
                                                    gap: 6,
                                                }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center' }, children: [_jsx("p", { style: { margin: 0, color: 'var(--hub-text-soft)', fontSize: 13, fontWeight: 700 }, children: item.label }), _jsx("p", { style: { margin: 0, color: 'var(--hub-text-strong)', fontSize: 13, fontWeight: 800 }, children: item.score == null ? '— / 5' : `${item.score}/5` })] }), _jsx("div", { style: { ...progressTrackStyle, height: 8 }, children: _jsx("div", { className: "progress-fill", style: { width: `${scorePercent}%`, height: '100%', background: tone.color } }) }), _jsxs("p", { style: { margin: 0, fontSize: 12, color: 'var(--hub-text-accent-2)' }, children: [tone.label, item.delta != null ? ` · ${item.delta >= 0 ? '+' : ''}${item.delta.toFixed(1)} this month` : ''] })] }, item.key));
                                        }) }), _jsxs("p", { style: { margin: 0, color: 'var(--hub-text-accent)' }, children: ["Main focus: ", toStudentLabel(dashboard.data?.weekly_plan_summary?.primary ?? 'Not set yet')] }), !showWeeklyEvidence ? (_jsxs(_Fragment, { children: [_jsx("p", { style: { margin: 0, color: 'var(--hub-subtext)' }, children: "You\u2019re just getting started this week." }), _jsx("p", { style: { margin: 0, color: 'var(--hub-subtext)' }, children: "Complete today\u2019s task to unlock clearer progress feedback." })] })) : (_jsxs(_Fragment, { children: [_jsxs("p", { style: { margin: 0, color: 'var(--hub-text-accent)' }, children: ["What\u2019s improving: You completed ", weeklySummary?.completed_tasks, " task", weeklySummary?.completed_tasks === 1 ? '' : 's', " this week."] }), _jsxs("p", { style: { margin: 0, color: 'var(--hub-subtext)' }, children: ["What to work on today: ", weeklySummary?.top_remaining_weaknesses.map((item) => toStudentLabel(item)).join(', ') || 'Keep practising your weekly goals.'] }), _jsxs("p", { style: { margin: 0, color: '#86efac' }, children: ["Next step: ", toStudentLabel(nextWeekInputs?.carry_forward_primary_target ?? 'Keep building your weekly focus skills.')] })] })), !showMonthlyEvidence ? (_jsx("p", { style: { margin: 0, color: 'var(--hub-subtext)' }, children: "Complete more writing this month to unlock your growth view." })) : (_jsxs(_Fragment, { children: [_jsx("p", { style: { margin: 0, color: 'var(--hub-text-accent)' }, children: "Monthly growth" }), _jsx("p", { style: { margin: 0 }, children: toStudentLabel(monthlyFacingReport?.score_change ?? '') }), aiMonthlyWording && _jsx("p", { style: { margin: 0, color: 'var(--hub-text-accent)' }, children: aiMonthlyWording }), _jsx("p", { style: { margin: 0, color: 'var(--hub-subtext)' }, children: monthlyFacingReport?.subscale_progress.join(' ') }), _jsxs("p", { style: { margin: 0, color: '#86efac' }, children: ["Strongest gains: ", monthlyFacingReport?.strongest_gains.map((item) => toStudentLabel(item)).join(', ')] }), _jsxs("p", { style: { margin: 0, color: '#fca5a5' }, children: ["Main blocker: ", toStudentLabel(monthlyFacingReport?.remaining_blockers[0] ?? 'None right now')] }), _jsxs("p", { style: { margin: 0, color: 'var(--hub-text-accent-2)' }, children: ["Next step: ", toStudentLabel(monthlyFacingReport?.next_month_priorities[0] ?? 'Keep completing your weekly writing tasks.')] })] }))] })] })), (uiNotice || isRefreshingProgress) && (_jsx("p", { style: { ...shellCardStyle, margin: 0, color: 'var(--hub-text-accent)', borderColor: 'rgba(96, 165, 250, 0.45)' }, children: isRefreshingProgress ? 'Loading progress updates…' : uiNotice })), persistenceStatus.state === 'saving' && (_jsx("p", { style: { ...shellCardStyle, margin: 0, color: 'var(--hub-text-accent)', borderColor: 'rgba(56, 189, 248, 0.45)' }, children: "Saving your writing progress\u2026" })), persistenceStatus.state === 'failed' && (_jsxs("section", { style: { ...shellCardStyle, margin: 0, color: '#fecaca', borderColor: 'rgba(248, 113, 113, 0.55)' }, children: [_jsx("p", { style: { margin: '0 0 6px', fontWeight: 700 }, children: "Progress save failed." }), _jsx("p", { style: { margin: 0, color: '#fee2e2', fontSize: 13 }, children: persistenceStatus.message || 'Your latest changes may only be stored locally on this device.' })] })), error && (_jsxs("section", { style: { ...shellCardStyle, margin: 0, color: '#fecaca', borderColor: 'rgba(248, 113, 113, 0.45)' }, children: [_jsx("p", { style: { margin: 0 }, children: error }), _jsx("button", { type: "button", onClick: handleRetryLoad, style: {
                                    marginTop: 10,
                                    padding: '8px 10px',
                                    borderRadius: 10,
                                    ...
                                    fontWeight: 700,
                                    cursor: 'pointer',
                                }, children: "Retry" })] })), writingHistoryByGenre.ok && (_jsxs("section", { className: "writing-hub-card", style: { ...shellCardStyle, display: 'grid', gap: 12 }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }, children: [_jsx("p", { style: { ...dashboardSectionTitleStyle, color: 'var(--hub-text-accent-2)', margin: 0 }, children: "Writing Archive" }), _jsx("span", { style: { ...sectionLabelPillStyle, color: 'var(--hub-text-accent)' }, children: "All genres" })] }), _jsx("h3", { style: { margin: 0, fontSize: 21, color: 'var(--hub-text-strong)' }, children: "Your previous writing + feedback, ready anytime" }), _jsx("p", { style: { margin: 0, color: 'var(--hub-subtext)', fontSize: 14 }, children: "Browse every saved attempt by genre. Open this area anytime to review your old writing and the coaching notes that came with it." }), _jsx("div", { style: { display: 'grid', gap: 10 }, children: writingHistoryByGenre.data?.map((genreHistory) => (_jsxs("article", { style: {
                                        borderRadius: 12,
                                        border: '1px solid var(--hub-border)',
                                        background: 'var(--hub-muted-surface-soft)',
                                        padding: 12,
                                        display: 'grid',
                                        gap: 10,
                                    }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }, children: [_jsx("p", { style: { margin: 0, color: 'var(--hub-text-strong)', fontWeight: 800 }, children: toGenreLabel(genreHistory.genre) }), _jsxs("span", { style: { color: 'var(--hub-text-accent)', fontSize: 12, fontWeight: 700 }, children: [genreHistory.entries.length, " saved ", genreHistory.entries.length === 1 ? 'entry' : 'entries'] })] }), genreHistory.entries.length === 0 ? (_jsx("p", { style: { margin: 0, color: 'var(--hub-subtext)', fontSize: 13 }, children: "No saved writing for this genre yet." })) : (_jsx("div", { style: { display: 'grid', gap: 8 }, children: genreHistory.entries.map((entry) => (_jsxs("div", { style: {
                                                    borderRadius: 10,
                                                    border: '1px solid var(--hub-border)',
                                                    background: 'var(--hub-overlay-soft)',
                                                    padding: 10,
                                                    display: 'grid',
                                                    gap: 6,
                                                }, children: [_jsxs("p", { style: { margin: 0, color: 'var(--hub-text-accent)', fontSize: 12, fontWeight: 700 }, children: [toAttemptTypeLabel(entry.attempt_type), " \u00B7 ", formatHistoryDate(entry.created_at), " \u00B7 Score ", entry.total_score ?? '--', "/20"] }), _jsxs("p", { style: { margin: 0, color: 'var(--hub-text)', fontSize: 13 }, children: [_jsx("strong", { children: "Prompt:" }), " ", compactSnippet(entry.prompt_text, 140)] }), _jsxs("p", { style: { margin: 0, color: 'var(--hub-text)', fontSize: 13 }, children: [_jsx("strong", { children: "Your writing:" }), " ", compactSnippet(entry.student_submission)] }), _jsxs("p", { style: { margin: 0, color: 'var(--hub-text-accent-2)', fontSize: 13 }, children: [_jsx("strong", { children: "Feedback:" }), " ", entry.has_feedback
                                                                ? compactSnippet(entry.feedback_summary || entry.feedback_next_move || 'Feedback was saved and is ready to review.', 170)
                                                                : 'Feedback not saved for this entry yet.'] })] }, entry.id))) }))] }, `history-${genreHistory.genre}`))) })] })), dashboard.ok && dashboard.data && (_jsxs("p", { style: { margin: 0, color: '#64748b', fontSize: 12, textAlign: 'center' }, children: ["Mission progress: ", dashboard.data.completed_tasks_count, " completed \u00B7 latest score ", dashboard.data.latest_total_score ?? '--'] })), !stateRes.ok && (_jsxs("section", { className: "writing-hub-card", style: { ...shellCardStyle, borderColor: 'rgba(248,113,113,0.5)' }, children: [_jsx("h3", { style: { marginTop: 0, marginBottom: 8, color: '#fecaca' }, children: "We\u2019re having trouble loading your writing data." }), _jsx("p", { style: { margin: 0, color: '#fecaca' }, children: "Please refresh and try again. Your writing progress is safe." }), _jsx("button", { type: "button", onClick: handleRetryLoad, style: {
                                    marginTop: 10,
                                    padding: '8px 10px',
                                    borderRadius: 10,
                                    border: '1px solid rgba(248, 113, 113, 0.6)',
                                    ...
                                    fontWeight: 700,
                                    cursor: 'pointer',
                                }, children: "Retry loading" })] })), showNoWritingState && !isPreWeek && (_jsxs("section", { className: "writing-hub-card", style: shellCardStyle, children: [_jsx("h3", { style: { marginTop: 0, marginBottom: 8, color: 'var(--hub-text-strong)' }, children: "No active writing task yet" }), _jsx("p", { style: { margin: 0, color: 'var(--hub-text-muted)' }, children: "Start your first week to unlock your mission board and daily coaching." })] }))] })), showAiReviewModal && submittedPracticeText.trim() && createPortal((_jsx("div", { role: "dialog", "aria-modal": "true", "aria-label": "AI feedback review", onClick: () => setShowAiReviewModal(false), style: {
                    ...modalThemeVars,
                    position: 'fixed',
                    inset: 0,
                    zIndex: 10050,
                    ...
                    overflow: 'hidden',
                }, children: _jsxs("div", { className: "ai-review-modal-shell", onClick: (event) => event.stopPropagation(), style: {
                        width: 'min(840px, 100%)',
                        ...
                        gap: 10,
                    }, children: [_jsxs("div", { style: { position: 'relative', zIndex: 1, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10 }, children: [_jsxs("div", { style: { display: 'grid', gap: 6 }, children: [_jsxs("div", { className: "ai-review-brand", children: [_jsx("img", { src: "/logo.png", alt: "Brains Heist", style: { width: 22, height: 22, objectFit: 'contain' } }), _jsx("span", { children: "Brains Heist" })] }), _jsx("p", { style: { margin: 0, color: 'var(--hub-text-accent-2)', fontSize: 12, fontWeight: 900, letterSpacing: 0.9, textTransform: 'uppercase' }, children: "AI Feedback" }), _jsx("h3", { style: { margin: 0, color: 'var(--hub-text-strong)', fontSize: 22, textShadow: '0 1px 0 color-mix(in srgb, var(--hub-text) 14%, transparent)' }, children: aiReviewViewMode === 'full' ? 'Submitted · Full feedback report' : 'Submitted · Smart review in progress' })] }), _jsx("button", { type: "button", onClick: () => setShowAiReviewModal(false), style: {
                                        borderRadius: 999,
                                        border: '1px solid var(--hub-border)',
                                        ...
                                        lineHeight: '18px',
                                        cursor: 'pointer',
                                    }, "aria-label": "Close AI feedback modal", children: "\u00D7" })] }), _jsxs("div", { style: { position: 'relative', zIndex: 1, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }, children: [_jsx("span", { style: { borderRadius: 999, padding: '4px 10px', border: '1px solid var(--hub-border-strong)', color: 'var(--hub-text-accent-2)', background: 'var(--hub-accent-surface)', fontSize: 12, fontWeight: 800 }, children: "Submitted \u2713" }), _jsx("span", { style: { borderRadius: 999, padding: '4px 10px', border: '1px solid var(--hub-border-strong)', color: 'var(--hub-text-accent-2)', background: 'var(--hub-accent-surface)', fontSize: 12, fontWeight: 800 }, children: reviewScanComplete ? 'Review complete ✓' : 'AI review scanning…' }), !shouldUseFallbackRanges ? (_jsx("span", { style: { borderRadius: 999, padding: '4px 10px', border: '1px solid var(--hub-border-strong)', color: 'var(--hub-text-soft)', background: 'var(--hub-muted-surface-soft)', fontSize: 12, fontWeight: 800 }, children: "Trusted anchors" })) : (_jsx("span", { style: { borderRadius: 999, padding: '4px 10px', border: '1px solid var(--hub-border)', color: 'var(--hub-text-muted)', background: 'var(--hub-muted-surface-soft)', fontSize: 12, fontWeight: 800 }, children: "Guided highlights" })), _jsxs("span", { style: { display: 'inline-flex', alignItems: 'center', gap: 6, color: 'var(--hub-text-accent-2)', fontSize: 12 }, children: [_jsx("span", { className: "ai-review-status-dot" }), reviewScanComplete ? 'Highlights locked' : 'Live analysis'] }), _jsxs("div", { style: { marginLeft: 'auto', display: 'inline-flex', borderRadius: 999, border: '1px solid var(--hub-border)', background: 'var(--hub-panel)', padding: 3, gap: 4 }, children: [_jsx("button", { type: "button", onClick: () => setAiReviewViewMode('full'), style: {
                                                borderRadius: 999,
                                                border: 'none',
                                                ...
                                                padding: '6px 10px',
                                                cursor: 'pointer',
                                            }, children: "Full feedback" }), _jsx("button", { type: "button", onClick: () => setAiReviewViewMode('cinematic'), style: {
                                                borderRadius: 999,
                                                border: 'none',
                                                ...
                                                padding: '6px 10px',
                                                cursor: 'pointer',
                                            }, children: "Cinematic" })] })] }), aiReviewViewMode === 'full' ? (_jsxs("div", { style: {
                                position: 'relative',
                                zIndex: 1,
                                minHeight: 0,
                                overflowY: 'auto',
                                ...
                                display: 'grid',
                                gap: 18,
                            }, children: [_jsxs("section", { style: { borderRadius: 12, background: 'var(--hub-muted-surface)', border: '1px solid var(--hub-border-strong)', padding: '14px 14px' }, children: [_jsx("p", { style: { margin: 0, fontSize: 14, color: 'var(--hub-text-accent)', fontWeight: 800 }, children: "Overall" }), _jsx("p", { style: { margin: '6px 0 0', fontSize: 28, fontWeight: 900, color: 'var(--hub-text-strong)' }, children: progressAssessment?.total_score != null ? `${progressAssessment.total_score}/20` : '—' }), _jsx("p", { style: { margin: '6px 0 0', fontSize: 16, lineHeight: 1.6, color: 'var(--hub-text)' }, children: aiFeedbackDetails?.task_understanding || toAlignmentLabel(aiFeedbackDetails?.alignment) }), _jsx("p", { style: { margin: '8px 0 0', fontSize: 16, lineHeight: 1.7, color: 'var(--hub-text)' }, children: aiFeedbackDetails?.submission_read || 'You completed your writing attempt. Keep your ideas and now polish precision and clarity.' })] }), _jsxs("section", { style: { display: 'grid', gap: 8 }, children: [_jsx("h4", { style: { margin: 0, fontSize: 34, fontWeight: 900, color: 'var(--hub-text-strong)' }, children: "What\u2019s good" }), fullFeedbackStrengths.length > 0 ? (_jsx("ul", { style: { margin: 0, paddingLeft: 30, display: 'grid', gap: 8 }, children: fullFeedbackStrengths.map((item, idx) => (_jsx("li", { style: { fontSize: 20, lineHeight: 1.8, fontWeight: 600, color: 'var(--hub-text)' }, children: simplifyStudentLanguage(item) }, `full-good-${idx}`))) })) : (_jsx("p", { style: { margin: 0, fontSize: 18, lineHeight: 1.8, color: 'var(--hub-subtext)' }, children: "You have a clear starting point. Keep your key idea and structure." }))] }), _jsxs("section", { style: { display: 'grid', gap: 8 }, children: [_jsx("h4", { style: { margin: 0, fontSize: 34, fontWeight: 900, color: 'var(--hub-text-strong)' }, children: "Main things to improve" }), fullFeedbackImprovements.length > 0 ? (_jsx("ol", { style: { margin: 0, paddingLeft: 30, display: 'grid', gap: 10 }, children: fullFeedbackImprovements.map((item, idx) => (_jsx("li", { style: { fontSize: 20, lineHeight: 1.8, fontWeight: 700, color: 'var(--hub-text)' }, children: simplifyStudentLanguage(item) }, `full-improve-${idx}`))) })) : (_jsx("p", { style: { margin: 0, fontSize: 18, lineHeight: 1.8, color: 'var(--hub-subtext)' }, children: "Focus on one section at a time and make each sentence more precise." }))] }), fullFeedbackGrammarFixes.length > 0 && (_jsxs("section", { style: { display: 'grid', gap: 8 }, children: [_jsx("h4", { style: { margin: 0, fontSize: 30, fontWeight: 900, color: 'var(--hub-text-strong)' }, children: "Grammar & punctuation" }), _jsx("ul", { style: { margin: 0, paddingLeft: 30, display: 'grid', gap: 12 }, children: fullFeedbackGrammarFixes.slice(0, 6).map((item, idx) => (_jsxs("li", { style: { fontSize: 18, lineHeight: 1.75, color: 'var(--hub-text)' }, children: [_jsx("p", { style: { margin: 0, fontWeight: 800 }, children: item.category }), _jsx("p", { style: { margin: '4px 0 0' }, children: _jsx("strong", { children: item.original }) }), _jsxs("p", { style: { margin: '2px 0 0', fontWeight: 800, color: 'var(--hub-text-accent-2)' }, children: ["\u2192 better: ", item.better_version] }), item.issue && _jsx("p", { style: { margin: '4px 0 0', color: 'var(--hub-subtext)' }, children: item.issue })] }, `full-fix-${idx}`))) })] })), (fullFeedbackPhraseUpgrades.length > 0 || fullFeedbackStyleTone.length > 0) && (_jsxs("section", { style: { display: 'grid', gap: 8 }, children: [_jsx("h4", { style: { margin: 0, fontSize: 30, fontWeight: 900, color: 'var(--hub-text-strong)' }, children: "Word choice / phrasing" }), _jsxs("ul", { style: { margin: 0, paddingLeft: 30, display: 'grid', gap: 10 }, children: [fullFeedbackPhraseUpgrades.slice(0, 5).map((item, idx) => (_jsxs("li", { style: { fontSize: 18, lineHeight: 1.75, color: 'var(--hub-text)' }, children: [_jsx("p", { style: { margin: 0 }, children: _jsx("strong", { children: item.original }) }), _jsxs("p", { style: { margin: '2px 0 0', fontWeight: 800, color: 'var(--hub-text-accent-2)' }, children: ["\u2192 better: ", item.better_version] }), item.why_it_helps && _jsx("p", { style: { margin: '4px 0 0', color: 'var(--hub-subtext)' }, children: item.why_it_helps })] }, `full-phrase-${idx}`))), fullFeedbackStyleTone.slice(0, 3).map((item, idx) => (_jsxs("li", { style: { fontSize: 18, lineHeight: 1.75, color: 'var(--hub-text)' }, children: [_jsx("p", { style: { margin: 0 }, children: _jsx("strong", { children: item.evidence || 'Style/tone note' }) }), item.issue && _jsx("p", { style: { margin: '4px 0 0' }, children: item.issue }), item.suggestion && _jsxs("p", { style: { margin: '2px 0 0', fontWeight: 800, color: 'var(--hub-text-accent-2)' }, children: ["\u2192 better: ", item.suggestion] })] }, `full-style-${idx}`)))] })] })), _jsxs("section", { style: { borderRadius: 12, border: '1px solid var(--hub-next-border)', background: 'var(--hub-next-bg)', padding: '14px 14px', display: 'grid', gap: 8 }, children: [_jsx("h4", { style: { margin: 0, fontSize: 28, fontWeight: 900, color: 'var(--hub-next-heading)' }, children: "Improvement point" }), _jsx("p", { style: { margin: 0, fontSize: 18, lineHeight: 1.8, color: 'var(--hub-next-text)' }, children: aiFeedbackDetails?.next_move || (aiFeedbackDetails?.next_steps ?? [])[0] || 'Pick one improvement item and rewrite that sentence first.' }), (aiFeedbackDetails?.next_steps ?? []).slice(0, 3).length > 0 && (_jsx("ul", { style: { margin: 0, paddingLeft: 26, display: 'grid', gap: 6 }, children: (aiFeedbackDetails?.next_steps ?? []).slice(0, 3).map((item, idx) => (_jsx("li", { style: { fontSize: 17, lineHeight: 1.7, color: 'var(--hub-next-text)' }, children: item }, `full-next-${idx}`))) }))] })] })) : (_jsxs("div", { className: "ai-review-body", style: { position: 'relative', zIndex: 1 }, children: [_jsxs("div", { className: "ai-review-essay-panel", ref: reviewEssayPanelRef, style: {
                                        position: 'relative',
                                        zIndex: 1,
                                        borderRadius: 14,
                                        border: '1px solid var(--hub-border)',
                                        ...
                                        overflowY: 'auto',
                                        overflowX: 'hidden',
                                    }, children: [renderAnnotatedText(submittedPracticeText, visibleSubmittedHighlightRanges, reviewActiveIndex, handleReviewHighlightMount), activeLineMeasure.rects.map((line, idx) => {
                                            return (_jsx("span", { ref: (element) => {
                                        ...
                                        })] }), _jsx("div", { className: "ai-review-feedback-card", style: { position: 'relative', zIndex: 1, borderRadius: 12, border: `1px solid ${activeReviewRange?.polarity === 'strong' ? 'var(--hub-border-strong)' : 'var(--hub-border)'}`, background: 'var(--hub-muted-surface)', padding: '12px 12px' }, children: activeReviewRange ? (_jsxs(_Fragment, { children: [_jsx("p", { style: { margin: '0 0 8px', color: activeReviewRange.polarity === 'strong' ? 'var(--hub-text-accent-2)' : 'var(--hub-feedback-weak)', fontWeight: 800, fontSize: 'clamp(15px, 2.6vw, 17px)' }, children: activeReviewNote.label }), _jsx("p", { style: { margin: 0, color: 'var(--hub-text)', fontSize: 'clamp(15px, 2.8vw, 17px)', lineHeight: 1.7 }, children: activeReviewNote.detail }), activeReviewNote.correction && (_jsxs("p", { style: { margin: '10px 0 0', color: 'var(--hub-text-accent-2)', fontSize: 'clamp(15px, 2.7vw, 17px)', lineHeight: 1.7 }, children: ["Better version: ", activeReviewNote.correction] }))] })) : (_jsx("p", { style: { margin: 0, color: 'var(--hub-subtext)', fontSize: 13 }, children: "Select a highlight to view detailed guidance." })) }), reviewScanComplete && (_jsxs("div", { className: "ai-review-next-card", style: {
                                        position: 'relative',
                                        zIndex: 1,
                                        borderRadius: 12,
                                        ...
                                        gap: 10,
                                        flexWrap: 'wrap',
                                    }, children: [_jsx("p", { style: { margin: 0, color: 'var(--hub-next-heading)', fontWeight: 800, fontSize: 'clamp(14px, 2.3vw, 16px)' }, children: "Next action" }), _jsx("p", { style: { margin: 0, color: 'var(--hub-next-text)', fontSize: 'clamp(14px, 2.4vw, 16px)', lineHeight: 1.6, flex: '1 1 280px' }, children: aiFeedbackDetails?.next_move || (aiFeedbackDetails?.next_steps ?? []).slice(0, 1)[0] || 'Pick one red highlight and revise that sentence now.' }), _jsx("button", { type: "button", onClick: () => {
                                                ...
                                                fontSize: 14,
                                                cursor: 'pointer',
                                            }, children: "Start revision" })] }))] })), _jsx("div", { style: { position: 'relative', zIndex: 1, borderTop: '1px solid var(--hub-border)', paddingTop: 10, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }, children: aiReviewViewMode === 'cinematic' ? (_jsxs(_Fragment, { children: [_jsx("p", { style: { margin: 0, color: 'var(--hub-text-strong)', fontSize: 12, fontWeight: 800 }, children: reviewScanPlan.length > 0
                                            ? `Highlight ${(reviewActiveIndex ?? 0) + 1} of ${visibleSubmittedHighlightRanges.length}`
                                            : 'No highlights available yet' }), _jsxs("div", { style: { display: 'flex', gap: 8 }, children: [_jsx("button", { type: "button", onClick: () => handleReviewStepNavigation('previous'), disabled: reviewScanPlan.length === 0 || (reviewActiveIndex ?? 0) <= 0, style: {
                                                    borderRadius: 10,
                                                ...
                                                }, children: "Previous" }), _jsx("button", { type: "button", onClick: () => handleReviewStepNavigation('next'), disabled: reviewScanPlan.length === 0 || (reviewActiveIndex ?? 0) >= Math.max(0, visibleSubmittedHighlightRanges.length - 1), style: {
                                                    borderRadius: 10,
                                                    border: '1px solid var(--hub-nav-button-border)',
                                                    background: 'var(--hub-nav-button-bg)',
                                                    color: 'var(--hub-text)',
                                                ...
                                                }, children: "Next" })] })] })) : (_jsxs(_Fragment, { children: [_jsx("p", { style: { margin: 0, color: 'var(--hub-text-soft)', fontSize: 13, fontWeight: 700 }, children: "Clear structure + larger text mode for easier review." }), _jsx("button", { type: "button", onClick: () => {
                                            setPracticeResponse(submittedPracticeText);
                                            setShowAiReviewModal(false);
                                            ...
                                            fontSize: 14,
                                            cursor: 'pointer',
                                        }, children: "Start revision" })] })) })] }) })), document.body), showProgressDetailsModal && createPortal((_jsx("div", { role: "dialog", "aria-modal": "true", "aria-label": "Full progress details", onClick: () => setShowProgressDetailsModal(false), style: { position: 'fixed', inset: 0, zIndex: 1125, display: 'grid', placeItems: 'center', background: 'rgba(2, 6, 23, 0.78)', padding: 16 }, children: _jsxs("div", { onClick: (event) => event.stopPropagation(), style: { width: 'min(900px, 100%)', maxHeight: 'calc(100vh - 32px)', borderRadius: 16, border: '1px solid rgba(148,163,184,0.4)', background: 'linear-gradient(180deg, #0f172a 0%, #111827 100%)', padding: 14, overflowY: 'auto', display: 'grid', gap: 10 }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }, children: [_jsx("p", { style: { margin: 0, color: 'var(--hub-text)', fontSize: 17, fontWeight: 800 }, children: "Full progress details" }), _jsx("button", { type: "button", ref: closeProgressDetailsButtonRef, onClick: () => setShowProgressDetailsModal(false), "aria-label": "Close full progress details", style: { borderRadius: 999, border: '1px solid rgba(148,163,184,0.45)', background: 'rgba(15,23,42,0.75)', color: 'var(--hub-text)', width: 34, height: 34, fontSize: 18, cursor: 'pointer' }, children: "\u00D7" })] }), _jsx("div", { className: "focus-grid", children: subscaleCards.map((item) => {
                                const tone = getProgressTone(item.score);
                                const scorePercent = item.score == null ? 0 : Math.max(0, Math.min(100, (item.score / 5) * 100));
                                return (_jsxs("div", { style: { borderRadius: 12, border: `1px solid ${tone.glow}`, background: 'rgba(15, 23, 42, 0.55)', padding: 10, display: 'grid', gap: 6 }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center' }, children: [_jsx("p", { style: { margin: 0, color: 'var(--hub-text-soft)', fontSize: 13, fontWeight: 700 }, children: item.label }), _jsx("p", { style: { margin: 0, color: 'var(--hub-text-strong)', fontSize: 13, fontWeight: 800 }, children: item.score == null ? '— / 5' : `${item.score}/5` })] }), _jsx("div", { style: { ...progressTrackStyle, height: 8 }, children: _jsx("div", { className: "progress-fill", style: { width: `${scorePercent}%`, height: '100%', background: tone.color } }) })] }, item.key));
                            }) }), _jsxs("p", { style: { margin: 0, color: 'var(--hub-text-accent)' }, children: ["Main focus: ", toStudentLabel(dashboard.data?.weekly_plan_summary?.primary ?? 'Not set yet')] }), !showMonthlyEvidence ? (_jsx("p", { style: { margin: 0, color: 'var(--hub-subtext)' }, children: "Complete more writing this month to unlock your growth view." })) : (_jsxs(_Fragment, { children: [_jsx("p", { style: { margin: 0, color: 'var(--hub-text-accent)' }, children: "Monthly growth" }), _jsx("p", { style: { margin: 0 }, children: toStudentLabel(monthlyFacingReport?.score_change ?? '') })] }))] }) })), document.body), showTaskContextModal && createPortal((_jsx("div", { role: "dialog", "aria-modal": "true", "aria-label": "Task and first attempt", onClick: () => setShowTaskContextModal(false), style: {
                    position: 'fixed',
                    inset: 0,
                    zIndex: 1100,
                    display: 'flex',
                    ...
                    overflowY: 'auto',
                        ...
                        gridTemplateRows: 'auto auto minmax(0, 1fr) auto',
                        gap: 10,
                    }, children: [_jsxs("div", { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }, children: [_jsxs("div", { children: [_jsx("p", { style: { margin: 0, color: 'var(--hub-text-soft)', fontSize: 16, fontWeight: 800 }, children: "Your first coaching session" }), _jsx("p", { style: { margin: '4px 0 0', color: 'var(--hub-text-accent-2)', fontSize: 12 }, children: "We start with what worked, then fix one thing at a time." })] }), _jsx("button", { type: "button", onClick: () => setShowTaskContextModal(false), "aria-label": "Close task context modal", ref: closeModalButtonRef, style: {
                                        borderRadius: 999,
                                        ...
                                        lineHeight: '18px',
                                        cursor: 'pointer',
                                    }, children: "\u00D7" })] }), _jsxs("div", { style: { display: 'flex', gap: 8, flexWrap: 'wrap' }, children: [_jsx("span", { style: { borderRadius: 999, padding: '3px 9px', border: '1px solid rgba(125, 211, 252, 0.45)', color: '#bae6fd', fontSize: 11 }, children: "1) Understand task" }), _jsx("span", { style: { borderRadius: 999, padding: '3px 9px', border: '1px solid rgba(74, 222, 128, 0.45)', color: '#bbf7d0', fontSize: 11 }, children: "2) Keep strengths" }), _jsx("span", { style: { borderRadius: 999, padding: '3px 9px', border: '1px solid rgba(248, 113, 113, 0.45)', color: '#fecaca', fontSize: 11 }, children: "3) Repair step by step" })] }), _jsx("div", { style: { minHeight: 0, overflowY: 'auto', paddingRight: 2, display: 'grid', gap: 10, alignContent: 'start' }, children: _jsxs("div", { className: "focus-grid", style: { gap: 12 }, children: [_jsxs("div", { style: { ...fieldStyle, background: 'rgba(15, 23, 42, 0.46)', minHeight: 200 }, children: [_jsx("p", { style: { margin: '0 0 8px', color: 'var(--hub-text-accent-2)', fontSize: 12, fontWeight: 700 }, children: "Step 1 \u00B7 What the task asked" }), _jsx("p", { style: { margin: 0, fontSize: 14, color: 'var(--hub-text)', whiteSpace: 'pre-wrap', overflowWrap: 'anywhere' }, children: originalPromptText ?? promptText }), _jsxs("div", { style: { marginTop: 10, borderTop: '1px solid rgba(148, 163, 184, 0.25)', paddingTop: 10 }, children: [_jsx("p", { style: { margin: '0 0 6px', color: '#86efac', fontSize: 12, fontWeight: 700 }, children: "Step 2 \u00B7 Keep these strengths" }), studentStrengths.length > 0 ? (_jsx("ul", { style: { margin: 0, paddingLeft: 18, color: '#dcfce7', fontSize: 13, display: 'grid', gap: 4 }, children: studentStrengths.map((item, idx) => (_jsx("li", { children: item }, `first-strength-${idx}`))) })) : (_jsx("p", { style: { margin: 0, color: '#bbf7d0', fontSize: 13 }, children: "You already have a useful starting idea. Keep that clarity while you revise." }))] })] }), _jsxs("div", { style: { ...fieldStyle, background: 'rgba(15, 23, 42, 0.46)', minHeight: 200, display: 'grid', gap: 10 }, children: [_jsxs("div", { children: [_jsx("p", { style: { margin: '0 0 8px', color: 'var(--hub-text-accent-2)', fontSize: 12, fontWeight: 700 }, children: "Your first attempt (coached view)" }), _jsxs("div", { style: { display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }, children: [_jsx("span", { style: { borderRadius: 999, padding: '2px 8px', border: '1px solid rgba(74, 222, 128, 0.4)', color: '#86efac', fontSize: 11 }, children: "Strong signals" }), _jsx("span", { style: { borderRadius: 999, padding: '2px 8px', border: '1px solid rgba(248, 113, 113, 0.45)', color: '#fca5a5', fontSize: 11 }, children: "Repair targets" }), anchorTrustEvaluation.mode === 'trusted' ? (_jsx("span", { style: { borderRadius: 999, padding: '2px 8px', border: '1px solid rgba(96, 165, 250, 0.45)', color: 'var(--hub-text-accent)', fontSize: 11 }, children: "Trusted anchor highlight mode" })) : anchorTrustEvaluation.mode === 'stale_feedback' ? (_jsx("span", { style: { borderRadius: 999, padding: '2px 8px', border: '1px solid rgba(248,113,113,0.45)', color: '#fca5a5', fontSize: 11 }, children: "Anchors blocked (out-of-sync feedback)" })) : anchorTrustEvaluation.mode === 'missing_fingerprint' ? (_jsx("span", { style: { borderRadius: 999, padding: '2px 8px', border: '1px solid rgba(250, 204, 21, 0.45)', color: '#fde68a', fontSize: 11 }, children: "Anchors unavailable (missing fingerprint)" })) : anchorTrustEvaluation.mode === 'no_anchors' ? (_jsx("span", { style: { borderRadius: 999, padding: '2px 8px', border: '1px solid rgba(148, 163, 184, 0.4)', color: 'var(--hub-text-muted)', fontSize: 11 }, children: "Guidance-only mode (no anchors provided)" })) : (_jsx("span", { style: { borderRadius: 999, padding: '2px 8px', border: '1px solid rgba(148, 163, 184, 0.4)', color: 'var(--hub-text-muted)', fontSize: 11 }, children: "Safe fallback mode" }))] }), _jsx("div", { style: {
                                                            borderRadius: 10,
                                                            border: '1px solid rgba(148, 163, 184, 0.35)',
                                                            padding: 10,
                                                            background: 'rgba(15, 23, 42, 0.55)',
                                                        ...
                                                        })() })] }), _jsxs("div", { style: { borderTop: '1px solid rgba(148, 163, 184, 0.25)', paddingTop: 10, minHeight: 230 }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center', marginBottom: 8 }, children: [_jsx("p", { style: { margin: 0, color: 'var(--hub-text-muted)', fontSize: 12, fontWeight: 700 }, children: "Step 3 \u00B7 Let\u2019s fix one thing at a time" }), repairQueue.length > 0 && (_jsxs("span", { style: { color: 'var(--hub-text-accent)', fontSize: 11 }, children: ["Step ", Math.min(repairQueue.length, sessionSeenRepairCount || 1), " of ", repairQueue.length, " \u00B7 session progress"] }))] }), repairQueue.length > 0 && (_jsx("div", { style: { height: 6, borderRadius: 999, background: 'rgba(30,41,59,0.8)', border: '1px solid rgba(148,163,184,0.3)', marginBottom: 10 }, children: _jsx("div", { style: { height: '100%', borderRadius: 999, width: `${(sessionSeenRepairCount / repairQueue.length) * 100}%`, background: 'linear-gradient(90deg, #38bdf8 0%, #34d399 100%)', transition: 'width 180ms ease' } }) })), _jsx("p", { style: { margin: '0 0 8px', color: 'var(--hub-subtext)', fontSize: 12 }, children: "Pick one step, revise that part, then move to the next." }), _jsx("div", { style: { display: 'grid', gap: 6 }, children: repairQueue.length === 0 ? (_jsx("p", { style: { margin: 0, color: 'var(--hub-subtext)', fontSize: 13 }, children: "No repair steps available yet. Submit feedback to unlock targeted fixes." })) : repairQueue.map((item, idx) => {
                                                            const active = item.id === activeRepairItem?.id;
                                                            return (_jsxs("button", { type: "button", onClick: () => setActiveRepairId(item.id), ref: idx === 0 ? firstRepairButtonRef : undefined, style: {
                                                                    textAlign: 'left',
                                                                    borderRadius: 10,
                                                                    ...
                                                                    cursor: 'pointer',
                                                                    color: 'var(--hub-text)',
                                                                }, children: [_jsxs("p", { style: { margin: 0, fontSize: 12, color: '#fca5a5', fontWeight: 700 }, children: [active ? 'Start here' : `Step ${idx + 1}`, " \u00B7 ", item.category.replace('_', ' ')] }), _jsx("p", { style: { margin: '2px 0 0', fontSize: 13, fontWeight: 700 }, children: item.title }), _jsx("p", { style: { margin: '4px 0 0', fontSize: 12, color: 'var(--hub-text-muted)' }, children: item.explanation })] }, item.id));
                                                        }) })] }), _jsx("p", { style: { margin: 0, color: 'var(--hub-text-accent)', fontSize: 12, minHeight: 18 }, children: repairStatusMessage })] })] }) }), _jsx("div", { style: { display: 'flex', justifyContent: 'flex-end' }, children: _jsx("button", { type: "button", onClick: () => setShowTaskContextModal(false), style: {
                                    padding: '10px 14px',
                                    borderRadius: 10,
                                    border: '1px solid rgba(148, 163, 184, 0.45)',
...
const isLegacyForced = () => {
    const envFlag = typeof import.meta !== 'undefined' && import.meta.env
        ? import.meta.env?.['VITE_WRITING_HUB_FORCE_LEGACY_UI']
        : undefined;
    if (typeof envFlag === 'string' && ['1', 'true', 'on', 'yes'].includes(envFlag.toLowerCase()))
        return true;
    if (typeof window === 'undefined')
        return false;
...
const defaultWritingTarget = (grade) => getCambridgeWritingProfile(grade).target_word_count;
...
const WritingHubSimpleLoop = ({ studentId, studentName, grade, genre }) => {
    ...
    const [cinematicReplaySource, setCinematicReplaySource] = useState(null);
    const [cinematicIndex, setCinematicIndex] = useState(null);
    ...
    const [revisionCycleId, setRevisionCycleId] = useState(() => createRevisionCycleId());
    const [attemptNumber, setAttemptNumber] = useState(1);
    const [lastAttemptId, setLastAttemptId] = useState(null);
    const [lastRetryKind, setLastRetryKind] = useState('new_prompt');
    const integrityMode = 'formal';
    const [compositionTelemetry, setCompositionTelemetry] = useState(() => createWritingCompositionTelemetry('formal', null, initialProfile.time_limit_seconds));
    const [latestIntegritySignals, setLatestIntegritySignals] = useState(null);
    ...
    const responseFieldRef = useRef(null);
    const cinematicTextPanelRef = useRef(null);
    const cinematicRangeRefs = useRef({});
    ...
    const studentHistoryReady = hydratedForStudentId === studentId
        ...
        && academicYearsReady;
    const draftStorageKey = useMemo(() => buildWritingDraftStorageKey(studentId, activeGenre, promptText), [studentId, activeGenre, promptText]);
    const allWritingHistoryByGenre = useMemo(() => listStudentWritingHistoryByGenre(studentId), [studentId, hydrationStatus, assessment?.total_score, aiFeedback]);
            ...
            data: allWritingHistoryByGenre.data.map((item) => ({
                ...
                entries: item.entries.filter((entry) => {
                    ...
                    if (entry.academic_year_id)
                        return entry.academic_year_id === selectedAcademicYear.id;
                    ...
                    if (dateMatchedYear)
                        return dateMatchedYear.id === selectedAcademicYear.id;
    ...
    const archivedEntries = useMemo(() => (writingHistoryByGenre.data ?? []).flatMap((item) => item.entries), [writingHistoryByGenre]);
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        void ensureWritingHydrationForStudent(studentId).finally(() => {
            if (!cancelled)
                setHydratedForStudentId(studentId);
        });
        return () => {
            cancelled = true;
        };
    ...
    const loadFreshPrompt = async (genreOverride, force = false) => {
        if (busy && !force)
            return;
        if (draft.trim() && !force) {
    ...
    const preserveVoidedAttemptAndRotate = async (reason, signals) => {
        if (voidingAttemptRef.current || assessment)
            return;
        voidingAttemptRef.current = true;
        setBusy(true);
        const finalSignals = finalizeWritingCompositionTelemetry({ ...signals, prompt_restart_count: signals.prompt_restart_count + 1 }, draft);
    ...
    useEffect(() => {
        ...
        const handleVisibility = () => {
            if (document.visibilityState !== 'hidden' || !wizardOpen || !promptId || assessment || voidingAttemptRef.current)
                return;
    ...
    useEffect(() => {
        if (!wizardOpen || !promptId || assessment || voidingAttemptRef.current)
            return;
        const timer = window.setInterval(() => setRemainingSeconds((current) => Math.max(0, current - 1)), 1000);
        return () => window.clearInterval(timer);
    }, [assessment, promptId, wizardOpen]);
    useEffect(() => {
        if (remainingSeconds !== 0 || !wizardOpen || !promptId || assessment || voidingAttemptRef.current)
            return;
    ...
    const activeCinematicDetail = useMemo(() => describeHighlight(activeCinematicRange, activeCinematicText, activeCinematicFeedback), [activeCinematicRange, activeCinematicText, activeCinematicFeedback]);
    ...
    useEffect(() => {
        if (issueConsistency.valid || process.env['NODE_ENV'] === 'production')
            return;
    ...
    const handleRangeMount = (index, element) => {
    ...
    useEffect(() => {
        if (!showCinematicFeedback || !cinematicDone)
            return;
    ...
    useEffect(() => {
        if (!showCinematicFeedback || cinematicIndex == null)
            return;
        ...
        stepTl.fromTo(activeRangeEl, { backgroundColor: 'rgba(15,23,42,0.1)', boxShadow: `0 0 0px ${glowColor}` }, { backgroundColor: softGlow, boxShadow: `0 0 18px ${glowColor}`, duration: 0.4 }, '-=0.35');
    ...
    useEffect(() => {
        if (typeof document === 'undefined' || !showCinematicFeedback)
            return;
    ...
    useEffect(() => {
        if (typeof window === 'undefined' || !showCinematicFeedback)
            return;
    ...
    useEffect(() => {
        if (!showCinematicFeedback || cinematicRanges.length === 0)
            return;
        if (typeof window === 'undefined')
            return;
        const onKeyDown = (event) => {
            ...
            if (event.key === 'ArrowLeft')
                setCinematicIndex((prev) => Math.max(0, (prev ?? 0) - 1));
        };
    ...
    return (_jsxs("div", { className: "writing-studio", children: [_jsxs("section", { "data-testid": "writing-hub-student-view", className: "writing-studio__hero", children: [_jsxs("div", { children: [_jsx("span", { className: "writing-studio__eyebrow", children: "AI writing coach" }), _jsx("h2", { "data-testid": "writing-hub-title", children: "Writing Hub" }), _jsx("span", { className: "writing-studio__legacy-label", children: "Your Writing Space" }), _jsx("p", { children: "Your progress, saved work, and next writing task in one clear place." })] }), _jsxs("div", { className: "writing-studio__save-state", "data-state": persistenceStatus.state, "aria-live": "polite", children: [_jsx("span", { "aria-hidden": "true" }), hydrationStatus === 'loading'
                                ? 'Loading your work…'
                                    ...
                                    : persistenceStatus.state === 'saving'
                                        ...
                                        : persistenceStatus.state === 'failed'
                                            ...
                                            : persistenceStatus.state === 'saved'
                                                ? 'Saved securely'
                                                : 'Ready'] }), wizardOpen && _jsx("ol", { className: "writing-studio__steps", "aria-label": "Writing journey", children: ['Understand the question', 'Write your response', 'Show the feedback'].map((label, index) => {
                            const activeStep = assessment
                                ? 3
                                : draft.trim()
                                    ? 2
                                    : 1;
                            return (_jsxs("li", { className: index + 1 <= activeStep ? 'is-active' : '', children: [_jsx("span", { children: index + 1 }), label] }, label));
                        }) }), wizardOpen && notice && _jsx("p", { className: "writing-studio__notice", "aria-live": "polite", children: notice }), error && _jsx("p", { className: "writing-studio__error", role: "alert", children: error })] }), !wizardOpen && academicYears.length > 0 && _jsxs("section", { className: "writing-studio__card", "aria-label": "Writing academic year", children: [_jsxs("div", { className: "writing-studio__section-heading", children: [_jsxs("div", { children: [_jsx("span", { children: "Academic Year" }), _jsx("h3", { style: { margin: 0 }, children: selectedAcademicYear?.name || 'School year' })] }), _jsx("select", { value: selectedAcademicYearId, onChange: (event) => setSelectedAcademicYearId(event.target.value), "aria-label": "Writing academic year", style: { border: '1px solid #cbd5e1', borderRadius: 10, padding: '8px 10px', background: '#fff', color: '#0f172a', fontWeight: 700 }, children: academicYears.map((year) => _jsxs("option", { value: year.id, children: [year.name, " ", year.status === 'current' ? '(Current)' : '(Archived)'] }, year.id)) })] }), _jsx("p", { style: { margin: 0, color: '#475569' }, children: isArchivedAcademicYear
                            ? 'Archived · read only. Your writing, scores and feedback are preserved exactly as school-year history.'
                            : 'Current academic year. New writing and live progress are recorded here.' })] }), !wizardOpen && _jsxs("section", { className: "writing-studio__card writing-studio__dashboard", "aria-labelledby": "writing-progress-title", children: [_jsxs("div", { className: "writing-studio__section-heading", children: [_jsxs("div", { children: [_jsx("span", { children: "Your dashboard" }), _jsx("h3", { id: "writing-progress-title", children: "Your writing analysis" })] }), _jsxs("span", { className: "writing-studio__dashboard-badge", children: [archivedEntries.length, " saved"] })] }), _jsxs("div", { className: "writing-studio__dashboard-grid writing-studio__dashboard-grid--detailed", children: [_jsxs("div", { children: [_jsxs("strong", { children: [currentDashboardScore ?? '—', _jsx("small", { children: "/20" })] }), _jsx("span", { children: "Latest score" })] }), _jsxs("div", { children: [_jsx("strong", { children: scoreChange == null ? '—' : `${scoreChange > 0 ? '+' : ''}${scoreChange}` }), _jsx("span", { children: "Progress since last score" })] }), _jsxs("div", { children: [_jsx("strong", { children: archivedEntries.length }), _jsx("span", { children: "Completed submissions" })] }), _jsxs("div", { children: [_jsxs("strong", { children: [writingHistoryByGenre.data?.filter((item) => item.entries.length > 0).length ?? 0, _jsxs("small", { children: ["/", SUPPORTED_GENRES.length] })] }), _jsx("span", { children: "Genres practised" })] }), _jsxs("div", { children: [_jsx("strong", { children: dashboardFocusAreas.length }), _jsx("span", { children: "Current focus areas" })] }), _jsxs("div", { children: [_jsx("strong", { children: totalGrammarIssues + totalPunctuationIssues }), _jsx("span", { children: "Language fixes identified" })] })] }), _jsxs("div", { className: "writing-studio__focus-panel", children: [_jsx("strong", { children: "All focus areas" }), dashboardFocusAreas.length > 0 ? (_jsx("ul", { children: dashboardFocusAreas.map((tag) => _jsx("li", { children: tag.replaceAll('_', ' ') }, tag)) })) : _jsx("p", { children: "Complete a response to unlock your personalised focus areas." })] }), _jsxs("div", { className: "writing-studio__dashboard-actions", children: [_jsx("p", { children: scoreChange != null && scoreChange > 0 ? `You improved by ${scoreChange} points on your latest scored task.` : 'Your saved work, scores, focus areas, and feedback history stay available below.' }), _jsx("button", { type: "button", className: "writing-studio__primary-button", disabled: isArchivedAcademicYear, onClick: () => {
                                    if (isArchivedAcademicYear)
                                        return;
                                    setRemainingSeconds(timeLimitSeconds);
                                    setCompositionTelemetry(createWritingCompositionTelemetry('formal', null, timeLimitSeconds));
                                    setWizardOpen(true);
                                }, children: isArchivedAcademicYear ? 'Archived year — switch to current year' : 'Start a new writing task' })] })] }), wizardOpen && _jsxs(_Fragment, { children: [_jsxs("section", { "data-testid": "writing-hub-today-section", className: "writing-studio__card writing-studio__prompt-card", children: [_jsxs("div", { className: "writing-studio__section-heading", children: [_jsxs("div", { children: [_jsx("span", { children: "Step 1 \u00B7 Today" }), _jsx("h3", { children: "Understand your task" })] }), _jsx("button", { type: "button", className: "writing-studio__secondary-button", onClick: () => setShowPromptChooser((value) => !value), disabled: busy || Boolean(draft.trim()), children: showPromptChooser ? 'Hide genres' : 'Change genre' })] }), _jsxs("div", { style: { display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }, children: [_jsx("strong", { children: toGenreLabel(activeGenre) }), _jsx("span", { style: { color: '#334155' }, children: getCambridgeWritingTaskLabel(grade, activeGenre) }), _jsxs("span", { style: { color: '#334155' }, children: ["\uD83D\uDD58 ", !studentHistoryReady
                                                ? 'Checking saved submissions…'
                                                : `${promptHistoryCount} previous ${promptHistoryCount === 1 ? 'submission' : 'submissions'}`] })] }), showPromptChooser && _jsx("div", { className: "writing-studio__genre-grid", children: SUPPORTED_GENRES.map((item) => (_jsx("button", { type: "button", onClick: () => void loadFreshPrompt(item), disabled: busy || Boolean(draft.trim()), style: {
                                        borderRadius: 8,
                                        ...
                                        padding: '6px 10px',
                                        cursor: 'pointer',
                                    }, children: toGenreLabel(item) }, item))) }), _jsx("div", { className: "writing-studio__prompt-copy", children: promptText })] }), _jsxs("section", { "data-testid": "writing-hub-response-section", className: "writing-studio__card writing-studio__response-card", children: [_jsxs("div", { className: "writing-studio__section-heading", children: [_jsxs("div", { children: [_jsx("span", { children: "Step 2" }), _jsx("h3", { children: "Your Response" })] }), _jsxs("span", { style: { color: wordTone.accent }, children: [wordCount, " / ", minimumWordCount, "\u2013", maximumWordCount, " words"] })] }), _jsxs("div", { className: `writing-studio__integrity-mode writing-studio__integrity-mode--${integrityMode}`, children: [_jsxs("div", { children: [_jsx("strong", { children: toWritingIntegrityModeLabel(integrityMode) }), _jsx("span", { children: "Write directly in this editor. Pasting is blocked; tab changes and elapsed time are recorded." })] }), _jsxs("span", { className: "writing-studio__integrity-badge", children: [Math.floor(remainingSeconds / 60), ":", String(remainingSeconds % 60).padStart(2, '0'), " remaining"] })] }), _jsxs("p", { className: "writing-studio__editor-help", children: ["Required range: ", minimumWordCount, "\u2013", maximumWordCount, " words \u00B7 target ", targetWordCount, " words"] }), _jsx("textarea", { ref: responseFieldRef, value: draft, disabled: busy || !promptId, onChange: (event) => {
                                    const nextValue = event.target.value;
                                    const ignoredPasteCharacters = pastedCharactersToIgnoreRef.current;
                                    pastedCharactersToIgnoreRef.current = 0;
                                    setCompositionTelemetry((current) => recordWritingInput(current, draft, nextValue, new Date(), ignoredPasteCharacters ?? 0));
                                    setDraft(nextValue);
                                }, onPaste: (event) => {
                                    const pastedText = event.clipboardData.getData('text');
                                    const shouldBlock = true;
                                    setCompositionTelemetry((current) => recordWritingPaste(current, pastedText.length, shouldBlock));
                                    if (shouldBlock) {
                                        event.preventDefault();
                                    ...
                                    setCompositionTelemetry((current) => recordWritingFocusLoss(current));
                                }, placeholder: "Write your response here.", className: "writing-studio__editor", "aria-describedby": "writing-editor-help" }), _jsx("p", { id: "writing-editor-help", className: "writing-studio__editor-help", children: "Your draft saves securely while you work. Accessibility accommodations can be arranged by your teacher." }), isVeryShortDraft && _jsx("p", { className: "writing-studio__gentle-warning", children: "Add a little more detail before submitting so your feedback can be useful." }), _jsxs("div", { style: { display: 'flex', gap: 8, flexWrap: 'wrap' }, children: [_jsx("button", { type: "button", onClick: () => void submitAttempt(lastRetryKind), disabled: busy || !promptId || !draft.trim(), className: "writing-studio__primary-button", children: busy ? 'Analyzing…' : 'Submit formal assessment' }), _jsx("button", { type: "button", onClick: () => void loadFreshPrompt(activeGenre), disabled: busy || Boolean(draft.trim()), className: "writing-studio__secondary-button", children: "Change prompt" })] })] }), assessment && (_jsxs("section", { className: "writing-studio__card writing-studio__feedback-card feedback-result-card", children: [_jsxs("div", { className: "writing-studio__section-heading", children: [_jsxs("div", { children: [_jsx("span", { children: "Step 3" }), _jsx("h3", { children: "Your assessment feedback" })] }), _jsxs("div", { children: [_jsx("span", { className: "writing-studio__score-label", children: "AI assessment \u00B7 teacher review when flagged" }), _jsxs("div", { className: "writing-studio__score", children: [assessment.total_score, _jsx("small", { children: "/20" })] })] })] }), latestIntegritySignals && (_jsxs("div", { className: `writing-studio__integrity-result writing-studio__integrity-result--${latestIntegritySignals.review_status}`, children: [_jsx("strong", { children: toWritingIntegrityReviewLabel(latestIntegritySignals.review_status) }), _jsx("span", { children: latestIntegritySignals.review_status === 'review_recommended'
                                            ...
                                            : latestIntegritySignals.review_status === 'practice_mode'
                                                ...
                                                : 'No concern was observed by the writing-process checks. Your teacher remains the final judge.' })] })), cinematicRanges.length > 0 && (_jsxs("button", { type: "button", className: "writing-studio__cinematic-button", onClick: () => {
                                    setCinematicReplaySource(null);
                                    setCinematicIndex(null);
                                    setCinematicDone(false);
                                    setShowCinematicFeedback(true);
                                }, children: [_jsx("span", { "aria-hidden": "true", children: "\u25B6" }), "Play Cinematic Feedback"] })), _jsx("div", { style: { display: 'grid', gap: 10, marginTop: 2 }, children: rubricScores.map((item) => {
                                    ...
                                    return (_jsxs("div", { style: { display: 'grid', gap: 6 }, children: [_jsxs("div", { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 14 }, children: [_jsx("strong", { children: item.label }), _jsx("span", { style: { color: '#475569' }, children: item.value != null ? `${item.value}/5` : '—/5' })] }), _jsx("div", { style: { width: '100%', height: 9, borderRadius: 999, background: '#e2e8f0', overflow: 'hidden' }, children: _jsx("div", { style: { width: progress, height: '100%', borderRadius: 999, background: barColor, transition: 'width 0.45s ease' } }) })] }, item.key));
                                            ...
                                            : cinematicRanges.length
                                                ...
                                                : 'Review complete' }), _jsxs("div", { className: "cinematic-feedback__controls", children: [_jsx("button", { type: "button", className: "is-secondary", onClick: () => {
                                                    if (cinematicDone) {
                                                        setCinematicDone(false);
                                                ...
                                                }, disabled: !cinematicDone && (cinematicIndex == null || cinematicIndex <= 0), children: "Previous" }), !cinematicDone ? (_jsx("button", { type: "button", className: "is-primary", onClick: () => {
                                                ...
                                                }, children: cinematicIndex === cinematicRanges.length - 1 ? 'See my revision mission' : 'Continue' })) : (_jsx("button", { type: "button", className: "is-primary", "aria-label": cinematicReplaySource ? 'Close feedback replay' : 'Improve my draft', onClick: () => {
                                                    setShowCinematicFeedback(false);
                                                ...
                                                }, children: cinematicReplaySource ? 'Close replay' : 'Start my revision' }))] })] })] })] }), document.body)] }));
};
export const WritingHub = (props) => {
    return _jsx(WritingHubSimpleLoop, { ...props });
};
export const seedWritingHubForDemo = (studentId, grade, genre) => {
    submitInitialWritingAssessment({
        ...
        student_response: 'This response describes the event, explains why it mattered, and gives one practical suggestion for next time.',
    });
};
export { getSafeIssueExplanation, isMeaningfullyDifferent, normalizeForComparison, remapIssueType, validateIssueConsistency, };
