export const isActiveSupportStatus = (status) => status === 'new_focus' || status === 'recurring' || status === 'persistent';
export const isEvidenceToConfirmStatus = (status) => status === 'insufficient_evidence';
export const isTeacherReviewStatus = (status) => status === 'contradictory';
...
export const observationDisplayLabel = (type) => {
    if (type === 'focus')
...
export const evidenceConfirmationLabel = (latestType) => {
    if (latestType === 'strength')
        return 'Positive evidence · more evidence needed';
    if (latestType === 'developing')
        return 'Developing evidence · more evidence needed';
    if (latestType === 'focus')
        return 'Potential support signal · more evidence needed';
    return 'More evidence needed';
};
export const focusStatusLabel = (status, latestType, firstObservedAt, lastObservedAt) => {
    switch (status) {
...
export const reportingStatusTone = (status, latestType) => {
    if (status === 'contradictory')
        return 'review';
    if (status === 'persistent')
        return 'critical';
    if (status === 'new_focus' || status === 'recurring')
        return 'focus';
...
export const comparableTrendSegments = (events) => {
...
export const summarizeComparableTrend = (events) => {
    if (!events.length)
        return 'No evidence in this period';
    if (events.length === 1)
        return 'One evidence point so far';
    const bySkill = new Map();
    ...
    if (movingUp && movingDown)
        return 'Mixed movement across comparable skills';
    if (movingUp)
        return 'Comparable evidence shows positive movement';
    if (movingDown)
        return 'Recent comparable evidence needs attention';
