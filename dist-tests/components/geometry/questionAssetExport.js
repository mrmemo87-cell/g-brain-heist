export const GEOMETRY_QUESTION_PADDING = {
    tight: 24,
    standard: 40,
    worksheet: 64,
};
...
const normalizeExportColor = (value, fallback = '#0f172a') => {
...
const expandBounds = (bounds, point, margin = 0) => {
...
const pointsFromFlatArray = (values) => {
    const points = [];
    if (!values)
        return points;
...
const includeShapeBounds = (bounds, shape) => {
...
const includeBlankBounds = (bounds, blank) => {
...
const resolveContentBounds = (shapes, blanks) => {
...
export const buildGeometryQuestionSvg = (shapes, blanks, options) => {
...
const svgToPngFile = async (svg, baseName, width, height) => {
    ...
    try {
        ...
        if (!context)
            throw new Error('Canvas is unavailable for PNG fallback export.');
...
export const createGeometryQuestionAssetDraft = async (title, shapes, blanks, options) => {
