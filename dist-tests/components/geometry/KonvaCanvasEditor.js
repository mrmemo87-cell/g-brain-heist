...
import { generateShapeId } from './geometryService.js';
const GRID_SIZE = 50;
...
const KonvaCanvasEditor = ({ height: propHeight = 450, activeTool, shapes, onShapesChange, blanks, onBlanksChange, selectedShapeIds, onSelectShapes, stageRef, readOnly = false, onEditText, }) => {
    const containerRef = useRef(null);
    const stageObjRef = useRef(null);
    ...
    const [selectionRect, setSelectionRect] = useState({ isSelecting: false, startPoint: null, currentPoint: null });
    ...
    // Handle shape drag
    const handleShapeDrag = useCallback((shapeId, newX, newY, isLine, newPoints) => {
    ...
    const handleShapeTransform = useCallback((shapeId, attrs) => {
        const currentShapes = shapesRef.current;
        if (!currentShapes)
            return;
        onShapesChange(currentShapes.map(s => s.id === shapeId ? { ...s, ...attrs } : s));
    }, [onShapesChange]);
    // Handle blank drag
    const handleBlankDrag = useCallback((blankId, newX, newY) => {
        const currentBlanks = blanksRef.current;
        if (!currentBlanks)
            return;
        onBlanksChange(currentBlanks.map(b => b.id === blankId ? { ...b, x: newX, y: newY } : b));
    }, [onBlanksChange]);
    // Handle blank transform - PERSIST scale
    const handleBlankTransform = useCallback((blankId, attrs) => {
        const currentBlanks = blanksRef.current;
        if (!currentBlanks)
            return;
        onBlanksChange(currentBlanks.map(b => b.id === blankId ? { ...b, ...attrs } : b));
    }, [onBlanksChange]);
    ...
    const drawCanvas = useCallback(() => {
        ...
        if (!layer)
            return;
        // Clear shapes only, keep transformer and selection rect
        const children = layer.children?.slice() || [];
        children.forEach(child => {
            if (child !== transformer && child !== selectionRectRef.current)
                child.destroy();
        });
        const selectedNodes = [];
        ...
        shapes.forEach((shape) => {
                    ...
                    if (activeToolRef.current === 'delete' && shapesRef.current) {
                        ...
                        if (e.evt.shiftKey) {
                            ...
                            if (current.includes(shape.id)) {
                                onSelectShapes(current.filter(id => id !== shape.id));
                            }
                            else {
                                onSelectShapes([...current, shape.id]);
                            }
                        }
                        else {
            ...
            if (konvaShape) {
                konvaShape.setAttr('shapeId', shape.id);
                layer.add(konvaShape);
                nodeMap.set(shape.id, konvaShape);
                if (isSelected)
                    selectedNodes.push(konvaShape);
            }
        });
        // Draw blanks (scalable - bake transforms into dimensions)
        blanks.forEach((blank) => {
            const isSelected = selectedShapeIds.includes(blank.id);
            const group = new Konva.Group({
                x: blank.x,
                y: blank.y,
                draggable: canDrag,
            ...
            group.on('click tap', (e) => {
                ...
                if (activeToolRef.current === 'delete' && blanksRef.current) {
                    ...
                    if (e.evt.shiftKey) {
                        const current = selectedShapeIdsRef.current || [];
                        if (current.includes(blank.id)) {
                            onSelectShapes(current.filter(id => id !== blank.id));
                        }
                        else {
                            onSelectShapes([...current, blank.id]);
                        }
                    }
                    else {
                        onSelectShapes([blank.id]);
                    }
                }
            });
            group.on('dragend', function () {
            ...
            group.on('mouseenter', () => canDrag && setCursorStyle('move'));
            group.on('mouseleave', () => setCursorStyle(activeToolRef.current === 'select' ? 'default' : 'crosshair'));
            group.setAttr('shapeId', blank.id);
            layer.add(group);
            nodeMap.set(blank.id, group);
            if (isSelected)
                selectedNodes.push(group);
        });
        // Update transformer for multi-select
        if (transformer) {
            if (selectedNodes.length > 0 && activeTool === 'select') {
    ...
    useEffect(() => {
        ...
        const handleMouseDown = (e) => {
            ...
            if (!pos)
                return;
            // Select tool - handle selection and drag-to-select
            if (tool === 'select') {
                const target = e.target;
                const isBackground = target === stage || target.getLayer() === gridLayerRef.current;
                if (isBackground) {
                    // Start selection rectangle
                    setSelectionRect({ isSelecting: true, startPoint: pos, currentPoint: pos });
                    onSelectShapes([]);
                }
                return;
            }
        ...
        const handleMouseMove = () => {
            ...
            if (selectionRect.isSelecting && selectionRect.startPoint) {
                setSelectionRect(prev => ({ ...prev, currentPoint: pos }));
                // Draw selection rectangle
                const oldRect = layer.findOne('#selectionRect');
                if (oldRect)
                    oldRect.destroy();
                const start = selectionRect.startPoint;
        ...
        const handleMouseUp = () => {
            ...
            if (selectionRect.isSelecting && selectionRect.startPoint) {
                ...
                if (oldRect)
                    oldRect.destroy();
                const pos = stage.getPointerPosition();
                if (pos && selectionRect.startPoint) {
                    const start = selectionRect.startPoint;
                    const selBox = {
                        x: Math.min(start.x, pos.x),
                        y: Math.min(start.y, pos.y),
                    ...
                    if (selBox.width > 5 || selBox.height > 5) {
                        ...
                        shapesRef.current?.forEach(shape => {
                            ...
                            if (shapeBox && intersects(selBox, shapeBox)) {
                                selected.push(shape.id);
                            }
                        });
                        // Check blanks
                        blanksRef.current?.forEach(blank => {
                            const blankBox = {
                                x: blank.x,
                                y: blank.y,
                                width: blank.width,
                ...
                setSelectionRect({ isSelecting: false, startPoint: null, currentPoint: null });
                layer.batchDraw();
                return;
            }
            if (!drawingState.isDrawing || !drawingState.startPoint)
    ...
    }, [readOnly, drawingState, selectionRect, onShapesChange, onBlanksChange, onSelectShapes]);
