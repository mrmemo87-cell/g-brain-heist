...
import { saveToStorage, loadFromStorage, STORAGE_KEYS, addPlayerToSharedList, addActivityEvent, getActivityFeed, getTaskProgress, incrementPvPWin, incrementWeeklyTaskCompleted } from './storageService.js';
...
const withTimeout = async (promise, timeoutMs, signal, label) => {
    let timeoutId;
    let abortHandler;
...
const runNonCritical = async (key, action, timeoutMs, signal, onSuccess, onError) => {
    try {
...
const normalizeTopicName = (topic, fallback) => {
    const value = topic || fallback || 'General';
    const trimmed = value.trim();
    return trimmed.length > 0 ? trimmed : 'General';
};
const coerceQuestionOptions = (rawOptions, questionType) => {
    ...
    if (rawOptions && typeof rawOptions === 'object' && !Array.isArray(rawOptions)) {
        ...
        if ('text' in rawOptions) {
            return [{
                    text: String(rawOptions.text || ''),
                    image_url: rawOptions.image_url || undefined
                }];
        }
...
const normalizeTeacherQuestionPayload = (question) => {
    const resolvedTimeLimitRaw = question.time_limit ?? question.time_limit_seconds;
    ...
    const resolvedPointsRaw = question.points;
    ...
    return {
        ...
        options: coerceQuestionOptions(question.options, question.question_type),
        time_limit: resolvedTimeLimit,
        points: resolvedPoints,
    };
};
const KYRGYZ_BOT_PERSONAS = [
    {
        firstName: 'Aibek',
        lastName: 'Sharipov',
        batch: '8B',
...
const getPersonaBotId = (persona) => `bot_${persona.firstName.toLowerCase()}_${persona.lastName.toLowerCase().replace(/[^a-z]/g, '')}`;
const KYRGYZ_PERSONA_LOOKUP = new Map(KYRGYZ_BOT_PERSONAS.map(persona => [getPersonaBotId(persona), persona]));
const KYRGYZ_BOT_USERNAMES = new Set(KYRGYZ_BOT_PERSONAS.map(persona => `${persona.firstName} ${persona.lastName}`.toLowerCase()));
const isKyrgyzBotId = (value) => {
    if (!value)
        return false;
    return value.startsWith('bot_') && KYRGYZ_PERSONA_LOOKUP.has(value);
};
const isKyrgyzBotUsername = (value) => {
    if (!value)
        return false;
    return KYRGYZ_BOT_USERNAMES.has(value.trim().toLowerCase());
};
const isBotActivityEvent = (activity) => {
    const data = activity?.data || {};
    return (activity?.actor_role === 'bot' ||
        data?.is_bot === true ||
        data?.bot === true ||
...
const approximateXpForLevel = (level) => {
    if (level <= 1) {
        return 0;
    }
...
const generateKyrgyzBots = (count, existingIds) => {
    if (count <= 0) {
        return [];
    }
...
const loadKyrgyzBotStates = () => {
    const stored = loadFromStorage(KYRGYZ_BOT_STATE_KEY);
    return stored ? stored.map(bot => ({ ...bot })) : [];
};
const createInitialBotState = (persona) => {
    const level = randomIntInRange(persona.levelRange);
    const coins = randomIntInRange(persona.coinsRange);
...
const clampBotStateToPersona = (bot) => {
...
const applyKyrgyzBotReactions = (events) => {
...
const simulateKyrgyzBotBackgroundActivity = () => {
    ...
    bots.forEach(bot => {
        ...
        if (minutesSinceRaid > 30 && Math.random() < 0.15) {
            const eventKind = Math.random() < 0.6 ? 'pvp_win' : 'quest_cleared';
            addActivityEvent({
                kind: eventKind,
                actor: bot.username,
                target: undefined,
                data: {
                    details: eventKind === 'pvp_win'
...
const seededRandomFromString = (seed) => {
    let hash = 0;
...
const seededIntInRange = (seed, [min, max]) => {
    if (max <= min) {
...
const createBotLeaderboardProfile = (persona) => {
    const username = `${persona.firstName} ${persona.lastName}`;
...
const buildKyrgyzBotLeaderboardSnapshot = () => {
    ...
    const pvpEntries = personaProfiles
        .map(({ persona, profile }) => {
        const winsRange = persona.style === 'aggressive'
            ? [24, 60]
    ...
    const clanNames = Array.from(new Set(KYRGYZ_BOT_PERSONAS
        .map(persona => persona.clan)
        .filter((clanName) => Boolean(clanName))));
...
const applyRewardDelta = async ({ xpDelta = 0, coinsDelta = 0, gemstonesDelta = 0, applyLevelMilestone = false, }) => {
    void xpDelta;
    void coinsDelta;
    void gemstonesDelta;
...
const updateProfile = async (userId, updates, maxRetries = 3) => {
    let lastError = null;
...
const getRpcSingleRow = (data) => {
    if (!data) {
        return null;
    }
    return Array.isArray(data) ? data[0] ?? null : data;
};
export const getKyrgyzBotLeaderboardProfiles = async () => {
...
let MOCK_PROFILE = loadFromStorage(STORAGE_KEYS.PROFILE) || { ...DEFAULT_PROFILE };
let MOCK_CLAN = loadFromStorage(STORAGE_KEYS.CLAN) || null;
let MOCK_CHAT = loadFromStorage(STORAGE_KEYS.CHAT) || [
...
let MOCK_INVENTORY = loadFromStorage(STORAGE_KEYS.INVENTORY) || [...DEFAULT_INVENTORY];
...
const calculateTotalScore = (xp = 0, pvpScore = 0) => xp + pvpScore * 10;
let hasClanContributionTable = null;
const mapBuffTemplateRow = (row) => ({
    id: row.id,
    code: row.code,
    name: row.name,
...
const combineClanBuffEffects = (buffs) => {
...
const applyClanBuffsToProfile = (profile, buffs) => {
...
const fetchClanDepositTotals = async (clanId, userIds) => {
    if (!userIds.length)
        return new Map();
    ...
    return new Map(data.map((row) => [row.user_id, Number(row.total_deposited ?? 0)]));
};
const fetchClanDepositTotalsFromActivities = async (clanId, userIds) => {
    const { data, error } = await supabase
        .from('activities')
...
const fetchClanActiveBuffs = async (clanId) => {
    const { data, error } = await supabase
...
const fetchClanScoreValue = async (clanId) => {
    const { data, error } = await supabase
        .from('clan_scores')
...
const getTotalAttackPower = (profile, inventory) => {
...
const getTotalDefensePower = (profile, inventory) => {
...
const getActiveCosmeticFrame = async (userId) => {
    const { data, error } = await supabase
        .from('inventory')
...
const getActiveCosmeticTheme = async (userId) => {
    const { data, error } = await supabase
        .from('inventory')
        .select('item_id, kind, state')
        .eq('user_id', userId)
...
const getActiveCosmeticEffect = async (userId) => {
    const { data, error } = await supabase
        .from('inventory')
        .select('item_id, kind, state')
        .eq('user_id', userId)
 ...
 */
export const whoamiTeacher = async () => {
    const user = await getCurrentUser();
...
export const whoami = async () => {
    ...
    whoamiInFlight = (async () => {
        ...
        let resolvedClanId = profile.clan_id ?? null;
        let resolvedClanRole = profile.clan_role;
        let resolvedCustomTitle = profile.clan_custom_title ?? null;
        let resolvedClanName = profile.clan_name ?? null;
        ...
        if (resolvedMembership && resolvedMembership.clan_id) {
            ...
            resolvedClanRole = resolvedMembership.role;
...
export const getCriticalBootData = async ({ signal, timeoutMs = 12000, retryOnTimeout = 1, } = {}) => {
    if (signal?.aborted) {
...
export const kickOffNonCriticalBootLoads = ({ signal, targets, timeoutsMs, onTasks, onCaps, onNews, onAssignment, onSessionStatus, onError, }) => {
    const shouldRun = (key) => !targets || targets.includes(key);
    const timeouts = {
        tasks: 12000,
        caps: 8000,
        news: 12000,
...
export const getPublicProfile = async (userId) => {
    ...
    let active_cosmetic_frame = null;
    let active_cosmetic_theme = null;
    let active_cosmetic_effect = null;
    try {
        ...
        if (neonOwners.has(userId))
            active_cosmetic_frame = 'neon';
        if (flickerOwners.has(userId))
            active_cosmetic_theme = 'flicker';
        if (glitchOwners.has(userId))
    ...
    let clan_name = null;
    let clan_role = undefined;
    let clan_custom_title = null;
    const { data: membership } = await supabase
        .from('clan_members')
        .select('role, custom_title, clans(name)')
        .eq('user_id', userId)
        .maybeSingle();
    if (membership && membership.clans?.name) {
        clan_name = membership.clans.name;
...
export const tasks_list = async () => {
    ...
    const claimedDailyTasks = JSON.parse(localStorage.getItem(claimedDailyKey) || '[]');
    const claimedWeeklyTasks = JSON.parse(localStorage.getItem(claimedWeeklyKey) || '[]');
    const legacyClaimedTasks = JSON.parse(localStorage.getItem(legacyClaimedKey) || '[]');
    ...
    let claimedDailyFromDb = [];
    let claimedWeeklyFromDb = [];
...
export const task_claim = async (task_id) => {
    // Get task details
...
export const caps_status = async () => {
...
export const news_feed = async () => {
    ...
    const studentActivities = (activities || [])
        .filter((activity) => !isBotActivityEvent(activity))
        .slice(0, 20);
    ...
    if (missingUserIds.size > 0) {
        ...
        if (userError) {
            ...
            userLookup = (userRows || []).reduce((acc, row) => {
    ...
    const dbEvents = studentActivities.map((activity) => {
    ...
    const combined = [...localEvents, ...dbEvents]
        ...
        .slice(0, 20);
    return mockApiCall(combined.map(({ timestamp, ...event }) => event));
};
...
export const activity_reaction_toggle = async (activity_id, emoji) => {
    const user = await getCurrentUser();
    if (activity_id.startsWith('evt_')) {
        const events = getActivityFeed();
        const eventIndex = (events || []).findIndex((event) => event.id === activity_id);
        if (eventIndex >= 0) {
            const updatedEvents = [...events];
            const event = { ...updatedEvents[eventIndex] };
            const reactions = { '🔥': 0, '😮': 0, '😂': 0, '❤️': 0, ...(event.reactions || {}) };
            const currentReaction = event.my_reaction || null;
            let added = true;
 ...
 */
export const get_student_subject_progress = async () => {
    try {
        const user = await getCurrentUser();
        // Get all subjects
 ...
 */
export const get_student_subject_progress_with_difficulty = async () => {
    try {
        const user = await getCurrentUser();
        ...
        const normalizeDifficulty = (d) => {
            if (!d)
                return 'easy'; // Default to easy if no difficulty set
            const lower = d.toLowerCase();
            if (lower === 'easy')
                return 'easy';
            if (lower === 'med' || lower === 'medium')
                return 'medium';
            if (lower === 'hard')
                return 'hard';
...
export const raid_targets = async () => {
    ...
    const prioritizedTargets = realTargets.sort((a, b) => {
        ...
        if (a.batch === userBatch && b.batch !== userBatch)
            return -1;
        if (b.batch === userBatch && a.batch !== userBatch)
            return 1;
...
export const getActiveRaidStatus = () => {
    return RaidFeatureService.getActiveRaid();
};
export const getRaidStatusById = (raidId) => {
    return RaidFeatureService.getRaidStatus(raidId);
};
export const getBossUnlockState = (userId) => {
    return RaidFeatureService.getBossUnlockState(userId);
};
export const startRaidEncounter = (bossId) => {
    return RaidFeatureService.startRaid(bossId);
};
export const joinRaid = (raidId, username, userId) => {
    return RaidFeatureService.joinRaid(raidId, username, userId);
};
export const submitRaidWaveAnswer = (payload, wave, participant) => {
    return RaidFeatureService.submitRaidAnswer(payload, wave, participant);
};
export const finalizeRaidEncounter = (raidId, participants) => {
    return RaidFeatureService.finalizeRaid(raidId, participants);
};
export const getRaidWaveQuestions = (request) => {
    return RaidFeatureService.getRaidWaveQuestions(request);
};
export const raid_attack = async (defender_id, _use_cracker, _target, requestId) => {
    ...
    const response = {
        ...
        shield_state: (payload.shield_state ?? 'none'),
...
export const shop_buy = async (item_id, quantity) => {
    const user = await getCurrentUser();
...
export const deactivate_flicker_theme = async () => {
    const user = await getCurrentUser();
...
export const deactivate_glitch_effect = async () => {
    const user = await getCurrentUser();
...
export const clan_list = async () => {
    ...
    if (clanIds.length > 0) {
        ...
        if (metaError) {
            ...
            metaById = new Map(clanMeta.map((clan) => [
                clan.id,
                {
                    notice: clan.notice,
    ...
    const mappedClans = (clanScores || []).map((clan) => {
...
export const clan_get_members_by_id = async (clanId) => {
    const { data, error } = await supabase
    ...
    const neonOwners = await fetchNeonFrameOwners((data || []).map((member) => member.user_id));
    const flickerOwners = await fetchFlickerThemeOwners((data || []).map((member) => member.user_id));
    const glitchOwners = await fetchGlitchEffectOwners((data || []).map((member) => member.user_id));
    const depositTotals = await fetchClanDepositTotals(clanId, (data || []).map((member) => member.user_id));
...
export const clan_join = async (clan_id) => {
    const user = await getCurrentUser();
...
export const clan_get_my_pending_request = async () => {
    const user = await getCurrentUser();
    const { data, error } = await supabase
        .from('clan_join_requests')
...
export const clan_get_pending_request_count = async () => {
    const user = await getCurrentUser();
    const { data: membership } = await supabase
        .from('clan_members')
        .select('clan_id, role')
...
export const clan_get_pending_join_requests = async () => {
    const user = await getCurrentUser();
    const { data: membership } = await supabase
        .from('clan_members')
        .select('clan_id, role')
    ...
    const profileResults = await Promise.allSettled(userIds.map(uid => supabase.rpc('get_public_profile', { target_user_id: uid })));
...
export const clan_approve_join_request = async (requestId) => {
    const user = await getCurrentUser();
    const { data: membership } = await supabase
        .from('clan_members')
        .select('clan_id, role')
    ...
    if (error) {
        ...
        const reasonParts = [error.details, error.hint, error.message, error.code]
            .map((part) => (typeof part === 'string' ? part.trim() : ''))
            .filter((part, index, arr) => part.length > 0 && arr.indexOf(part) === index);
        const combinedReason = reasonParts.length > 0 ? reasonParts.join(' | ') : 'Failed to approve request.';
        throw new Error(`Failed to approve request: ${combinedReason}`);
    }
    return mockApiCall(true);
};
export const clan_reject_join_request = async (requestId) => {
    const user = await getCurrentUser();
    const { data: membership } = await supabase
        .from('clan_members')
        .select('clan_id, role')
...
export const clan_cancel_join_request = async (requestId) => {
    const user = await getCurrentUser();
...
export const clan_details = async () => {
    const user = await getCurrentUser();
    const { data: membership, error: membershipError } = await supabase
        .from('clan_members')
        .select('clan_id')
        .eq('user_id', user.id)
        .maybeSingle();
    let clanId = membership?.clan_id ?? null;
    if (membershipError && membershipError.code !== 'PGRST116') {
    ...
    const neonOwners = await fetchNeonFrameOwners(memberRows.map((m) => m.user_id));
    const flickerOwners = await fetchFlickerThemeOwners(memberRows.map((m) => m.user_id));
    const glitchOwners = await fetchGlitchEffectOwners(memberRows.map((m) => m.user_id));
    const depositTotals = await fetchClanDepositTotals(clan.id, memberRows.map((m) => m.user_id));
...
export const clan_create = async (name, notice) => {
    const user = await getCurrentUser();
...
export const clan_chat_recent = async (clanId) => {
    const user = await getCurrentUser();
...
export const clan_chat_post = async (message, clanId) => {
    const user = await getCurrentUser();
...
export const clan_get_available_buffs = async () => {
    try {
...
export const clan_deposit_coins = async (amount) => {
    const user = await getCurrentUser();
    if (amount <= 0) {
...
export const clan_buy_buff = async (buffCode) => {
...
export const clan_buy_member_slot = async () => {
    await getCurrentUser();
...
export const clan_transfer_leadership = async (targetUserId) => {
    await getCurrentUser();
...
export const clan_update_member_role = async (memberId, newRole, customTitle) => {
    await getCurrentUser();
...
export const clan_delete = async () => {
    const user = await getCurrentUser();
...
export const clan_promote_member = async (user_id) => {
    return clan_update_member_role(user_id, 'officer');
};
export const clan_demote_member = async (user_id) => {
    return clan_update_member_role(user_id, 'member');
};
export const clan_kick_member = async (user_id) => {
    const user = await getCurrentUser();
...
export const clan_update_notice = async (notice) => {
    const user = await getCurrentUser();
...
const sanitizeAvatarExtension = (filename, fallback) => {
...
const compressAvatarIfNeeded = async (file) => {
...
export const upload_avatar_file = async (file) => {
    const user = await getCurrentUser();
...
export const update_avatar = async (avatar_url) => {
...
export const update_username = async (newUsername) => {
    ...
    if (!/^[a-zA-Z0-9_ .\-]+$/.test(trimmed))
        throw new Error('Username can only contain letters, numbers, spaces, hyphens, underscores, and dots.');
    const { data: authData } = await supabase.auth.getUser();
    if (!authData.user)
        throw new Error('Not authenticated');
    const { data, error } = await supabase
 ...
 */
export const upload_question_image = async (file) => {
    const user = await getCurrentUser();
    // Validate file type
...
export const achievements_list = async () => {
    const user = await getCurrentUser();
    ...
    const [questsCompletedRes, questRunsCompletedRes, itemsPurchasedRes, purchasesRes, clanMembershipRes, correctAnswersRes,] = await Promise.all([
        supabase
            .from('activities')
            .select('id', { count: 'exact', head: true })
            .eq('actor_id', user.id)
    ...
    const legacyQuestCount = questsCompletedRes?.count || 0;
    const questRunsCount = questRunsCompletedRes?.count || 0;
    const questCount = Math.max(legacyQuestCount, questRunsCount);
    const purchaseCount = itemsPurchasedRes?.count || 0;
    const hasClanMembership = Boolean(clanMembershipRes?.data?.clan_id) || Boolean(profile?.clan_id);
    const correctAnswersCount = correctAnswersRes?.count || 0;
    // Calculate coins earned (current + spent)
    // Note: activities table uses 'data' JSONB column, not 'amount'
    const purchases = purchasesRes.data || [];
    const coinsSpent = purchases.reduce((sum, p) => sum + (p.data?.amount || p.data?.price || 0), 0);
    ...
    try {
        ...
        if (assignmentError) {
            ...
            perfectScores = assignmentResults.filter((r) => r.accuracy === 100).length;
        }
    }
    catch (err) {
...
export const achievements_reference = async () => {
    ...
    const nextUnlocks = achievements
        ...
        .slice(0, 3)
        .map(({ _remaining, ...achievement }) => achievement);
    return {
        achievements,
        totals,
        latestEarnedAt,
...
export const check_achievements = async () => {
    const user = await getCurrentUser();
    ...
    if (newlyEarned.length > 0) {
        ...
        try {
            for (const achievement of newlyEarned) {
                await notificationService.createNotification(user.id, 'achievement_earned', '🏆 Achievement Unlocked!', `You earned "${achievement.name}"! ${achievement.description}`, 'high', { achievement_id: achievement.id, achievement_name: achievement.name });
            }
        }
        catch (notifError) {
 ...
 */
export const create_teacher_profile = async (schoolName, subjectSpecializations, bio) => {
    const user = await getCurrentUser();
 ...
 */
export const get_teacher_profile = async () => {
    const user = await getCurrentUser();
    const { data, error } = await supabase
        .from('teachers')
        .select('*')
 ...
 */
export const create_question = async (questionData) => {
    const user = await getCurrentUser();
    // Get teacher ID
    ...
    const getDefaultPoints = (difficulty) => {
        switch (difficulty) {
            case 'easy': return 10;
 ...
 */
export const get_my_questions = async () => {
    const teacher = await get_teacher_profile();
    if (!teacher)
        throw new Error('User is not a teacher');
    const { data, error } = await supabase
 ...
 */
export const submit_question_answer = async (questionId, answer, timeTaken, questSessionId) => {
    const user = await getCurrentUser();
    ...
    if (error) {
        ...
        if (error.message?.includes('unique') || ('code' in error && error.code === '23505')) {
            return {
                is_correct: true,
                points_earned: 0,
 ...
 */
export const create_quest_template = async (title, description, subject, questionIds, difficulty, xpReward, coinsReward) => {
    const teacher = await get_teacher_profile();
    if (!teacher)
        throw new Error('User is not a teacher');
    const { data, error } = await supabase
 ...
 */
export const get_quest_templates = async (subject) => {
    const teacher = await get_teacher_profile();
    let query = supabase
        .from('quest_templates')
        .select('*')
...
export const create_assignment = async (payload) => {
    const teacher = await get_teacher_profile();
    if (!teacher)
        throw new Error('User is not a teacher');
...
export const get_student_active_assignment = async () => {
    ...
    let normalizedQuestions = (parsedRow.questions ?? []).map(normalizeTeacherQuestionPayload);
    if (!normalizedQuestions.length) {
        ...
        if (assignmentQuestionError) {
            ...
            normalizedQuestions = (assignmentQuestionRows ?? [])
                .map((row) => normalizeTeacherQuestionPayload(row))
...
export const brains_master_toggle_badge = async (show) => {
    const user = await getCurrentUser();
    await updateProfile(user.id, { brains_master_show_badge: show });
};
export const get_student_pending_assignments = async () => {
 ...
 */
export const get_all_assignment_reports = async (assignmentIds) => {
    const teacher = await get_teacher_profile();
    if (!teacher)
        throw new Error('User is not a teacher');
    const results = {};
 ...
 */
export const get_assignment_student_answers = async (assignmentId, studentId) => {
    const teacher = await get_teacher_profile();
    if (!teacher)
        throw new Error('User is not a teacher');
    ...
    if (error)
        throw new Error(error.message || 'Failed to load student answers');
    return data || [];
};
/**
 ...
 */
export const get_assignment_question_analysis = async (assignmentId) => {
    const teacher = await get_teacher_profile();
    if (!teacher)
        throw new Error('User is not a teacher');
    ...
    if (error)
        throw new Error(error.message || 'Failed to load question analysis');
    return data || [];
};
/**
 * Get student's completed assignments for achievement display
 */
export const get_student_completed_assignments = async () => {
 ...
 */
export const check_assignment_achievements = async (userId) => {
 ...
 */
export const get_my_assignment_answers = async (assignmentId) => {
...
export const mcq_answer_submit = async (question, choice) => {
    if (!question?.id) {
...
// ============================================================
export const createClan = async (clanName, description, avatarUrl) => {
    try {
        ...
        return { clanId: '', success: false, error: err.message };
    }
};
export const joinClan = async (clanId) => {
    try {
...
export const getClanLeaderboard = async (limit = 20) => {
    try {
...
export const updatePvPScore = async (userId, isWin) => {
    try {
        ...
        return { newPvpScore: 0, newTotalScore: 0, success: false, error: err.message };
    }
};
// ============================================================================
 ...
 */
export const generate_assignment_analysis = async (assignmentId, studentId) => {
    try {
        const user = await getCurrentUser();
        ...
        const response = await fetch(`${supabaseUrl}/functions/v1/analyze_assignment_answers`, {
            method: 'POST',
            headers: {
...
const normalizeQuestRunState = (payload) => {
    const state = normalizeRpcPayload(payload);
    if (!state) {
...
export const claim_ftue_training_reward = async () => {
...
export const quest_get_missions = async (subject) => {
...
export const quest_start_run = async (missionId) => {
...
export const quest_resume_run = async (runId) => {
...
export const quest_answer_node = async (runId, nodeIndex, answer) => {
...
export const quest_claim_event = async (runId, nodeIndex) => {
...
export const quest_retreat = async (runId) => {
...
export const quest_open_chest = async (runId) => {
...
export const quest_abandon = async (runId) => {
 ...
 */
export const teacher_backfill_quest_missions_from_questions = async (teacherId) => {
...
export const teacher_get_my_quests = async () => {
...
export const teacher_toggle_quest_active = async (missionId, isActive) => {
    const { error } = await supabase
        .from('quest_missions')
...
export const teacher_delete_quest = async (missionId) => {
    const { error } = await supabase
        .from('quest_missions')
        .delete()
        .eq('id', missionId);
