...
const nullableNumber = (value) => (value === null || value === undefined || value === '' ? null : numberValue(value));
const stringValue = (value, fallback = '') => (typeof value === 'string' ? value : fallback);
export function normalizeSchoolHeadSnapshot(value) {
    const payload = record(value);
    if (payload['success'] !== true || payload['account_type'] !== 'school_head')
        return null;
    ...
    if (!stringValue(school['id']) || !stringValue(school['name']))
        return null;
    const decisions = Array.isArray(payload['decisions'])
        ? payload['decisions'].flatMap((item) => {
            ...
            if (!['info', 'notice', 'warning', 'critical'].includes(String(severity)))
                return [];
...
export async function getSchoolHeadSnapshot(schoolId, days = 30) {
    ...
    if (error)
        throw new Error(error.message || 'Executive data could not be loaded.');
    const snapshot = normalizeSchoolHeadSnapshot(data);
    if (!snapshot)
        throw new Error('Executive data returned an invalid response.');
    return snapshot;
}
export async function listSchoolGovernanceAudit(schoolId, options = {}) {
    ...
    if (error)
        throw new Error(error.message || 'Governance history could not be loaded.');
    if (!Array.isArray(data))
        return [];
    return data.flatMap((item) => {
        const entry = record(item);
        const severity = String(entry['severity']);
        if (!stringValue(entry['id']) || !['info', 'notice', 'warning', 'critical'].includes(severity))
            return [];
...
export async function transferSchoolHeadOwnership(input) {
    ...
    if (error)
        return { success: false, error: error.message || 'Ownership could not be transferred.' };
...
export async function getSchoolHeadSetupChecklist(schoolId) {
    const { data, error } = await supabase.rpc('school_head_get_setup_checklist', { p_school_id: schoolId });
    if (error)
        throw new Error(error.message || 'School setup checklist could not be loaded.');
    const payload = record(data);
    if (payload['success'] !== true)
        throw new Error(stringValue(payload['error'], 'School setup checklist returned an invalid response.'));
    return {
        requested_modules: Array.isArray(payload['requested_modules']) ? payload['requested_modules'].filter((item) => typeof item === 'string') : ['core'],
        steps: Array.isArray(payload['steps']) ? payload['steps'].flatMap((item) => {
...
export async function updateSchoolHeadSetup(input) {
