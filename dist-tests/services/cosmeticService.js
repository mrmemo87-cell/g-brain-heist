 ...
 */
export const fetchNeonFrameOwners = async (userIds) => {
    if (!userIds.length) {
        return new Set();
    }
    try {
        ...
        const result = new Set((data || []).map((row) => row.id));
        return result;
    }
    catch (e) {
 ...
 * Queries from the users table column instead of inventory to avoid RLS restrictions.
 */
export const fetchFlickerThemeOwners = async (userIds) => {
    if (!userIds.length) {
        return new Set();
    }
    try {
 ...
 */
export const fetchNeonFrameOwnersFromInventory = async (userIds) => {
    if (!userIds.length) {
        return new Set();
    }
    try {
 ...
 */
export const fetchFlickerThemeOwnersFromInventory = async (userIds) => {
    if (!userIds.length) {
        return new Set();
    }
    try {
 ...
 */
export const fetchGlitchEffectOwners = async (userIds) => {
    if (!userIds.length) {
        return new Set();
    }
    try {
 ...
 */
export const fetchGlitchEffectOwnersFromInventory = async (userIds) => {
    if (!userIds.length) {
        return new Set();
    }
    try {
 ...
 */
export const fetchNeonFrameOwnersViaRPC = async (userIds) => {
    if (!userIds.length) {
        return new Set();
    }
    try {
