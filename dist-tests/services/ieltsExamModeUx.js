...
export const getIeltsExamLifecycleMeta = (state) => ({
    state,
    ...LIFECYCLE_META[state],
});
export const resolveIeltsExamLifecycleState = (rawStatus, startsAt, endsAt, nowMs = Date.now()) => {
...
export const resolveIeltsExamLifecycleMeta = (rawStatus, startsAt, endsAt, nowMs = Date.now()) => getIeltsExamLifecycleMeta(resolveIeltsExamLifecycleState(rawStatus, startsAt, endsAt, nowMs));
export const formatIeltsCountdown = (seconds) => {
...
export const getIeltsAttemptOperationalLabel = (status, hasConnectionIssue = false) => {
    if (hasConnectionIssue)
        return 'Possible connection issue';
    const normalized = (status ?? '').toLowerCase();
    if (!normalized || normalized === 'assigned' || normalized === 'not_started')
        return 'Not started';
...
export const isIeltsVoidedAttemptStatus = (status, reason) => (['void', 'voided'].includes(normalizeExamStatus(status)) || normalizeExamStatus(reason) === 'assignment_void');
export const resolveIeltsStudentExamSyncState = (attemptStatus, eventStatus, reason) => {
    if (isIeltsVoidedAttemptStatus(attemptStatus, reason))
        return 'voided';
    if (isIeltsTeacherSubmittedStatus(attemptStatus))
        return 'teacher_submitted';
    if (normalizeExamStatus(attemptStatus) === 'not_in_progress' || normalizeExamStatus(attemptStatus) === 'locked')
        return 'not_in_progress';
    if (isIeltsExamEventPaused(eventStatus, reason))
        return 'paused';
    if (['ended', 'closed', 'complete', 'completed'].includes(normalizeExamStatus(eventStatus)))
        return 'ended';
    return 'active';
};
export const getIeltsStudentExamSyncMessage = (state) => {
    if (state === 'teacher_submitted')
        return 'Your exam has been submitted by your teacher.';
    if (state === 'voided')
        return 'This attempt was voided by the teacher.';
    if (state === 'paused')
        return 'This exam is paused by the teacher.';
    if (state === 'not_in_progress')
        return 'Your exam is no longer in progress.';
    if (state === 'ended')
        return 'This IELTS exam is closed.';
    return null;
};
export const shouldIeltsAutosaveRun = (state) => state === 'active';
