# Brains Heist Documentation Hub

Welcome to the documentation bundle for **Brains Heist** – a light-mode, spy/heist themed adaptive learning platform built with Supabase, Next.js, TypeScript, and Tailwind. These pages are designed to onboard developers, teachers, and student agents quickly.

## Contents

| Audience | Document | Highlights |
| --- | --- | --- |
| Developers | [Project Overview](./overview.md) | Architecture, data flow, major UI concepts |
| Developers | [Local Setup Guide](./local-setup.md) | Environment variables, migrations, dev workflows |
| Developers | [Database Schema Reference](./database.md) | Tables, relationships, triggers |
| Developers | [RPC & API Reference](./api.md) | RPC names, parameters, request/response samples |
| Teachers (Handlers) | [Teacher Mission Builder Guide](./teacher-guide.md) | Creating subjects, topics, missions, analytics |
| Teachers & Admins | [Teacher & School Systems Blueprint](./teacher-systems.md) | Content flows, analytics dashboards, assignments, events |
| Students (Agents) | [Student Field Manual](./student-guide.md) | Missions, progress map, battles, scoring |
| Content Editors | [Biology Question Import Workflow](./BIOLOGY_QUESTION_IMPORT_WORKFLOW.md) | Chapter-based import from image PDFs with max 30 questions per test part |

Each guide can be copied directly into a docs site. Update these files whenever schema, RPCs, or gameplay rules evolve so that onboarding stays frictionless.
