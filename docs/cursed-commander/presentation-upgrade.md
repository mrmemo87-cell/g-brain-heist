...
### Mobile formation follow-up

Restored distinct horizontal positions for each formation lane below 900px; the desktop projection and canonical gameplay coordinates are unchanged. Numeric rows can wrap on narrow unit panels without overflowing. The actual Arena harness passed at 320, 375, 390, 430, 768, 1024, and 1440px with all six shells inside the board, no shell intersections, no horizontal document overflow, 11px numeric text, and no page errors. Phone screenshots were inspected. Typecheck, production build, and 12 focused cache, presentation, intro, and playback tests passed.
