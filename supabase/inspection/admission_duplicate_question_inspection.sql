...
WITH form_question_id_duplicates AS (
  SELECT f.form_code, bp.target_grade AS grade, bp.subject, fq.form_id, fq.question_id, COUNT(*) AS duplicate_count,
         ...
         array_agg(left(q.prompt, 160) ORDER BY fq.question_order) AS stems
  ...
  SELECT f.form_code, bp.target_grade AS grade, bp.subject, fq.form_id, adm_normalize_question_stem(q.prompt) AS normalized_stem,
  ...
  LEFT JOIN adm_blueprints bp ON bp.id = f.blueprint_id
  JOIN adm_questions q ON q.id = fq.question_id
  GROUP BY f.form_code, bp.target_grade, bp.subject, fq.form_id, adm_normalize_question_stem(q.prompt)
  HAVING COUNT(*) > 1
), official_bank_stem_duplicates AS (
  SELECT NULL::text AS form_code, q.grade_level AS grade, q.subject, NULL::uuid AS form_id,
         adm_normalize_question_stem(q.prompt) AS normalized_stem, COUNT(*) AS duplicate_count,
         array_agg(q.external_id ORDER BY q.external_id) AS external_ids,
         array_agg(left(q.prompt, 160) ORDER BY q.external_id) AS stems
  FROM adm_questions q
  WHERE q.is_official = true AND q.is_locked = true AND q.status = 'published'
  GROUP BY q.grade_level, q.subject, q.question_type, adm_normalize_question_stem(q.prompt)
  HAVING COUNT(*) > 1
)
