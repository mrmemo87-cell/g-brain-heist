...
CREATE OR REPLACE FUNCTION public.rpc_student_learning_catalog(p_subject_code text DEFAULT NULL::text, p_limit integer DEFAULT 20)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO ''
...
begin
  ...
  if v_teacher is not null then
    ...
    if v_teacher_school is not null and (
      exists(select 1 from private.teacher_current_teaching_groups(v_student,v_teacher_school))
      or exists (
        select 1
      ...
      select coalesce(g.academic_subject_name,g.school_subject_name)
      into v_teacher_subject_name
      from private.teacher_current_teaching_groups(v_student,v_teacher_school) g
      where (
        g.academic_subject_id::text = p_subject_code
        or lower(coalesce(g.academic_subject_code,'')) = lower(public.academic_normalize_subject_key(p_subject_code))
      ...
      if v_teacher_subject_name is null
         and not exists(select 1 from private.teacher_current_teaching_groups(v_student,v_teacher_school)) then
