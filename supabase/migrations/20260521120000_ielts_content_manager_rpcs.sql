...
begin
  ...
  return (
    ...
    from (
      select id::text, title, 'reading'::text as skill, is_active, level as difficulty, rs.est_band_min as band_min, rs.est_band_max as band_max, created_at,
        (is_active and exists (select 1 from public.ielts_reading_questions q where q.set_id = rs.id)) as ready_to_assign,
        (case when not is_active then array['Inactive']::text[] when not exists (select 1 from public.ielts_reading_questions q where q.set_id = rs.id) then array['Missing questions']::text[] else array[]::text[] end) as warnings
      from public.ielts_reading_sets rs
      union all
      select id::text, title, 'listening'::text, is_active, level, ls.est_band_min as band_min, ls.est_band_max as band_max, created_at,
      ...
      union all
      select id::text, 'Speaking Part ' || part::text as title, 'speaking'::text, is_active, 'part '||part::text, null::numeric, null::numeric, created_at, is_active, (case when not is_active then array['Inactive']::text[] else array[]::text[] end)
