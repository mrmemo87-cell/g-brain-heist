-- Prevent historical focus rows from triggering expensive question-bank lookups
-- before current academic/placement scoping is applied. This keeps the
-- intervention workspace fast and ensures only current-placement evidence can
-- produce a current support recommendation.
    ...
    select
      ...
      case
        ...
        when f.current_status = 'persistent' then format(
          ...
          f.skill, f.evidence_items, to_char(f.last_observed_at, 'DD Mon YYYY')
        )
...
comment on function public.rpc_teacher_student_intervention_intelligence(uuid, text) is
  'Current-placement intervention intelligence. Current academic/placement scope is applied before verified-question lookup so historical signals cannot slow or contaminate live recommendations.';
