-- Keep published teacher work visible to its assigned students, return valid
-- PostgREST entitlement errors, and stream programme-request queue changes.
...
begin
  ...
  -- These functions are already scoped to the signed-in student. Keep existing
  -- published academic work readable if a school plan changes; creation and
  -- submission remain entitlement-controlled below.
  if v_path = any(array[
  ...
  -- Teacher allocation is school administration, not academic assignment work.
  if v_path = any(array[
  ...
  if v_path = any(array[
    ...
    '/rpc/rpc_get_students_for_assignment','/rpc/rpc_submit_assignment_answer',
...
revoke all on function private.enforce_request_entitlement()
  from public;
grant execute on function private.enforce_request_entitlement()
...
begin
  if not exists (
    ...
    where pubname = 'supabase_realtime'
      ...
      and tablename = 'school_programme_access_requests'
  ) then
    alter publication supabase_realtime add table public.school_programme_access_requests;
  end if;
end;
$$;
