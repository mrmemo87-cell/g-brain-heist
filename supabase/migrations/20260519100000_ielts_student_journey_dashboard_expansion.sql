...
begin
  ...
  if to_regclass('public.ielts_practice_assignment_students') is not null then
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'due_at', a.due_at
    ) order by coalesce(a.due_at, s.assigned_at) asc nulls last), '[]'::jsonb)
  ...
  if to_regclass('public.ielts_teacher_reviews') is not null then
    select coalesce(jsonb_agg(jsonb_build_object(
      'review_id', r.id,
      'skill', r.skill,
      ...
      'feedback_preview', left(coalesce(r.feedback_summary, r.feedback, ''), 160),
      'reviewed_at', r.reviewed_at,
      'review_result_link', '/ielts/review-result/' || r.skill || '/' || r.attempt_id
    ) order by r.reviewed_at desc nulls last), '[]'::jsonb)
    into v_teacher_feedback
    from public.ielts_teacher_reviews r
    where r.student_id = v_student_id and coalesce(r.finalized, false) = true;
  end if;
  ...
  if to_regclass('public.ielts_exam_submissions') is not null and to_regclass('public.ielts_exam_attempts') is not null then
    if exists (select 1 from information_schema.columns where table_schema = 'public' and table_name = 'ielts_exam_submissions' and column_name = 'student_id') then
      select coalesce(jsonb_agg(jsonb_build_object(
        ...
        'result_status', case when s.grading_status is null or s.grading_status in ('pending', 'queued') then 'Submitted — results pending' else s.grading_status end
      ...
      where s.student_id = v_student_id and coalesce(a.status, '') in ('submitted', 'force_submitted');
    end if;
  end if;
