-- Unify school branding and viewer context across the Academic Progress Suite.
-- Also allow the active School Head to open individual student progress records in their own school.

create or replace function public.rpc_academic_progress_experience_context(p_student_id uuid default null)
returns jsonb
language plpgsql
stable
security definer
...
revoke all on function public.rpc_academic_progress_experience_context(uuid) from public, anon;
grant execute on function public.rpc_academic_progress_experience_context(uuid) to authenticated, service_role;
...
begin
  ...
  if p_subject is not null and v_is_teacher and not (v_is_self or v_is_school_admin or v_is_school_head)
  ...
  scoped_focus as (
    ...
    where s.student_id = v_student_id
      ...
      and (v_is_self or v_is_school_admin or v_is_school_head or lower(trim(s.subject)) = any(v_allowed_subjects))
  ...
  scoped_timeline as (
    ...
    where o.student_id = v_student_id
      ...
      and (v_is_self or v_is_school_admin or v_is_school_head or lower(trim(o.subject)) = any(v_allowed_subjects))
  ...
  select jsonb_build_object(
    ...
    'scope', jsonb_build_object(
      ...
      'date_to', p_date_to,
      'viewer', case when v_is_self then 'student' when v_is_school_head then 'school_head' when v_is_school_admin then 'school_admin' else 'teacher' end,
      'allowed_subjects', case when v_is_teacher and not (v_is_self or v_is_school_admin or v_is_school_head) then to_jsonb(v_allowed_subjects) else '[]'::jsonb end
    ),
