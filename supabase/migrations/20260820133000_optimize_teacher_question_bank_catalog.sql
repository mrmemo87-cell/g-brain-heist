...
begin
  ...
  with authorized_scopes as materialized (
    -- Allocated teachers: only their active class + subject + grade scopes.
    select distinct
    ...
    -- Existing fallback semantics for a school teacher with no active class
    -- allocations: browse the school's active curriculum catalogue.
    select
