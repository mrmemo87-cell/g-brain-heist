...
create or replace function public.student_learning_ingest_writing_attempt(
  p_attempt_id uuid,
  p_payload jsonb,
...
begin
  ...
  v_submission := trim(coalesce(v_payload->>'student_submission', ''));
  if v_submission = '' then return; end if;

  v_genre := coalesce(nullif(trim(v_payload->>'genre'), ''), nullif(trim(v_assessment->>'genre'), ''), 'writing');
  v_logical_key := coalesce(
    nullif(trim(v_payload->>'attempt_key'), ''), nullif(trim(v_payload->>'id'), ''),
    nullif(trim(v_payload->>'attempt_id'), ''), p_attempt_id::text
  );
  ...
  if coalesce(v_assessment->>'actual_word_count', '') ~ '^\d+$' then
    v_word_count := (v_assessment->>'actual_word_count')::integer;
  elsif coalesce(v_payload->>'actual_word_count', '') ~ '^\d+$' then
    v_word_count := (v_payload->>'actual_word_count')::integer;
  else
    v_word_count := coalesce(array_length(regexp_split_to_array(v_submission, '\s+'), 1), 0);
  end if;
  if coalesce(v_assessment->>'target_word_count', '') ~ '^\d+$' then
    v_target_words := greatest(1, (v_assessment->>'target_word_count')::integer);
  elsif coalesce(v_payload->>'target_word_count', '') ~ '^\d+$' then
    v_target_words := greatest(1, (v_payload->>'target_word_count')::integer);
  end if;
  ...
  if coalesce(v_assessment->>'total_score', '') ~ '^\d+(\.\d+)?$' then
    v_total_score := (v_assessment->>'total_score')::numeric;
  end if;

  -- Synchronise the logical attempt. Payload updates must not leave stale weakness rows behind.
  ...
  v_tags := case
    when jsonb_typeof(v_payload->'feedback_weakness_tags') = 'array' then v_payload->'feedback_weakness_tags'
    when jsonb_typeof(v_assessment->'weakness_tags') = 'array' then v_assessment->'weakness_tags'
    else '[]'::jsonb
  end;
  v_counts := case when jsonb_typeof(v_payload->'feedback_weakness_tag_counts') = 'object'
    then v_payload->'feedback_weakness_tag_counts' else '{}'::jsonb end;

  -- Aggregate raw tags into stable academic skills so one essay cannot double-count related labels.
  for v_group in
    with raw_tags as (
      select lower(trim(t.value)) as raw_tag,
        case when coalesce(v_counts->>t.value, '') ~ '^\d+$' then greatest(1, (v_counts->>t.value)::integer) else 1 end as tag_count
      from jsonb_array_elements_text(v_tags) t(value)
      where nullif(trim(t.value), '') is not null
    )
    select public.student_learning_canonical_writing_skill(raw_tag) as skill,
      public.student_learning_writing_dimension(raw_tag) as dimension,
      sum(tag_count)::integer as occurrence_count,
      jsonb_agg(raw_tag order by raw_tag) as raw_tags
    from raw_tags
    group by 1, 2
  loop
    ...
    insert into public.student_learning_observations (
      ...
      v_school_id, v_student_id, 'English', 'Writing', v_group.skill, v_genre, v_skill_key,
      'focus', 'writing_attempt', p_attempt_id, v_source_key, v_observed_at,
      case when v_total_score is null then null else greatest(0, least(100, v_total_score * 5)) end,
      v_group.occurrence_count,
      jsonb_build_object(
        'writing_signal', 'weakness_tag', 'logical_attempt_key', v_logical_key,
        'attempt_id', p_attempt_id, 'genre', v_genre, 'raw_tags', v_group.raw_tags,
        'rubric_dimension', v_group.dimension, 'word_count', v_word_count,
        'target_word_count', v_target_words, 'total_score', v_total_score,
        'evidence_quality', v_quality
      ), true, v_quality, v_contributes
    );
  end loop;

  -- Rubric dimensions are independent longitudinal evidence, including positive performance.
  if jsonb_typeof(v_assessment->'subscores') = 'object' then
    for v_subscore_key, v_subscore_json in select key, value from jsonb_each(v_assessment->'subscores') loop
      if jsonb_typeof(v_subscore_json) = 'number' then
        v_subscore_value := (v_subscore_json #>> '{}')::numeric;
      elsif jsonb_typeof(v_subscore_json) = 'string' and (v_subscore_json #>> '{}') ~ '^\d+(\.\d+)?$' then
        v_subscore_value := (v_subscore_json #>> '{}')::numeric;
      else
        continue;
      end if;
      v_kind := case when v_subscore_value <= 2 then 'focus' when v_subscore_value >= 4 then 'strength' else 'developing' end;
      v_group.skill := replace(initcap(replace(v_subscore_key, '_', ' ')), ' And ', ' & ');
      v_skill_key := public.student_learning_build_skill_key('English', 'Writing rubric', v_group.skill, null);
      ...
      insert into public.student_learning_observations (
        ...
        v_school_id, v_student_id, 'English', 'Writing rubric', v_group.skill, v_genre, v_skill_key,
        v_kind, 'writing_attempt', p_attempt_id, v_source_key, v_observed_at,
        greatest(0, least(100, v_subscore_value * 20)), 1,
        jsonb_build_object(
          'writing_signal', 'rubric', 'logical_attempt_key', v_logical_key,
          'attempt_id', p_attempt_id, 'genre', v_genre, 'rubric_dimension', v_subscore_key,
          'rubric_score', v_subscore_value, 'rubric_max', 5, 'word_count', v_word_count,
          'target_word_count', v_target_words, 'total_score', v_total_score,
          'evidence_quality', v_quality
        ), true, v_quality, v_contributes
      );
  ...
  -- Recovery evidence is conservative: no weakness tag + strong-quality writing + related rubric >= 4.
  if v_quality = 'strong' and jsonb_typeof(v_assessment->'subscores') = 'object' then
    for v_recovery in
      select distinct o.skill, o.skill_key, o.evidence->>'rubric_dimension' as dimension
      from public.student_learning_observations o
      where o.student_id = v_student_id
        and o.subject = 'English' and o.topic = 'Writing'
        and o.source_type = 'writing_attempt'
        and o.observation_type = 'focus'
        and o.evidence->>'writing_signal' = 'weakness_tag'
        and o.observed_at < v_observed_at
    loop
      if exists (
        select 1 from jsonb_array_elements_text(v_tags) t(value)
        where public.student_learning_canonical_writing_skill(t.value) = v_recovery.skill
      ) then continue; end if;
      v_subscore_json := v_assessment->'subscores'->v_recovery.dimension;
      if v_subscore_json is null then continue; end if;
      ...
      insert into public.student_learning_observations (
        ...
        v_school_id, v_student_id, 'English', 'Writing', v_recovery.skill, v_genre, v_recovery.skill_key,
        'strength', 'writing_attempt', p_attempt_id, v_source_key, v_observed_at,
        greatest(0, least(100, v_dimension_score * 20)), 1,
        jsonb_build_object(
          'writing_signal', 'recovery', 'logical_attempt_key', v_logical_key,
          'attempt_id', p_attempt_id, 'genre', v_genre,
          'rubric_dimension', v_recovery.dimension, 'rubric_score', v_dimension_score,
          'reason', 'Related weakness absent and rubric dimension scored 4 or 5',
          'word_count', v_word_count, 'target_word_count', v_target_words,
          'evidence_quality', v_quality
        ), true, 'strong', true
      );
    end loop;
  ...
  -- Refresh any skill that may have disappeared after synchronising an edited logical attempt.
  foreach v_old_key in array v_old_keys loop
    perform public.student_learning_refresh_focus_state(v_student_id, v_old_key);
  end loop;
end;
$$;
  ...
  for r in
    with ranked as (
      select w.id, w.payload, w.created_at,
        row_number() over (
          partition by w.payload->>'student_id',
            coalesce(w.payload->>'genre', w.payload->'assessment'->>'genre', 'writing'),
...
-- Writing rows were rebuilt; rebuild the shared projection so removed duplicate/noisy evidence disappears.
