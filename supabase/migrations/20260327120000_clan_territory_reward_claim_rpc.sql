-- Canonical, idempotent clan territory reward claims.
...
begin
  if coalesce(auth.role(), '') <> 'service_role' then
    raise exception 'rpc_claim_clan_territory_reward requires service_role execution';
  end if;
  ...
  if v_arena_mode = 'open' then
    v_caps := row(1500, 5000, 10);
  else
    v_caps := row(2500, 10000, 30);
  end if;

  if coalesce(p_xp_delta, 0) > (v_caps).f1
     or coalesce(p_coins_delta, 0) > (v_caps).f2
     or coalesce(p_gemstones_delta, 0) > (v_caps).f3 then
    raise exception 'Reward delta exceeds safety cap for arena mode %', v_arena_mode;
  end if;

  v_idempotency_key := coalesce(
    nullif(trim(coalesce(p_idempotency_key, '')), ''),
    'clan-territory:' || p_room_id || ':' || p_user_id::text
  );
...
revoke execute on function public.rpc_claim_clan_territory_reward(uuid, text, text, int, int, int, text, text) from public;
revoke execute on function public.rpc_claim_clan_territory_reward(uuid, text, text, int, int, int, text, text) from anon;
revoke execute on function public.rpc_claim_clan_territory_reward(uuid, text, text, int, int, int, text, text) from authenticated;
grant execute on function public.rpc_claim_clan_territory_reward(uuid, text, text, int, int, int, text, text) to service_role;
