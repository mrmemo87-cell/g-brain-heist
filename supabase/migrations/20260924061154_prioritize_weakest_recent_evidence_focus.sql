-- Live-history follow-up: carry the most relevant recent Evidence Focus into intervention recommendations.
...
begin
  ...
  recommendations as (
    select
      ...
      case
        when f.days_since_evidence >= 60 then format('%s was previously identified as %s, but the latest qualifying evidence is %s days old. Reassess before assuming the difficulty is still current.', coalesce(f.evidence_focus_name, f.subskill, f.skill), replace(f.current_status, '_', ' '), f.days_since_evidence)
        when f.skill_key like 'diagnostic:%' and f.evidence_focus_code is not null and f.available_exact_questions = 0 then
          format('%s is the current precise focus, but no independently verified practice question currently targets that exact Evidence Focus. Related questions are shown for teacher context only and will not be auto-assigned.', coalesce(f.evidence_focus_name, f.subskill, f.skill))
