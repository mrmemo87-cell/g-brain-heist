-- Prefer concept-level subskill uniqueness during official admission form generation.
-- Keeps exact blueprint buckets and existing dedupe filters, but fills each bucket in two passes:
-- unique-subskill first, then controlled shortage fallback.
...
BEGIN
    ...
    IF v_pool_ids IS NULL THEN RETURN jsonb_build_object('success', false, 'error', 'No official Brains Heist admission question pools match this blueprint', 'debug_reason', 'No matching official managed pools'); END IF;
    ...
    FOR v_dist_key, v_dist_val IN SELECT * FROM jsonb_each(v_bp.question_distribution) LOOP
        FOR v_diff_key, v_diff_count IN SELECT key, value::int FROM jsonb_each_text(v_dist_val) LOOP
            ...
            -- appeared anywhere in this form. This prevents semantic duplicate concepts
            -- across blueprint buckets when enough unique concepts are available.
            INSERT INTO adm_selected_questions_tmp(question_id, normalized_stem, strand, subskill, question_type, difficulty)
                ...
                WHERE q.pool_id = ANY(v_pool_ids) AND q.question_type = v_dist_key AND q.difficulty = v_diff_key AND q.status = 'published'
                  ...
                  AND NOT EXISTS (
                      ...
                      WHERE s.question_id = q.id
                         ...
                         OR s.subskill = COALESCE(NULLIF(q.subskill, ''), NULLIF(q.diagnostic_skill, ''), NULLIF(q.topic, ''), adm_normalize_question_stem(q.stem))
                  )
                ORDER BY adm_normalize_question_stem(q.stem), RANDOM()
            ),
            ranked AS (
            ...
            SELECT question_id, normalized_stem, strand, subskill, question_type, difficulty
            FROM ranked
            ...
            IF v_remaining_count > 0 THEN
                ...
                ranked AS (
                    SELECT
                        bc.*,
                        COALESCE((SELECT COUNT(*) FROM adm_selected_questions_tmp s WHERE s.subskill = bc.subskill), 0) AS existing_subskill_count,
                        COALESCE((SELECT COUNT(*) FROM adm_selected_questions_tmp s WHERE s.strand = bc.strand), 0) AS existing_strand_count,
                        ROW_NUMBER() OVER (PARTITION BY bc.subskill ORDER BY bc.random_order) AS subskill_round,
                        ROW_NUMBER() OVER (PARTITION BY bc.strand ORDER BY bc.random_order) AS strand_round
                    FROM base_candidates bc
                )
                SELECT question_id, normalized_stem, strand, subskill, question_type, difficulty
