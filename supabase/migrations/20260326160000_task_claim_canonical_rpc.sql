-- Canonical transactional task-claim flow: eligibility + idempotency + reward write + activity log.
...
begin
  ...
  v_event_id := coalesce(nullif(trim(coalesce(p_event_id, '')), ''), 'task:' || p_task_id || ':' || v_period_key);
  v_idempotency_key := coalesce(
    nullif(trim(coalesce(p_idempotency_key, '')), ''),
    'task-claim:' || v_user_id::text || ':' || v_event_id
  );
