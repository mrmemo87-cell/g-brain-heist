-- Fix batch-level canonical subskill uniqueness during official admission form generation.
-- First pass limits each INSERT batch to one row per canonical subskill; fallback runs only for shortages.
