-- Parenthesize JSON extraction before text concatenation in the formal student
-- prompt rotation expression. Without the outer parentheses PostgreSQL can
-- parse the expression as text ->> 'id', causing the student prompt RPC to fail
-- and leaving the Writing Hub response editor disabled because prompt_id stays null.
...
comment on function public.rpc_bh_writing_student_prompt(integer, text, text) is
  'Returns an approved Cambridge Writing Hub prompt for the authenticated student authoritative grade. JSON extraction is parenthesized before prompt-rotation text concatenation.';
