...
begin
  ...
  if v_caller is not null then
    select lower(trim(a.email)) into v_current_email
    from auth.users a
    where a.id = v_caller;
    v_email_matches := coalesce(v_current_email = v_inv.invited_email, false);
  end if;
  ...
  return jsonb_build_object(
    ...
    'email_matches_current_account', case when v_caller is null then null else v_email_matches end
  );
end;
$function$;
