-- Generate admission forms from the latest managed bank version for the normalized blueprint grade and subject.
...
BEGIN
    ...
    v_managed_subject := CASE lower(btrim(v_bp.subject))
        ...
        ELSE NULL
    END;
    IF v_blueprint_grade IS NULL OR v_managed_subject IS NULL THEN
    ...
    WHERE qp.is_active = true AND qp.is_official = true AND qp.is_locked = true AND qp.school_id IS NULL
      AND CASE lower(btrim(qp.subject))
            WHEN 'math' THEN 'maths'
            WHEN 'maths' THEN 'maths'
            WHEN 'mathematics' THEN 'maths'
            WHEN 'english' THEN 'english'
      ...
      AND qp.content_version ~ ('^adm-bank-v[0-9]+-g' || v_blueprint_grade::text || '-' || v_managed_subject || '$')
    ...
    IF v_bp.pool_id IS NOT NULL THEN
        ...
        WHERE id = v_bp.pool_id AND is_active = true AND is_official = true AND is_locked = true AND school_id IS NULL
          AND CASE lower(btrim(subject))
                ...
                WHEN 'science' THEN 'science'
                ELSE NULL
              END = v_managed_subject AND (stage = v_bp.target_stage OR stage_level = v_bp.target_stage OR v_bp.target_stage IS NULL)
    ...
    ELSE
        ...
        WHERE is_official = true AND is_locked = true AND is_active = true AND school_id IS NULL AND CASE lower(btrim(subject))
                WHEN 'math' THEN 'maths'
                WHEN 'maths' THEN 'maths'
                WHEN 'mathematics' THEN 'maths'
                WHEN 'english' THEN 'english'
