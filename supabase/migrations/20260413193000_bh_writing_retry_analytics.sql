-- Add retry-cycle analytics visibility to teacher-scoped Writing analytics RPC.
  ...
  return (
    ...
    attempts as (
      select
        ...
        coalesce(array(
          select jsonb_array_elements_text(coalesce(a.payload->'assessment'->'weakness_tags', '[]'::jsonb))
        ), '{}'::text[]) as weakness_tags,
        a.created_at
    ...
    repeated_cycle_tags as (
      ...
      from (
        ...
        cross join lateral unnest(coalesce(ce.first_tags, '{}'::text[])) t(tag)
        where t.tag = any(coalesce(ce.latest_tags, '{}'::text[]))
      ) rt
      group by tag
      order by cnt desc
      limit 8
              ...
              'recurring_mistake_tags', coalesce((
                ...
                from (
                  ...
                  from (
                    ...
                    from cycles_enriched ce
                    cross join lateral unnest(coalesce(ce.first_tags, '{}'::text[])) t(tag)
                    where ce.student_id = sr.student_id and t.tag = any(coalesce(ce.latest_tags, '{}'::text[]))
                    group by t.tag
                    order by cnt desc
                    limit 4
                  ) top_tags
