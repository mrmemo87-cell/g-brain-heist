...
BEGIN
    ...
    IF v_candidate.id IS NULL OR v_form.id IS NULL OR v_form.school_id <> v_attempt.school_id THEN
        RETURN jsonb_build_object('success', false, 'error', 'Report data unavailable');
    END IF;
