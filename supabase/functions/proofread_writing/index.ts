...
serve(async (req) => {
  ...
  try {
    ...
    if (!supabaseUrl || !serviceKey || !openAiKey) {
      console.error("Missing env vars:", { supabaseUrl: !!supabaseUrl, serviceKey: !!serviceKey, openAiKey: !!openAiKey });
    ...
    async function proofread(text: string, isPart1: boolean) {
      ...
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system",
            content: `You are an English teacher marking ${testType}. Respond with JSON:
{
  "feedback": "Encouraging feedback (2-3 sentences)",
