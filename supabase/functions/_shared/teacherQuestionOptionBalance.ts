export type BalanceableTeacherQuestion = {
...
const shuffledBalancedTargets = (
  ...
  const targets = Array.from(
    ...
    (_, index) => (index + offset) % optionCount,
  );
...
const findCorrectOptionIndex = (question: BalanceableTeacherQuestion) => {
 ...
 */
export const balanceGeneratedMultipleChoiceOptions = <T extends BalanceableTeacherQuestion>(
  ...
  for (const [optionCount, questionIndexes] of groups.entries()) {
    ...
    questionIndexes.forEach((questionIndex, groupIndex) => {
      ...
      if (currentCorrectIndex < 0 || currentCorrectIndex === targetCorrectIndex) return;
...
export const generatedMultipleChoiceAnswerPositionCounts = (
  questions: readonly BalanceableTeacherQuestion[],
) => {
  const groups = new Map<number, number[]>();

  for (const question of questions) {
    if (question.candidate_origin !== 'ai_generated_from_source') continue;
    if (question.question_type !== 'multiple_choice') continue;
    if (question.options.length < 2 || question.options.length > 6) continue;
...
export const hasBalancedGeneratedMultipleChoiceAnswers = (
  questions: readonly BalanceableTeacherQuestion[],
) => {
