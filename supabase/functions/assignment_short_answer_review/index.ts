...
const judgeAnswer = async (snapshot: Record<string, unknown>, studentAnswer: string): Promise<ReviewDecision> => {
  ...
  const acceptedAnswers = Array.isArray(snapshot.accepted_answers)
    ? snapshot.accepted_answers.map(String).map((value) => value.trim()).filter(Boolean).slice(0, 12)
    : [];
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    try {
      const response = await fetch("https://api.openai.com/v1/responses", {
        ...
        body: JSON.stringify({
          ...
          text: {
            format: {
              type: "json_schema",
              name: "short_answer_review",
              strict: true,
              schema: {
                type: "object",
                additionalProperties: false,
                required: ["is_correct", "confidence", "rationale"],
                properties: {
      ...
      if (!response.ok) {
        ...
        throw new Error(`AI review failed (${response.status}): ${message}`);
      }
      ...
      if (typeof parsed.is_correct !== "boolean" || !Number.isFinite(Number(parsed.confidence))) {
        throw new Error("AI review returned an invalid decision.");
      }
      return {
...
const processReview = async (answerId: string, assignmentId: string, questionId: string) => {
  const { data: claimed, error: claimError } = await admin
    .from("student_assignment_answers")
    .update({ grading_status: "reviewing", review_started_at: new Date().toISOString(), review_error: null })
    .eq("id", answerId)
    .eq("grading_status", "under_review")
  ...
  try {
    const { data: assignmentQuestion, error: snapshotError } = await admin
      .from("assignment_questions")
      .select("question_snapshot")
      .eq("assignment_id", assignmentId)
      .eq("question_id", questionId)
      .single();
    if (snapshotError || !assignmentQuestion?.question_snapshot) {
      throw new Error(snapshotError?.message || "Assignment question snapshot not found.");
    }

    const snapshot = assignmentQuestion.question_snapshot as Record<string, unknown>;
    if (String(snapshot.question_type || "") !== "short_answer") {
      throw new Error("Semantic review is restricted to short-answer questions.");
    }

    const decision = await judgeAnswer(snapshot, String(claimed.student_answer || ""));
    const { error: applyError } = await admin.rpc("rpc_apply_short_answer_review", {
      p_answer_id: answerId,
      p_is_correct: decision.is_correct,
    ...
    await admin
      .from("student_assignment_answers")
      .update({
        grading_status: "under_review",
        review_started_at: null,
        review_error: message.slice(0, 2000),
      })
      .eq("id", answerId)
      .eq("grading_status", "reviewing");
...
Deno.serve(async (req) => {
  ...
  if (!isUuid(assignmentId) || !isUuid(questionId)) {
    return jsonResponse(400, { error: "invalid_review_target" });
  }
  ...
  if (!answer || answer.grading_status === "graded") {
    return jsonResponse(200, { accepted: false, alreadyResolved: true });
  }
  if (answer.grading_status === "reviewing") {
    return jsonResponse(202, { accepted: true, alreadyProcessing: true });
  }

  EdgeRuntime.waitUntil(processReview(answer.id, assignmentId, questionId));
