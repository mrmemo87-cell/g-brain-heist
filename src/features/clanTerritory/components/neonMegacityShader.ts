...
void main(){
vec4 art=texture(uArt,vUv);vec3 original=art.rgb;vec3 c=original;float idf=zoneAt(vUv);int id=int(floor(idf+0.5));if(id<1||id>10){fragColor=vec4(c,1.0);return;}
...
float emissive=clamp(chromaLight*0.90 + brightLight*0.42,0.0,1.0);emissive*=1.0-whiteLike*0.96;
float materialMask=smoothstep(0.16,0.58,sat)*smoothstep(0.07,0.36,lum)*(1.0-smoothstep(0.42,0.76,lum))*0.26;
float neutralLum=clamp(lum*1.04,0.0,1.0);vec3 neutralLight=vec3(neutralLum*0.79,neutralLum*0.88,neutralLum);
float coverage=uCoverage[id];float border=boundaryBand(idf,vUv);
if(coverage>0.001){float cluster=hashCluster(vUv,float(id));if(cluster<coverage){float primary=uPrimaryFrac[id];float secondary=uSecondaryFrac[id];float ownerHash=hash21(floor(vUv*vec2(86.0,49.0))+vec2(float(id)*3.1,float(id)*11.7));if(uContested[id]>0.5&&secondary>0.001){float flow=sin(vUv.x*24.0+vUv.y*17.0+uTime*1.8+ownerHash*6.0)*0.035;primary=clamp(primary+flow,0.05,0.95);}vec3 clanColor=uPrimaryColor[id];if(secondary>0.001&&ownerHash>primary){clanColor=uSecondaryColor[id];}vec3 pureClanLight=hueReplace(clanColor,clamp(lum*1.16,0.0,0.96));float lightStrength=emissive*mix(0.60,0.96,smoothstep(0.24,0.82,emissive));vec3 lightRelit=mix(original,pureClanLight,lightStrength);vec3 clanMaterial=hueReplace(clanColor,clamp(lum*0.82,0.0,0.68));vec3 relit=mix(lightRelit,clanMaterial,materialMask);float seamKeep=1.0-border*0.76;c=mix(original,relit,seamKeep);}else{c=mix(original,neutralLight,emissive*0.42);}float age=uTime-uCaptureTime[id];if(age>=0.0&&age<1.45){float d=distance(vUv,uCenters[id]);float radius=age*0.39;float ring=1.0-smoothstep(0.0,0.022,abs(d-radius));vec3 pulseHue=hueReplace(uPrimaryColor[id],clamp(lum*1.30+0.18,0.22,1.0));c+=pulseHue*ring*(1.0-age/1.45)*(0.15+emissive*0.68);}}else{c=mix(original,neutralLight,emissive*0.24);}
if(id==uHovered){c+=vec3(0.18,0.74,0.92)*border*0.055;}float selAge=uTime-uSelectAt;if(id==uSelected&&selAge>=0.0&&selAge<0.85){float d=distance(vUv,uCenters[id]);float radius=selAge*0.35;float ring=1.0-smoothstep(0.0,0.017,abs(d-radius));c+=vec3(1.0,0.76,0.10)*ring*(1.0-selAge/0.85)*0.54;}
