export interface SchoolClassGradeOption {
...
export const normalizeConfiguredGrade = (value: unknown): string => (
 ...
 */
export const getConfiguredSchoolGrades = (
  classes: SchoolClassGradeOption[],
): string[] => Array.from(new Set(
  classes
    .map((schoolClass) => normalizeConfiguredGrade(schoolClass.grade_level))
    .filter(Boolean),
)).sort((left, right) => gradeCollator.compare(left, right));

export const classMatchesConfiguredGrade = (
  schoolClass: SchoolClassGradeOption,
  grade: string,
): boolean => normalizeConfiguredGrade(schoolClass.grade_level) === normalizeConfiguredGrade(grade);
