...
const Phrase = ({ children }: { children: string }) => <div className="signal-phrase" aria-label={children}><div className="signal-phrase__track" aria-hidden="true"><span>{children}</span></div></div>;

const WORLD_PATH = 'M505 80 C760 380 220 710 505 1060 S800 1600 555 2050 S210 2590 455 3100 S810 3610 585 4160 S225 4680 490 5200 S800 5740 535 6300 S190 6900 500 7480 S830 8090 560 8700 S190 9300 485 9940 S790 10700 500 11520';
...
const PresentationPage = () => {
  ...
  useGSAP(() => {
    ...
    const signal = root.querySelector<HTMLElement>('[data-signal-orb]');
    const worldSvg = root.querySelector<SVGSVGElement>('.world-line');
    ...
    mm.add('(prefers-reduced-motion: no-preference)', () => {
      ...
      if (proofJourney && proofTrack && desktop) {
        ...
        proofPanels.forEach((panel, index) => {
          if (index > 0) proofTimeline.to(proofTrack, { x: () => -index * window.innerWidth, duration: .58, ease: 'none' });
          const phraseTrack = panel.querySelector<HTMLElement>('.signal-phrase__track');
          if (phraseTrack) {
            proofTimeline.fromTo(phraseTrack,
              { x: () => window.innerWidth + 48 },
              { x: () => -phraseTrack.scrollWidth - 48, duration: 1.22, ease: 'none' }
            );
          }
          if (index === 2) {
  ...
  return <main className="journey" ref={rootRef}>
    <div className="journey-atmosphere" aria-hidden="true"><i/><i/><i/></div><WorldLine/><GooeyOrb className="world-signal"/>
