export type PlacementLabel =
  | 'Ready for target grade'
  | 'Accept with English support'
  | 'Accept with Maths support'
  | 'Interview recommended'
  | 'Consider lower grade placement'
  | 'Not ready yet';
...
export interface DiagnosticBreakdownRow {
  key: string;
  label: string;
  subject: string;
  skill: string;
...
export interface PlacementRecommendation {
  ...
  mathsPercentage: number | null;
  sciencePercentage?: number | null;
  currentSubject?: 'english' | 'math' | 'science' | 'unknown';
  isPackageReport?: boolean;
  strengths: string[];
  weakAreas: string[];
}
...
export const deriveAdmissionSubject = (subject?: string | null, formCode?: string | null, contentVersion?: string | null) => {
  ...
  if (raw.includes('sci_') || raw.includes('biology') || raw.includes('physics') || raw.includes('chem')) return 'science';
  return 'unknown';
};
...
export function calculateSubjectReadiness(answers: DiagnosticAnswer[], subject: 'english' | 'math' | 'science'): number | null {
...
export function calculatePlacementRecommendation(profile: AcademicProfile = {}, answers: DiagnosticAnswer[] = [], fallbackPercentage: number | null = null): PlacementRecommendation {
  ...
  let label: PlacementLabel = 'Interview recommended';
  if (e != null && m != null) {
    if (e >= 70 && m >= 70) label = 'Ready for target grade';
    else if (m >= 70 && e >= 40 && e < 60) label = 'Accept with English support';
    else if (e >= 70 && m >= 40 && m < 60) label = 'Accept with Maths support';
    else if (e < 35 && m < 35) label = (e < 25 && m < 25) ? 'Not ready yet' : 'Consider lower grade placement';
    else if (e >= 50 && m >= 50) label = 'Interview recommended';
    else label = 'Interview recommended';
  } else if (e != null || m != null) {
    const only = e ?? m ?? 0;
    label = only >= 70 ? 'Interview recommended' : only < 35 ? 'Consider lower grade placement' : 'Interview recommended';
  }
  if (mismatch && label === 'Ready for target grade') label = 'Interview recommended';
...
function buildReasons(e: number | null, m: number | null, s: number | null, currentSubject: string, isPackageReport: boolean, mismatch: boolean, profile: AcademicProfile, strengths: string[], weakAreas: string[]) {
  const reasons: string[] = [];
  const readiness = { english: e, math: m, science: s } as const;
  if (!isPackageReport && currentSubject in readiness) {
    const value = readiness[currentSubject as keyof typeof readiness];
    reasons.push(`${title(currentSubject)} readiness is ${value ?? 'not available yet'}${value != null ? '%' : ''}.`);
  } else {
    if (e != null) reasons.push(`English readiness is ${e}%.`); else reasons.push('English readiness is not available yet.');
    if (m != null) reasons.push(`Maths readiness is ${m}%.`); else reasons.push('Maths readiness is not available yet.');
...
function nextAction(label: PlacementLabel, interviewFlag: boolean) {
  if (interviewFlag) return 'Schedule an admissions interview and review school records.';
  if (label.includes('English support')) return 'Offer a place with an English support plan.';
  if (label.includes('Maths support')) return 'Offer a place with a Maths support plan.';
  if (label === 'Ready for target grade') return 'Proceed with target-grade admission review.';
