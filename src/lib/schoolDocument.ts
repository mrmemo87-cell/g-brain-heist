...
export type SchoolDocumentAudience = 'family' | 'student' | 'teacher' | 'internal';
export type SchoolDocumentStatus = 'draft' | 'final';
...
export interface SchoolDocumentMeta {
  ...
  audience: SchoolDocumentAudience;
  status: SchoolDocumentStatus;
  confidentiality: SchoolDocumentConfidentiality;
  generatedAt: string;
  generatedBy?: string;
  academicYear?: string;
  term?: string;
...
export interface SchoolDocumentOptions {
  meta: SchoolDocumentMeta;
  bodyHtml: string;
  orientation?: SchoolDocumentOrientation;
  paper?: SchoolDocumentPaper;
  fileName?: string;
  inkSaver?: boolean;
  persistPayload?: boolean;
}

const AUDIENCE_LABELS: Record<SchoolDocumentAudience, string> = {
  family: 'Family copy',
...
const CONFIDENTIALITY_LABELS: Record<SchoolDocumentConfidentiality, string> = {
...
export const escapeSchoolDocumentHtml = (value: unknown) => String(value ?? '')
  .replace(/&/g, '&amp;')
  .replace(/</g, '&lt;')
  .replace(/>/g, '&gt;')
  .replace(/"/g, '&quot;')
...
export const schoolDocumentFileName = (...parts: Array<string | null | undefined>) => {
  const value = parts
    .filter(Boolean)
...
const renderMetaItem = (label: string, value?: string | null) => value
...
const documentCss = `
  ...
  *{box-sizing:border-box}
  ...
  .school-document__body h2{margin:7mm 0 2.5mm;padding-bottom:1.5mm;border-bottom:1px solid var(--doc-line);font-size:15px;color:var(--doc-accent-dark);break-after:avoid}.school-document__body h3{margin:4mm 0 2mm;font-size:12px;break-after:avoid}.school-document__body p{margin:0 0 3mm}.school-document__body ul,.school-document__body ol{margin:2mm 0 4mm;padding-left:6mm}.school-document__body li{margin-bottom:1mm}.school-document__body .document-callout{border-left:1.2mm solid var(--doc-accent);border-radius:1.5mm;background:#eef4ff;padding:3mm 4mm;margin:3mm 0}.school-document__body .document-callout--warning{border-color:#e0a100;background:#fff8e5}.school-document__body .document-callout--private{border-color:#b42318;background:#fff3f2}.school-document__body .document-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:3mm}.school-document__body .document-card{border:1px solid var(--doc-line);border-radius:2mm;padding:3.5mm;break-inside:avoid-page;page-break-inside:avoid}.school-document__body .document-card strong{display:block;margin-bottom:1mm}.school-document__body table{width:100%;border-collapse:collapse;margin:2mm 0 5mm;font-size:8.5px}.school-document__body thead{display:table-header-group}.school-document__body tr{break-inside:avoid-page;page-break-inside:avoid}.school-document__body th{background:var(--doc-accent-dark);color:#fff;text-align:left;font-size:7.5px;text-transform:uppercase;letter-spacing:.04em}.school-document__body th,.school-document__body td{border:1px solid var(--doc-line);padding:2mm;vertical-align:top}.school-document__body tbody tr:nth-child(even){background:var(--doc-soft)}.school-document__body .document-page-break{break-before:page}.school-document__body .document-appendix{break-before:page}.school-document__body .document-signatures{display:grid;grid-template-columns:1fr 1fr;gap:12mm;margin-top:12mm}.school-document__body .document-signature{border-top:1px solid var(--doc-ink);padding-top:2mm;color:var(--doc-muted);font-size:8px}
...
export const renderSchoolDocumentHtml = ({ meta, bodyHtml, orientation = 'portrait', paper = 'A4', fileName, inkSaver = false }: SchoolDocumentOptions) => {
...
export const registerSchoolDocumentRecord = async (options: SchoolDocumentOptions) => {
  if (options.meta.schoolId) {
    ...
    await import('../../services/supabaseClient').then(({ supabase }) => supabase
      ...
      .then(({ error }) => {
        if (error && import.meta.env.DEV) console.warn('School document audit record was not saved', error.message);
      }))
      .catch((error: unknown) => {
        if (import.meta.env.DEV) console.warn('School document registry unavailable', error);
      });
  }
};
