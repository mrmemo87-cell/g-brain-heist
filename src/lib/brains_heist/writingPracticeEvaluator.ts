import { RepeatedErrorMemory, SupportedGenre, WeaknessTag, WritingAssessmentResult } from './writingAssessment';
import { DailyWritingTask } from './writingTaskGenerator';
...
export interface PracticeCriterionResult {
  criterion: string;
  met: boolean;
  comment: string;
}

export interface WritingPracticeEvaluationResult {
  day_number: number;
  task_type: string;
  task_mode: string;
  completion_status: CompletionStatus;
  target_skill_score: number;
  success_criteria_results: PracticeCriterionResult[];
  detected_weakness_tags: WeaknessTag[];
  detected_improvement_tags: WeaknessTag[];
  word_count_result: {
    expected_min: number;
    expected_max: number;
    actual: number;
  ...
  student_friendly_feedback: string;
  memory_update_ready: true;
  recommended_next_action: RecommendedNextAction;
}

export interface EvaluateWritingPracticeInput {
...
const expectedRange = (expectedWordCount: number): { min: number; max: number } => ({
  min: Math.max(1, Math.floor(expectedWordCount * 0.9)),
  max: Math.ceil(expectedWordCount * 1.1),
});

const hasFormalTone = (text: string): boolean => /\btherefore|moreover|in conclusion|according to\b/i.test(text);
const hasInformalTone = (text: string): boolean => /\bhey|cool|awesome|gonna|wanna\b/i.test(text);

const tagStillPresent = (tag: WeaknessTag, submission: string, genre: SupportedGenre): boolean => {
  const text = submission.trim();
  if (tag === 'tense_error') return /\b(?:go|eat|play)\s+yesterday\b/i.test(text);
  if (tag === 'agreement_error') return /\bi\s+is\b|\bthey\s+was\b|\bhe\s+go\b/i.test(text);
  if (tag === 'weak_paragraphing') return text.split(/\n\s*\n/).filter(Boolean).length < 2;
  if (tag === 'poor_sequencing') return !/\bfirst|then|however|finally|therefore\b/i.test(text);
  if (tag === 'wrong_tone' || tag === 'weak_register_control') {
    if (genre === 'report' || genre === 'essay') return hasInformalTone(text);
    if (genre === 'story') return hasFormalTone(text);
  }
  if (tag === 'under_length') return countWords(text) < 40;
  return false;
};
...
const evaluateCriterion = (
  ...
  if (normalized.includes('covers all required content points')) {
    ...
    if (genre === 'report' || genre === 'essay') met = !hasInformalTone(submission);
    else met = true;
...
const buildFeedback = (metCount: number, total: number, next: RecommendedNextAction): string => {
  if (metCount === total) return 'Great work. You met all task checks today—keep this standard tomorrow.';
  if (metCount >= Math.ceil(total * 0.6)) return `Good progress. You met ${metCount}/${total} checks; focus on one missed point next.`;
  if (next === 'reduce_difficulty') return 'This task was heavy today. We will simplify the next step and rebuild control.';
  return `You met ${metCount}/${total} checks. Repeat this skill with tighter focus on the missing criteria.`;
};

const repeatedErrorsUnchanged = (
...
const threeStrongInRow = (
  memory: RepeatedErrorMemory,
  studentId: string,
  targetTags: WeaknessTag[]
): boolean => {
...
const mapTaskTypeSkillCheck = (task: DailyWritingTask, criteriaResults: PracticeCriterionResult[]): number => {
  const met = criteriaResults.filter((item) => item.met).length;
  const ratio = criteriaResults.length === 0 ? 0 : met / criteriaResults.length;

  if (task.task_type === 'full exam-style response') return Math.round(ratio * 5);
  if (task.task_type === 'rewrite from feedback') return Math.round((ratio * 5 + (met >= 2 ? 1 : 0)) / 1.1);
  return Math.round(ratio * 5);
};

export const evaluateWritingPracticeTask = (input: EvaluateWritingPracticeInput): WritingPracticeEvaluationResult => {
  ...
  const criteriaResults = input.daily_task.success_criteria.map((criterion) =>
    evaluateCriterion(criterion, input.student_submission, input.genre, withinRange)
  );
  ...
  if ((input.genre === 'report' || input.genre === 'essay') && hasInformalTone(input.student_submission)) {
    if (!detectedWeakness.includes('wrong_tone')) detectedWeakness.push('wrong_tone');
    if (!detectedWeakness.includes('weak_register_control')) detectedWeakness.push('weak_register_control');
  }
  const detectedImprovement = input.daily_task.target_tags.filter((tag) => !detectedWeakness.includes(tag));

  const completionStatus: CompletionStatus =
    metCount >= Math.ceil(criteriaResults.length * 0.75) && withinRange
      ? 'complete'
      : metCount >= Math.ceil(criteriaResults.length * 0.4)
        ? 'partial'
        : 'incomplete';

  let recommendedNextAction: RecommendedNextAction = 'move_forward';
  if (failedCount >= 2) recommendedNextAction = 'repeat_skill';
  if (completionStatus === 'incomplete' && input.daily_task.support_level === 'low') recommendedNextAction = 'reduce_difficulty';
  if (
    (metCount >= Math.ceil(criteriaResults.length * 0.6) || (input.daily_task.task_mode === 'exam_style' && metCount >= 1)) &&
  ...
  if (
    ['genre convention task', 'guided writing', 'full exam-style response'].includes(input.daily_task.task_type) &&
