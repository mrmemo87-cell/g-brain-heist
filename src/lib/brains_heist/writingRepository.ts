...
export const persistWritingStoreSnapshot = async (snapshot: SerializedWritingPersistenceStore): Promise<void> => {
  ...
  await Promise.all([
    ...
    replaceTable('bh_writing_daily_submissions', snapshot.dailySubmissions),
    replaceTable('bh_writing_daily_evaluations', snapshot.dailyEvaluations),
    replaceTable('bh_writing_monthly_reports', snapshot.monthlyReports),
    replaceTable('bh_writing_memory_snapshots', snapshot.memorySnapshots),
    replaceTable('bh_writing_prompt_bank', snapshot.promptBank),
    replaceTable('bh_writing_review_signals', snapshot.reviewSignals),
