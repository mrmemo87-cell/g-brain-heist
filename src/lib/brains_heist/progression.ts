import { SoloDifficulty, TopicSummary, BranchProgressSummary } from '../../../types';
...
export const canUnlockNextTopic = ({
  ...
  accuracy,
  avgTimeRatio,
}: TopicProgressMetrics): boolean => {
  if (missionsCompleted < 5) return false;
  if (accuracy < 0.75) return false;
  if (avgTimeRatio > 0.9) return false;
  return true;
};

const missionSort = (missions: MissionRecord[]): MissionRecord[] =>
  missions
    .slice()
    .sort((a, b) => new Date(a.completedAt).getTime() - new Date(b.completedAt).getTime());

const qualifiesForBossWindow = (missions: MissionRecord[]): boolean => {
  if (missions.length < 3) return false;
...
export const canUnlockBossNode = ({
  recentMissions,
  crushedTopics,
}: BranchProgressMetrics): boolean => {
  if (crushedTopics < 1) return false;
