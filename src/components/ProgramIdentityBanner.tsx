...
const ProgramIdentityBanner: React.FC<ProgramIdentityBannerProps> = ({
  ...
  return (
    <section
      className={`overflow-hidden rounded-2xl border border-cyan-300/20 bg-slate-950/90 shadow-[0_18px_55px_rgba(2,6,23,0.28)] ${className}`}
      aria-label={`${title} programme`}
    >
      <div className="grid md:grid-cols-[minmax(0,1fr)_minmax(13rem,34%)] md:items-stretch">
        ...
        <div className={`relative overflow-hidden bg-slate-900 ${compact ? 'min-h-[8.5rem] md:min-h-[9.5rem]' : 'min-h-[10rem] md:min-h-[11.5rem]'}`}>
          <img
            src={artwork.src}
            alt=""
            loading="lazy"
          ...
          />
          <div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-slate-950/55 via-slate-950/10 to-transparent md:from-slate-950/15" aria-hidden="true" />
        </div>
      </div>
    </section>
  );
