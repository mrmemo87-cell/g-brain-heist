Warning: truncated output (original token count: 105649)
Total output lines: 8501
...
const TeacherPortal: React.FC<TeacherPortalProps> = ({ profile, onComplete, onLogout, isSchoolAdmin, onOpenSchoolAdmin, onOpenAdmissions, initialView = 'dashboard' }) => {
  ...
  const proofreadText = (text: string, wordTarget: string, isPart1: boolean): {
    ...
    const totalErrors = spellingMista…55649 tokens truncated…ation.map((m, i) => <p key={`p2-${i}`} className="mb-1"><span className="line-through text-red-600">{m.wrong}</span> → <span className="text-green-700 font-semibold">{m.correct}</span> — {m.explanation}</p>)}
                          </div>
                        );
                      })()}
                    </div>
