...
const PortalLanding: React.FC<PortalLandingProps> = ({ onExplore, children }) => {
    ...
    const alignClass = direction === 'rtl' ? 'lg:text-right' : 'lg:text-left';
                ...
                <div ref={brandRef} className="mx-auto flex flex-col items-center text-center">
                    <div className="flex items-center gap-3 sm:gap-4">
                        ...
                        <div className={direction === 'rtl' ? 'text-right' : 'text-left'}>
                            <div dir="ltr" className="text-3xl font-black tracking-tight sm:text-5xl">Brains <span className="bg-gradient-to-r from-cyan-300 via-teal-200 to-fuchsia-400 bg-clip-text text-transparent">Heist</span></div>
                            <div className="mt-1 text-[10px] font-extrabold uppercase tracking-[0.32em] text-slate-500 sm:text-xs">{copy.tagline}</div>
                        </div>
                    </div>
                </div>

                <section className="mx-auto grid w-full flex-1 items-center gap-10 py-10 lg:grid-cols-[1.08fr_0.92fr] lg:gap-14 lg:py-8">
                    <div className={`mx-auto max-w-3xl text-center ${alignClass}`}>
                        <h1 ref={headlineRef} className="text-[clamp(3.2rem,8.5vw,7.8rem)] font-black leading-[0.9] tracking-[-0.045em] text-white">
                            {copy.heroLine1}<br />{copy.heroLine2}<br /><span className="bg-gradient-to-r from-cyan-300 via-teal-300 to-fuchsia-400 bg-clip-text text-transparent">{copy.heroLine3}</span>
                        </h1>
                        <p className={`mx-auto mt-7 max-w-xl text-base leading-relaxed text-slate-400 sm:text-lg ${direction === 'rtl' ? 'lg:mr-0' : 'lg:ml-0'}`}>{copy.heroBody}</p>
                    </div>
                    ...
                    <div ref={cardWrapRef} className="mx-auto w-full max-w-md">
                        ...
                        <button type="button" onClick={onExplore} className="group mt-7 w-full rounded-2xl border border-white/10 bg-white/[0.025] px-5 py-4 text-center transition duration-300 hover:border-cyan-300/35 hover:bg-cyan-300/[0.05]">
                            <span className="block text-[10px] font-extrabold uppercase tracking-[0.22em] text-slate-500 transition group-hover:text-cyan-300/80">{copy.notHere}</span>
                            <span className="mt-1.5 flex items-center justify-center gap-2 text-sm font-black uppercase tracking-[0.12em] text-white">
                                ...
                                {copy.explore}
                                <span className="text-cyan-300 transition-transform duration-300 group-hover:translate-x-1" aria-hidden="true">{direction === 'rtl' ? '←' : '→'}</span>
                            </span>
                        </button>
                        <div className="mt-4 text-center text-[11px] text-slate-600">{copy.categories}</div>
                    </div>
                </section>
            </main>
        </div>
