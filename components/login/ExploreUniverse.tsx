...
const ExploreUniverse: React.FC<ExploreUniverseProps> = ({ onBack, onRequestDemo }) => {
    ...
    const nodes = [
        { eyebrow: copy.learnEyebrow, title: copy.learnTitle, copy: copy.learnCopy, icon: '🧠' },
        { eyebrow: copy.competeEyebrow, title: copy.competeTitle, copy: copy.competeCopy, icon: '⚔️' },
        { eyebrow: copy.prepareEyebrow, title: copy.ieltsTitle, copy: copy.ieltsCopy, icon: '🎯' },
        { eyebrow: copy.operateEyebrow, title: copy.schoolsTitle, copy: copy.schoolsCopy, icon: '🏫' },
        { eyebrow: copy.discoverEyebrow, title: copy.admissionsTitle, copy: copy.admissionsCopy, icon: '🔬' },
    ];
        ...
        <div ref={rootRef} dir={direction} lang={language} className="relative min-h-screen overflow-hidden bg-[#06101d] text-white">
            ...
            <div className="relative z-10 mx-auto max-w-7xl px-5 py-7 sm:px-8 lg:px-12">
                <header className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
                    ...
                    <button type="button" onClick={onRequestDemo} className={`rounded-xl border border-cyan-300/25 bg-cyan-300/[0.06] px-3 py-2 text-xs font-bold text-cyan-200 transition hover:bg-cyan-300/[0.10] sm:px-4 sm:text-sm ${direction === 'rtl' ? 'justify-self-end text-right' : 'justify-self-end text-left'}`}>{copy.requestDemo}</button>
                </header>

                <div ref={titleRef} className="mx-auto max-w-3xl pb-10 pt-14 text-center sm:pt-20">
                    <p className="text-[11px] font-black uppercase tracking-[0.28em] text-cyan-300/80">{copy.universe}</p>
                    <h2 className="mt-4 text-4xl font-black tracking-[-0.04em] sm:text-6xl">{copy.missions}</h2>
                    <p className="mx-auto mt-5 max-w-2xl text-base leading-relaxed text-slate-400 sm:text-lg">{copy.universeBody}</p>
                </div>
                    ...
                    {nodes.map((node) => (
                        <article key={node.title} data-universe-node className="group relative min-h-[18rem] overflow-hidden rounded-[1.75rem] border border-white/10 bg-[#081321]/85 p-6 shadow-[0_24px_80px_rgba(0,0,0,0.28)] backdrop-blur-xl transition duration-300 hover:-translate-y-1.5 hover:border-cyan-300/30">
                            ...
                            <div className="relative">
                                ...
                                <p className="mt-7 text-[10px] font-black uppercase tracking-[0.22em] text-cyan-300/70">{node.eyebrow}</p>
                                ...
                                <div className="mt-6 text-xs font-black uppercase tracking-[0.14em] text-slate-500 transition group-hover:text-cyan-300">{copy.discover}</div>
                            </div>
                        </article>
                    ))}
                </div>
                ...
                <div className="mx-auto mt-10 max-w-2xl rounded-[1.75rem] border border-white/10 bg-white/[0.025] p-6 text-center sm:p-8">
                    <p className="text-sm leading-relaxed text-slate-400">{copy.alreadyPart}</p>
