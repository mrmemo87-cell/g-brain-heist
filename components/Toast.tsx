...
const Toast: React.FC<ToastProps> = ({ id, message, type, retryAction, onDismiss, persistent = false, actionLabel, cancelLabel, onCancel }) => {
  ...
  const typeStyles = {
    ...
    info: {
      container: 'border-sky-300 bg-sky-950 text-sky-50',
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className={`pointer-events-auto w-[min(92vw,420px)] rounded-2xl border shadow-2xl ring-1 ring-white/30 transition-all duration-300 ${style.container} ${animationClasses}`}
    >
      <div className="p-4">
