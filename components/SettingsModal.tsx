...
const THEME_COLOR_OPTIONS: Record<StudentThemeColor, { label: MessageKey; personality: MessageKey; swatches: [string, string, string] }> = {
...
const SettingsModal: React.FC<SettingsModalProps> = ({ 
  ...
  const handleNeonDeactivate = async () => {
    ...
    const confirmed = window.confirm(t("This permanently removes the neon frame glow. You will need another neon drop to enable it again. Proceed?"));
    if (!confirmed) {
      return;
    }
    ...
    try {
      ...
      setNeonSuccess(t("Neon frame removed. This change is permanent."));
      await onNeonFrameDeactivated?.();
    } catch (error: any) {
      console.error('Failed to deactivate neon frame:', error);
      setNeonError(error?.message || t("Failed to deactivate neon frame."));
    } finally {
      setNeonBusy(false);
    }
  };
  ...
  const handleFlickerDeactivate = async () => {
    ...
    const confirmed = window.confirm(t("This permanently removes the flicker theme effect. You will need another flicker drop to enable it again. Proceed?"));
    if (!confirmed) {
      return;
    }
    ...
    try {
      ...
      setFlickerSuccess(t("Flicker theme removed. This change is permanent."));
      await onFlickerThemeDeactivated?.();
    } catch (error: any) {
      console.error('Failed to deactivate flicker theme:', error);
      setFlickerError(error?.message || t("Failed to deactivate flicker theme."));
    } finally {
  ...
  const handleUsernameSave = async () => {
    ...
    try {
      ...
      setUsernameError(error.message || t("Failed to update username."));
    } finally {
  ...
  const handleFullNameSave = async () => {
    ...
    if (trimmed.length < 5 || !trimmed.includes(' ')) {
      setFullNameError(t("Enter your real first and last name."));
      return;
    }
    const changingVerifiedName = fullNameStatus === 'verified' && trimmed !== profile.full_name;
    if (changingVerifiedName && !window.confirm(t("Changing a verified name sends it back to your school administrator for confirmation. Continue?"))) {
      return;
    }
    ...
    try {
      ...
      if (error || !data?.success) throw new Error(data?.error || error?.message || t("Could not save your real name."));
      setFullName(trimmed);
      setFullNameStatus('pending');
      setFullNameMessage(t("Saved. Your school administrator now needs to confirm this name."));
    } catch (error: any) {
      setFullNameError(error?.message || t("Could not save your real name."));
    } finally {
  ...
  return (
    <div
      ...
      className={`localized-ui fixed inset-0 z-[70] flex justify-center p-4 ${isHeaderAnchored ? 'items-start' : 'items-center'}`}
    ...
    >
      ...
      >
        ...
        >
          <section className="language-preference" aria-label={t('Interface language')}>
            <h3 className="font-heading text-xl text-white">{t('Interface language')}</h3>
            <LanguageMiniCard />
            <p>{t('Saved on this device. IELTS and Cambridge questions and materials stay in English.')}</p>
          </section>
          ...
          {requiredChanges && (requiredChanges.username || requiredChanges.avatar) && (
            <div className="rounded-xl border-2 border-yellow-500/60 bg-yellow-500/10 p-4 space-y-3">
              <h3 className="font-heading text-lg text-yellow-400">⚠️ {t('Profile Update Required')}</h3>
              {requiredChanges.reason && (
                <p className="text-sm text-yellow-200/80">{t('Reason:')} <bdi>{requiredChanges.reason}</bdi></p>
              )}
              ...
              {((requiredChanges.username && (usernameChanged || usernameSuccess)) || !requiredChanges.username) &&
               ((requiredChanges.avatar && (avatarChanged || avatarUploadSuccess)) || !requiredChanges.avatar) && (
                <p className="text-sm text-green-300 font-semibold mt-2">{t("🎉 All changes complete! Your access will be restored automatically.")}</p>
              )}
            </div>
          )}
          ...
          {/* Avatar Selection */}
          <div>
            <h3 className="font-heading text-xl mb-3" style={{ color: 'var(--amber-warn)' }}>{t("Avatar")}</h3>
            <div className="mb-4 flex items-center gap-3 rounded-2xl border border-white/10 bg-black/25 p-3">
              <AvatarWithFrame
                ...
                alt={profile.username || t("Current avatar")}
                size="md"
              ...
              />
              <div>
                <p className="text-sm font-semibold text-white">{t("Current avatar preview")}</p>
                <p className="text-xs text-gray-400">{t("Active cosmetics are rendered here the same way other players see them.")}</p>
              </div>
            </div>
            <div className="grid grid-cols-4 gap-2 mb-3">
              {avatarPresets.map((avatarUrl, idx) => (
                ...
                >
                  <img src={avatarUrl} alt={t('Avatar {number}', { number: idx + 1 })} className="w-full h-full object-cover" />
                </button>
              ))}
            </div>
            <div className="mt-4 border-t border-gray-700 pt-4">
              <label className="block text-sm text-gray-300 mb-2">{t("Upload custom avatar")}</label>
              <input
                type="file"
              ...
              />
              <p className="text-xs text-gray-500 mt-2">{t("PNG, JPG, or WebP. Large images are auto-resized to fit.")}</p>
              {avatarUploadError && (
                <p className="mt-2 text-xs text-red-300">{avatarUploadError}</p>
              )}
              {avatarUploadSuccess && (
                <p className="mt-2 text-xs text-green-300 font-semibold">{t("✅ Avatar updated successfully!")}</p>
              )}
            </div>
            {uploadingAvatar && (
              <p className="text-xs text-center text-cyan-400 animate-pulse mt-3">{t("Saving avatar...")}</p>
            )}
          </div>

          {/* Profile Section */}
          <div>
            <h3 className="font-heading text-xl mb-3" style={{ color: 'var(--amber-warn)' }}>{t("Profile")}</h3>
            <div className="space-y-3">
              {profile.role === 'student' && (
                <div className="p-4 bg-black/20 rounded-lg space-y-3 border border-gray-700">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="font-semibold text-white">{t("Real name")}</p>
                      <p className="text-xs text-gray-400">{t("Private school identity used on Cambridge tests—not your public codename.")}</p>
                    </div>
                    <span className={`shrink-0 text-xs px-2 py-1 rounded-full ${
                      ...
                      {fullNameStatus === 'verified' ? t("Confirmed") : fullNameStatus === 'rejected' ? t("Correction required") : t("Awaiting confirmation")}
                    </span>
                  </div>
                  <label htmlFor="settings-full-name" className="sr-only">{t("Real first and last name")}</label>
                  <input
                  ...
                  >
                    {fullNameSaving ? t("Saving…") : fullNameStatus === 'verified' ? t("Submit name change") : t("Send to school for confirmation")}
                  </button>
                  {fullNameStatus === 'pending' && <p className="text-xs text-amber-200">{t("You can use Cambridge tests after a school administrator confirms this name.")}</p>}
                  {fullNameStatus === 'rejected' && <p className="text-xs text-red-300">{t("Your school requested a correction. Update the name and send it again.")}</p>}
              ...
              <div className="p-3 bg-black/20 rounded-lg space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-gray-300">{t("Username")}</span>
                  {!editingUsername ? (
                ...
                {usernameSuccess && <p className="text-xs text-green-300">{t("✅ Username updated!")}</p>}
              </div>
              ...
              {profile.role === 'teacher' && (
                <div className="flex items-center justify-between p-3 bg-black/20 rounded-lg">
                  ...
                  <span className="font-bold text-cyan-400">{t("Teacher")}</span>
                </div>
              )}
              {isAdminMode && (
                <div className="flex items-center justify-between p-3 bg-black/20 rounded-lg border border-pink-500/50">
                  <span className="text-gray-300">{t("Admin")}</span>
                  <span className="font-bold text-pink-400">{t("✓ Verified")}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {profile.role !== 'teacher' && (
          <div>
            <h3 className="font-heading text-xl mb-3" style={{ color: 'var(--amber-warn)' }}>{t("Cosmetics")}</h3>
            {hasNeonFrame ? (
              <div className="rounded-2xl border border-amber-400/50 bg-amber-500/10 p-4 space-y-3">
                <div>
                  <p className="font-semibold text-amber-200">{t("Neon Frame Active")}</p>
                  <p className="text-sm text-amber-100/80">
                    {t("Your avatar glows across PvP, clan, and leaderboards. Turning it off is permanent and consumes the neon frame item.")}</p>
                </div>
                <button
                ...
                >
                  {neonBusy ? t("Removing…") : t("Deactivate Neon Frame Forever")}
                </button>
                <p className="text-xs text-amber-100/70">
                  {t("⚠️ Once disabled you must unlock another neon frame drop to regain the effect.")}</p>
              ...
              <div className="rounded-2xl border border-gray-700 bg-black/30 p-4 text-sm text-gray-400">
                <p>{t("No neon frame is currently active on this account.")}</p>
              </div>
            )}
            ...
            {hasFlickerTheme ? (
              <div className="mt-4 rounded-2xl border border-cyan-400/50 bg-cyan-500/10 p-4 space-y-3">
                <div>
                  <p className="font-semibold text-cyan-200">{t("Flicker Theme Active")}</p>
                  <p className="text-sm text-cyan-100/80">
                    {t("Your avatar has a flickering, datamosh effect visible across PvP, clan, and leaderboards. Turning it off is permanent and consumes the flicker theme item.")}</p>
                </div>
                <button
                ...
                >
                  {flickerBusy ? t("Removing…") : t("Deactivate Flicker Theme Forever")}
                </button>
                <p className="text-xs text-cyan-100/70">
                  {t("⚠️ Once disabled you must unlock another flicker theme drop to regain the effect.")}</p>
              ...
              <div className="mt-4 rounded-2xl border border-gray-700 bg-black/30 p-4 text-sm text-gray-400">
                <p>{t("No flicker theme is currently active on this account.")}</p>
              </div>
            )}
          </div>
          )}
          ...
          <div>
            ...
            <div className="rounded-2xl border border-amber-400/50 bg-amber-500/10 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-semibold text-amber-200">{t("🧠 Public Badge Display")}</p>
                  <p className="text-sm text-amber-100/80">
                    {t("Show the Brains Master badge next to your name on leaderboards, clans, and PvP.")}</p>
                </div>
                <button
                  onClick={async () => {
                    setBmBadgeBusy(true);
                ...
                >
                  {bmBadgeBusy ? '...' : bmShowBadge ? t("Visible") : t("Hidden")}
                </button>
              </div>
              <p className="text-xs text-amber-100/60">
                {t("Premium benefits remain active regardless of badge visibility.")}</p>
            </div>
          </div>
          )}
          ...
          <div>
            <h3 className="font-heading text-xl mb-3" style={{ color: 'var(--amber-warn)' }}>{t("Display")}</h3>
            <div className="student-display-settings border border-gray-700 rounded-2xl p-4 space-y-5">
              <fieldset>
                <legend className="font-semibold text-white">{t("Interface style")}</legend>
                <p className="mt-1 text-sm text-gray-400">{t("Choose how dashboard surfaces look. This does not change your content or progress.")}</p>
                <div className="mt-3 grid gap-3 sm:grid-cols-2">
                  <button type="button" role="radio" aria-checked={!isLightMode} onClick={() => setInterfaceStyle('glassy')} className={`student-display-style-option ${!isLightMode ? 'is-selected' : ''}`}>
                    ...
                    <span className="block font-bold text-white">{t("Glassy")}</span>
                    <span className="mt-1 block text-xs leading-5 text-gray-400">{t("Layered glass, soft glow, smooth depth, and richer motion.")}</span>
                  </button>
                  <button type="button" role="radio" aria-checked={isLightMode} onClick={() => setInterfaceStyle('basic')} className={`student-display-style-option ${isLightMode ? 'is-selected' : ''}`}>
                    ...
                    <span className="block font-bold text-white">{t("Basic")}</span>
                    <span className="mt-1 block text-xs leading-5 text-gray-400">{t("Clean solid surfaces with fewer effects for smoother use and longer battery life.")}</span>
                  </button>
                </div>
              </fieldset>
              ...
              {(profile.role === 'student' || profile.role === 'teacher') ? (
                <fieldset className="border-t border-gray-700 pt-4">
                  <legend className="font-semibold text-white">{t("Interface color")}</legend>
                  <p className="mt-1 text-sm text-gray-400">{t("Choose a complete dashboard personality. Your selection changes surfaces, navigation, buttons, progress, cards, and glow.")}</p>

                  <div className="student-theme-live-preview mt-4" role="status" aria-live="polite" aria-label={t('{color} interface preview', { color: t(THEME_COLOR_OPTIONS[studentThemeColor].label) })}>
                    <div className="student-theme-live-preview__topline">
                      <span>{t("Live preview")}</span>
                      <strong>{t(THEME_COLOR_OPTIONS[studentThemeColor].label)} · {isLightMode ? t("Basic") : t("Glassy")}</strong>
                    </div>
                    <div className="student-theme-live-preview__dashboard" aria-hidden>
                      ...
                      <span className="student-theme-live-preview__action">{t("Continue")}</span>
                    </div>
                    <div className="student-theme-live-preview__progress" aria-hidden><span /></div>
                    <div className="student-theme-live-preview__chips" aria-hidden><span>{t("Learn")}</span><span>{t("Tasks")}</span><span>{t("Profile")}</span></div>
                  </div>

                  <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3" role="radiogroup" aria-label={t("Dashboard interface color")}>
                    {STUDENT_THEME_COLORS.map((color) => {
                      const option = THEME_COLOR_OPTIONS[color];
                      const selected = studentThemeColor === color;
                      return (
                        <button key={color} type="button" role="radio" aria-checked={selected} aria-label={`${t(option.label)}: ${t(option.personality)}`} data-theme-color={color} onClick={() => setStudentThemeColor(color)} className={`student-theme-option ${selected ? 'is-selected' : ''}`}>
                          ...
                          <span className="min-w-0">
                            ...
                            <span className="mt-0.5 block text-[11px] leading-4 text-slate-400">{t(option.personality)}</span>
                          </span>
              ...
              {autoEnabledReason && (
                <div className="bg-amber-900/30 border border-amber-600/50 rounded p-3 text-sm text-amber-100 space-y-2">
                  <div className="font-semibold">{t("We protected your device automatically")}</div>
          ...
          <div className="text-xs text-gray-500 text-center pt-4 border-t border-gray-700">
            <p>{t("Your display choices are saved on this device.")}</p>
          </div>
        </div>
      </div>
    </div>
