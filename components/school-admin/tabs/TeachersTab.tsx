...
const TeachersTab: React.FC = () => {
  ...
  const [subjectCatalog, setSubjectCatalog] = React.useState<SchoolSubjectCatalog | null>(null);

  React.useEffect(() => {
    let active = true;
    void fetchSchoolSubjectCatalog(school.id)
      .then((value) => { if (active) setSubjectCatalog(value); })
      .catch(() => { if (active) setSubjectCatalog(null); });
    return () => { active = false; };
  }, [school.id]);
  ...
  const allocatableSubjects = React.useMemo(() => (subjectCatalog?.subjects || [])
    .filter((subject) => subject.isActive && subject.offerings.some((offering) => (
      selectedClassGradeLevel != null && Number(offering.gradeLevel) === Number(selectedClassGradeLevel)
    )))
    .sort((left, right) => left.name.localeCompare(right.name)), [selectedClassGradeLevel, subjectCatalog?.subjects]);
  ...
  const subjectOptions = React.useMemo(() => {
    ...
    (subjectCatalog?.subjects || []).forEach((subject) => {
      if (!subject.isActive) return;
      if (selectedFilterGrade && !subject.offerings.some((offering) => String(offering.gradeLevel) === selectedFilterGrade)) return;
      options.set(subject.id, subject.name);
    });
    (teacherAllocations || []).forEach((allocation: any) => {
      const schoolClass = classById[allocation.class_id];
      if (filterClassId && allocation.class_id !== filterClassId) return;
      if (selectedFilterGrade && String(schoolClass?.grade_level ?? allocation.grade_level ?? '') !== selectedFilterGrade) return;
      const key = allocation.school_subject_id || `legacy:${allocation.subject}`;
      options.set(key, allocation.subject);
    });
    return Array.from(options, ([id, name]) => ({ id, name }))
      .sort((left, right) => left.name.localeCompare(right.name));
  }, [classById, filterClassId, selectedFilterGrade, subjectCatalog?.subjects, teacherAllocations]);
  ...
  React.useEffect(() => {
    if (filterSubject && !subjectOptions.some((subject) => subject.id === filterSubject)) setFilterSubject('');
  ...
  const sortedAllocations = React.useMemo(() => {
    const rows = (teacherAllocations || []).filter((allocation: any) => {
      ...
      if (filterSubject && (allocation.school_subject_id || `legacy:${allocation.subject}`) !== filterSubject) return false;
  ...
  return <div className="space-y-6">
    ...
    <section className="admin-table-card" aria-labelledby="current-allocations-title">
      ...
      <div className="admin-assignment-filters admin-assignment-filters-bar">
        ...
        <label><span>Subject</span><select aria-label="Filter allocations by subject" value={filterSubject} onChange={(event) => setFilterSubject(event.target.value)}><option value="">All subjects</option>{subjectOptions.map((subject) => <option key={subject.id} value={subject.id}>{subject.name}</option>)}</select></label>
      ...
      {pagedAllocations.length ? <div className="admin-table-scroll"><table>
        ...
        <tbody>{pagedAllocations.map((allocation: any) => {
          ...
          return <tr key={allocation.id}><td>{gradeLevel != null ? `Grade ${gradeLevel}` : 'Not set'}</td><td><strong>{classCode}</strong><span className="admin-table-subline">{className}</span></td><td><strong>{allocation.subject}</strong>{allocation.academic_subject_name ? <span className="admin-table-subline">{allocation.academic_subject_name === allocation.subject ? 'Academic resources mapped' : `Academic map: ${allocation.academic_subject_name}`}</span> : <span className="admin-table-subline text-amber-700">No academic map yet</span>}</td><td><strong>{teacherName}</strong>{teacherEmail && <span className="admin-table-subline">{teacherEmail}</span>}{teacherUnavailable && <span className="admin-table-subline text-amber-700">Allocation needs staff-status review</span>}</td><td>{formatAdminDate(allocation.allocated_at)}</td><td className="admin-row-actions"><button className="admin-button-danger admin-button-small" onClick={() => {
    ...
    {isAllocationOpen && <section id="allocate-teacher-panel" className="admin-form-card" aria-labelledby="allocate-teacher-title">
      <div className="admin-card-heading"><div><h3 id="allocate-teacher-title">Allocate teacher to class and subject</h3><p>Select the exact school subject taught in this class. Academic mapping only controls which curriculum and question resources that subject can use.</p></div><button type="button" className="admin-button-ghost admin-button-small" onClick={() => setIsAllocationOpen(false)}>Close</button></div>
      <div className="admin-form-grid admin-form-grid-four">
        ...
        <label className="admin-field"><span>School subject <i>Required</i></span><select value={allocationSubjectInput} disabled={!allocationClassId} onChange={(event) => setAllocationSubjectInput(event.target.value)}><option value="">{allocationClassId ? 'Select school subject' : 'Select class first'}</option>{allocatableSubjects.map((subject) => <option key={subject.id} value={subject.id}>{subject.name}{subject.academicSubjectName && subject.academicSubjectName !== subject.name ? ` — mapped to ${subject.academicSubjectName}` : subject.mappingStatus === 'unmapped' ? ' — no academic map' : ''}</option>)}</select></label>
      ...
      {allocationClassId && !allocatableSubjects.length && <div className="admin-inline-warning" role="status"><strong>No school subjects offered for this grade</strong><span>Add or edit the subject in Curriculum &amp; Subjects → School Subjects first.</span></div>}
