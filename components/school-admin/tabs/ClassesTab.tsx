  ...
  return (
    <div className="space-y-6">
      <section className="admin-form-card class-creation-wizard">
        <div className="admin-card-heading"><div><h3>{classForm.id ? 'Edit class' : 'Add another class'}</h3><p>Saving a grade plan creates its first class automatically. Add another class only when a grade level needs sections such as 7A and 7B.</p></div><button type="button" className="admin-button-ghost admin-button-small" aria-expanded={showWizard} onClick={() => setShowWizard((open) => !open)}>{showWizard ? 'Close' : 'Add class'}</button></div>
        {showWizard ? <>
        ...
        {grades.length ? <div className="school-grade-groups">
          {grades.map((grade: any) => {
            ...
            return <section key={String(grade)} className="school-grade-group">
              <header><div><h4>{grade === 'Unassigned' ? 'Grade level not set' : `Grade ${grade}`}</h4><p>{rows.length} {rows.length === 1 ? 'class' : 'classes'}</p></div><div className="school-grade-totals"><span><strong>{gradeStudents}</strong> students</span><span><strong>{gradeTeachers}</strong> teachers</span></div></header>
