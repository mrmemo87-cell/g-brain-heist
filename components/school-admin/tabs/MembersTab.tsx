...
const MembersTab: React.FC = () => {
  ...
  const getAssignedClass = (member: any) => {
    if (member.role !== 'student') return null;
    const assignedClassId = studentAssignments[member.user_id] || '';
    return classes.find((schoolClass: any) => schoolClass.id === assignedClassId) || null;
  };
  const getDisplayPlacement = (member: any) => {
    const assignedClass = getAssignedClass(member);
    return {
      grade: assignedClass?.grade_level ?? member.grade,
      batch: assignedClass?.class_code ?? member.batch,
    };
  };
    ...
    <section className="community-directory">
      ...
      <div className="community-table-wrap community-table-desktop"><table><thead><tr><th>Person</th><th>Role</th><th>{activePeopleTab === 'student' ? 'Year / class' : 'Department / class'}</th><th>Date joined</th><th>Status</th><th /></tr></thead><tbody>
        {visiblePeople.map((member: any) => { const placement = getDisplayPlacement(member); return <tr key={member.user_id} onClick={() => openMember(member)} tabIndex={0} onKeyDown={e => e.key === 'Enter' && openMember(member)}>
          ...
          <td><span className="role-chip">{member.role === 'student' ? 'Student' : 'Teacher'}</span></td><td>{placement.grade ? `Year ${placement.grade}` : 'Not assigned'}{placement.batch ? ` · ${placement.batch}` : ''}</td><td>{member.joined_at ? new Date(member.joined_at).toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' }) : 'Not recorded'}</td><td><span className={`status-dot ${member.is_banned ? 'is-blocked' : ''}`} />{member.is_banned ? 'Restricted' : 'Active'}</td><td><button onClick={e => { e.stopPropagation(); openMember(member); }}>Manage →</button></td>
        </tr>; })}
      ...
      <div className="community-mobile-list" aria-label={`${activePeopleTab === 'student' ? 'Students' : 'Teachers'} directory`}>
        {visiblePeople.map((member: any) => { const placement = getDisplayPlacement(member); return <article className="community-mobile-card" key={member.user_id}>
          ...
          <dl className="community-mobile-card__details">
            <div><dt>{activePeopleTab === 'student' ? 'Year / class' : 'Department / class'}</dt><dd>{placement.grade ? `Year ${placement.grade}` : 'Not assigned'}{placement.batch ? ` · ${placement.batch}` : ''}</dd></div>
