  ...
  return <div className="space-y-5">
    ...
    <section className="community-directory">
      ...
      <div className="community-filters" aria-label={`${activePeopleTab === 'student' ? 'Student' : 'Teacher'} filters`}>
        <label><span>Academic year (grade)</span><select value={academicYearFilter} onChange={(event) => { setAcademicYearFilter(event.target.value); setClassFilter(''); }}><option value="">All academic years</option>{academicYears.map((grade) => <option key={grade} value={grade}>Grade {grade}</option>)}</select></label>
      ...
      <div className="community-table-wrap community-table-desktop"><table><thead><tr><th>Person</th><th>Role</th><th>Academic year (grade) / class</th><th>Date joined</th><th>Status</th><th /></tr></thead><tbody>
      ...
      <div className="community-mobile-list" aria-label={`${activePeopleTab === 'student' ? 'Students' : 'Teachers'} directory`}>
        {visiblePeople.map((member: any) => { const placement = getDisplayPlacement(member); return <article className="community-mobile-card" key={member.user_id}>
          ...
          <dl className="community-mobile-card__details">
            <div><dt>Academic year (grade) / class</dt><dd>{placement.grade ? `Grade ${placement.grade}` : 'Not assigned'}{placement.batch ? ` · ${placement.batch}` : ''}</dd></div>
