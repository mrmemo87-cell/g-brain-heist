...
import {
  ...
  requestProgrammeTransferException, switchProgrammeSeat, type ProgrammeSeatAssignment,
...
const ProgrammeSeatManager: React.FC<Props> = ({ schoolId, addToast }) => {
  ...
  return <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" aria-labelledby="programme-licences-title">
    <div className="border-b border-slate-200 bg-gradient-to-r from-slate-950 via-cyan-950 to-slate-950 p-5 text-white sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-cyan-300">Named-seat command centre</p><h3 id="programme-licences-title" className="mt-1 text-2xl font-bold">Put every purchased seat to work</h3><p className="mt-2 max-w-3xl text-sm leading-6 text-slate-300">Assign by learner or class, see the consequence before every change, and switch programmes atomically. The live agreement—not a percentage estimate—is always authoritative.</p></div><button type="button" onClick={() => void load()} className="rounded-lg border border-white/20 px-3 py-2 text-xs font-semibold">Refresh</button></div>
    ...
    <div className="p-5 sm:p-6">
      <div className="grid gap-3 md:grid-cols-3">{overview.programmes.map((item) => {
        ...
        return <button key={item.module_key} type="button" aria-pressed={selected} onClick={() => { setProgramme(item.module_key); setReleaseId(null); }} className={`rounded-xl border p-4 text-left transition ${selected ? 'border-cyan-700 bg-cyan-50 ring-1 ring-cyan-700' : 'border-slate-200 hover:border-cyan-300'}`}>
          ...
          <span className="mt-2 block text-xs text-slate-600">{item.assigned} assigned · {item.cooling_down} cooling · {Math.max(0, item.transfer_limit - item.transfers_used)}/{item.transfer_limit} transfers</span>
        </button>;
      })}</div>
      ...
      {pool?.seat_limit == null ? <div className="mt-5 rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-950"><strong>Seat allocation is waiting for a fixed agreement.</strong><p className="mt-1">Accept an approved package in Plan &amp; Billing Studio. Brains Heist will activate the exact quoted capacities after payment verification.</p><button type="button" onClick={() => document.getElementById('billing-studio-builder')?.scrollIntoView({ behavior: 'smooth' })} className="mt-3 font-bold text-amber-950 underline">Review plan options</button></div> : <>
        ...
        {view === 'activity' && <div className="mt-5 space-y-2">{overview.events.filter((event) => event.module_key === programme).length === 0 ? <p className="rounded-xl bg-slate-50 p-5 text-sm text-slate-500">No assignment activity yet.</p> : overview.events.filter((event) => event.module_key === programme).map((event) => <article key={event.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 p-3"><div><strong className="text-sm text-slate-950">{event.student_name}</strong><p className="text-xs text-slate-500">{event.event_type.replace('_',' ')}{event.reason ? ` · ${event.reason.replace('_',' ')}` : ''}</p></div><time className="text-xs text-slate-500">{dateTime(event.created_at)}</time></article>)}</div>}

        {view === 'policy' && <div className="mt-5 grid gap-4 lg:grid-cols-2"><div className="rounded-xl border border-slate-200 p-5"><h5 className="font-bold text-slate-950">What schools can change</h5><ul className="mt-3 space-y-2 text-sm leading-6 text-slate-600"><li>• Assign any available seat to an active learner.</li><li>• Correct an unused mistake within {overview.policy.correction_hours} hours.</li><li>• Transfer up to {overview.policy.base_transfer_percent}% of seats monthly, minimum two.</li><li>• Return a seat immediately after the membership is genuinely inactive.</li></ul></div><div className="rounded-xl border border-slate-200 p-5"><h5 className="font-bold text-slate-950">What prevents manipulation</h5><ul className="mt-3 space-y-2 text-sm leading-6 text-slate-600"><li>• Learning usage closes the free-correction path.</li><li>• Ordinary transfers cool down for {overview.policy.cooldown_days} days.</li><li>• “Left school” is rejected while membership remains active.</li><li>• Every assignment, correction, release and override is auditable.</li></ul></div></div>}
