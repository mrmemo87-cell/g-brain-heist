export type CambridgeExpectedAnswer = string | readonly string[];
...
export const CAMBRIDGE_LISTENING_TEST_1_ANSWER_KEY: Record<number, CambridgeExpectedAnswer> = {
  1: 'C', 2: 'C', 3: 'B', 4: 'A', 5: 'C',
 ...
 */
export const parseCambridgeResponses = (savedAnswers: unknown): Record<number, string> => {
...
export const normalizeCambridgeAnswer = (value: unknown): string =>
  String(value ?? '')
    .toLowerCase()
    .trim()
    .replace(/,/g, '')
...
export const isCambridgeAnswerCorrect = (
  studentAnswer: unknown,
  expectedAnswer: CambridgeExpectedAnswer,
): boolean => {
...
export const getPrimaryCambridgeAnswer = (expectedAnswer: CambridgeExpectedAnswer): string =>
  Array.isArray(expectedAnswer) ? String(expectedAnswer[0] ?? '') : String(expectedAnswer);
