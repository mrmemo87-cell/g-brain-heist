...
const SchoolHeadPortal: React.FC<SchoolHeadPortalProps> = ({
  ...
  const renderSubscription = () => (
    <div className="school-head-page">
      ...
      <section className="school-head-subscription-hero"><div><span>Current plan</span><h3>{hasActivePlan ? formatPlan(snapshot.subscription.plan) : 'No active plan'}</h3><p>{hasActivePlan ? `${formatPlan(snapshot.subscription.status)}${snapshot.subscription.billing_interval ? ` · ${formatPlan(snapshot.subscription.billing_interval)} billing` : ''}` : 'Activation required'}</p></div><dl><div><dt>Current period ends</dt><dd>{hasActivePlan ? formatDate(snapshot.subscription.current_period_end) : 'Starts after activation'}</dd></div><div><dt>Seats used</dt><dd>{snapshot.subscription.seats_used}{snapshot.subscription.seat_limit ? ` / ${snapshot.subscription.seat_limit}` : ''}</dd></div><div><dt>Renewal status</dt><dd>{hasActivePlan ? (snapshot.subscription.cancel_at_period_end ? 'Set to end' : 'Continuing') : 'Not applicable'}</dd></div></dl></section>
      <div className="school-head-two-column">
        <section className="school-head-panel"><div className="school-head-panel-heading"><div><p>Capacity</p><h3>Student seat usage</h3></div>{seatPercent !== null && <span>{seatPercent}% used</span>}</div>{snapshot.subscription.seat_limit ? <div className="school-head-seat-visual"><div><i style={{ width: `${seatPercent}%` }} /></div><p><strong>{Math.max(snapshot.subscription.seat_limit - snapshot.subscription.seats_used, 0)}</strong> seats available</p></div> : <EmptyState title="Seat limit not active">A student seat limit will appear after a plan or pilot is activated.</EmptyState>}</section>
        <section className="school-head-panel"><div className="school-head-panel-heading"><div><p>Value signals</p><h3>Current adoption</h3></div></div><ul className="school-head-value-list"><li><span>{snapshot.totals.students ? `${studentActivityRate}%` : 'Not started'}</span><div><strong>Weekly student adoption</strong><small>{snapshot.totals.students ? `${snapshot.engagement.active_students_7d} active students` : 'No registered students have joined yet'}</small></div></li><li><span>{snapshot.totals.teachers ? `${teacherActivityRate}%` : 'Not started'}</span><div><strong>Weekly staff adoption</strong><small>{snapshot.totals.teachers ? `${snapshot.engagement.active_teachers_7d} active teaching staff` : 'No teaching staff have joined yet'}</small></div></li><li><span>{snapshot.programs.cambridge_attempts}</span><div><strong>Assessment activity</strong><small>{snapshot.programs.cambridge_attempts ? 'Recorded attempts in the reporting period' : 'No recorded attempts yet'}</small></div></li></ul></section>
      </div>
