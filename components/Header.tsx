            ...
            <div className="flex items-center justify-between">
              ...
              <div className="relative flex items-center gap-2" ref={mobileMenuRef}>
                ...
                <button
                  ...
                  className={`relative overflow-hidden rounded-full ${hasNeonFrame || hasGlitchTheme ? (hasGlitchTheme ? 'glitch-frame glitch-frame-sm' : 'neon-frame neon-frame-sm') : 'border border-pink-500/70'}`}
                  aria-label="Open settings"
                >
                  <img
                    src={profile.avatar_url || avatarPresets[0]}
                    alt={profile.username || 'Player avatar'}
                    className={`h-10 w-10 rounded-full object-cover ${hasNeonFrame || hasGlitchTheme ? (hasGlitchTheme ? 'glitch-frame-avatar' : 'neon-frame-avatar') : ''}`}
                  />
                </button>
          ...
          <div className="hidden md:flex items-center justify-between gap-4">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-2 px-4 py-2 bg-black/40 rounded-full border border-cyan-500/30 backdrop-blur-sm">
                <div
                  className={`flex-shrink-0 rounded-full transition-transform duration-200 cursor-pointer hover:scale-110 ${hasNeonFrame || hasGlitchTheme ? (hasGlitchTheme ? 'glitch-frame glitch-frame-sm' : 'neon-frame neon-frame-sm') : 'border-2 border-pink-500'}`}
                  onClick={() => setShowSettingsModal(true)}
                >
                  <img 
                    src={profile.avatar_url} 
                    alt={profile.username} 
                    className={`w-8 h-8 rounded-full object-cover ${hasNeonFrame || hasGlitchTheme ? (hasGlitchTheme ? 'glitch-frame-avatar' : 'neon-frame-avatar') : ''}`}
                  />
                </div>
