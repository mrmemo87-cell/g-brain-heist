                    ...
                    <div className="question-batch__question-body">
                      ...
                      <div className="question-batch__editor-grid">
                        ...
                        <section className="question-batch__editor-section question-batch__editor-section--taxonomy">
                          ...
                          <div className="question-batch__fields two-columns">
                            <label><span>Primary skill</span><input value={taxonomy.primary_skill_name} maxLength={160} readOnly={taxonomy.registry_match === true} aria-readonly={taxonomy.registry_match === true} onChange={(event) => updateQuestion(question.client_id, (current) => ({ ...current, taxonomy_proposal: { ...current.taxonomy_proposal, primary_skill_name: event.target.value } }))} />{taxonomy.registry_match && taxonomy.primary_skill_code ? <small>Canonical · <code>{taxonomy.primary_skill_code}</code></small> : <small>Requires taxonomy review before it can become Academic Profile evidence.</small>}</label>
                            <label><span>Atomic subskill</span><input value={taxonomy.atomic_subskill_name} maxLength={200} readOnly={taxonomy.registry_match === true} aria-readonly={taxonomy.registry_match === true} onChange={(event) => updateQuestion(question.client_id, (current) => ({ ...current, taxonomy_proposal: { ...current.taxonomy_proposal, atomic_subskill_name: event.target.value } }))} />{taxonomy.registry_match && taxonomy.atomic_subskill_code ? <small>{taxonomy.registry_version || 'Published registry'} · <code>{taxonomy.atomic_subskill_code}</code></small> : <small>Do not invent a new official label here; governance will choose a canonical registry entry.</small>}</label>
                            <label><span>Evidence Focus</span><input value={taxonomy.evidence_focus_name || ''} readOnly aria-readonly="true" placeholder="Assigned after canonical subskill matching" />{taxonomy.evidence_focus_code ? <small>Governed intervention target · <code>{taxonomy.evidence_focus_code}</code></small> : <small>Superadmin governance must select a controlled focus before verification.</small>}</label>
