...
const StudentAcademicProfile: React.FC<StudentAcademicProfileProps> = ({
  ...
  return <section className="sap-shell">
    <AcademicProgressHeader
      ...
      onBack={onClose}
      backLabel={backLabel}
      actions={canGenerateReport ? <button type="button" className="aps-primary-button" onClick={() => setShowReport(true)}>Generate school report</button> : null}
    />
    ...
    <section className="sap-panel">
      <div className="sap-panel-heading"><div><span>Subject overview</span><h2>Attainment and progress</h2></div><p>Marks are shown alongside longer-term learning patterns so staff can distinguish a one-off result from a recurring need.</p></div>
