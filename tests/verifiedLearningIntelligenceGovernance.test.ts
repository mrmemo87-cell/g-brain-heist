...
const governanceMigrationPath =
  'supabase/migrations/20260824173143_strengthen_verified_learning_intelligence.sql';
const seedMigrationPath =
  'supabase/migrations/20260824173144_seed_verified_question_diagnostic_taxonomy.sql';
const profileMigrationPath =
  'supabase/migrations/20260824174442_lock_academic_profile_verified_evidence.sql';
const quarantineMigrationPath =
  'supabase/migrations/20260824181104_quarantine_defective_legacy_questions_and_mappings.sql';
...
const interventionWorkspace = readFileSync(
  'components/student-progress/InterventionTargetedPracticeWorkspace.tsx',
  'utf8',
);
const interventionService = readFileSync('services/studentInterventionService.ts', 'utf8');

const readJsonLines = (filePath: string) => readFileSync(filePath, 'utf8')
  .split(/\r?\n/)
...
test('reviewed canonical and legacy taxonomy artifacts have complete, disjoint authority states', () => {
  ...
  const embeddedManifest = seedMigration.match(
    /\$bh_verified_question_taxonomy\$\s*(\[[\s\S]*?\])\s*\$bh_verified_question_taxonomy\$::jsonb/,
  );
...
test('diagnostic taxonomy and item evidence ledger are RLS-protected, immutable, and service-readable only', () => {
  for (const table of [
    ...
    assert.match(governanceMigration, new RegExp(
    ...
    assert.doesNotMatch(governanceMigration, new RegExp(
      `grant (insert|update|delete|all)[^;]*public\\.${table}[^;]*service_role`,
      'i',
    ));
  }
  ...
  assert.match(
    governanceMigration,
    /create trigger trg_verified_question_diagnostic_taxonomy_immutable\s+before update or delete on public\.verified_question_diagnostic_taxonomy/,
  );
  assert.match(
    governanceMigration,
  ...
  assert.match(
    governanceMigration,
    /create trigger trg_student_learning_item_evidence_immutable\s+before update or delete on public\.student_learning_item_evidence/,
  );
  assert.match(
    governanceMigration,
...
test('assignment question diagnostic snapshots are captured once and guarded before ordinary updates', () => {
  ...
  assert.match(
    governanceMigration,
    /create trigger trg_aaa_assignment_question_snapshot_immutable\s+before update on public\.assignment_questions/,
  );
});
...
test('Writing Hub contributes to official learning state only after an immutable final teacher review', () => {
  ...
  assert.match(
    governanceMigration,
    /create trigger trg_bh_writing_assessments_immutable\s+before update or delete on public\.bh_writing_assessments/,
  );
  assert.match(
    governanceMigration,
    /create trigger trg_bh_writing_assessment_reviews_immutable\s+before update or delete on public\.bh_writing_assessment_reviews/,
  );
  assert.match(
    governanceMigration,
...
test('official assignment summaries fail closed on verified origin, grade, current hash, and practice provenance', () => {
  assert.match(
    governanceMigration,
    /drop trigger if exists trg_student_learning_capture_assignment_result\s+on public\.student_assignment_results;/,
  );
  const compatibilityIngest = functionBody(
    governanceMigration,
  ...
  const summaryViewStart = profileMigration.indexOf(
    'create or replace view private.student_verified_assignment_summaries',
  );
  const summaryViewEnd = profileMigration.indexOf(
...
test('targeted practice is atomic and selects authoritative, grade-eligible question IDs', () => {
  const createPractice = functionBody(
    profileMigration,
  ...
  const registerIndex = createPractice.indexOf(
  ...
  const questionAuthority = functionBody(
    profileMigration,
...
test('quarantine is exact, hash-bound, and records eight retirements plus fifteen superseded mappings', () => {
  ...
  for (const questionId of [...retiredQuestionIds, ...supersededMappingQuestionIds]) {
    assert.equal(
      quarantineMigration.split(questionId).length - 1,
      1,
      `Expected one exact governed reference for ${questionId}`,
    );
  }
