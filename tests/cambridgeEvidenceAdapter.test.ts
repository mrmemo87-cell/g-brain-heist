...
const migration = readFileSync(
  'supabase/migrations/20260810193000_cambridge_evidence_adapter.sql',
  'utf8',
);
const roadmap = readFileSync('docs/academic-intelligence-roadmap.md', 'utf8');
const listeningPilot = readFileSync(
...
test('direct browser table writes stay closed and the rollout is gated', () => {
  for (const table of evidenceTables) {
    ...
    assert.doesNotMatch(migration, new RegExp(
      `grant (?:insert|update|delete|all)[^;]*public\\.${table}[^;]*to authenticated`,
      'i',
    ));
  }
