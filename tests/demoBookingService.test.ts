...
test('requires a valid campaign day', () => {
  assert.equal(
    validateDemoBooking({ ...validBooking(), preferred_date: '2026-08-21' }, TEST_NOW),
...
test('requires a phone or WhatsApp number', () => {
  assert.equal(
    validateDemoBooking({ ...validBooking(), phone: '123' }, TEST_NOW),
...
test('rejects times outside the fixed half-hour calendar', () => {
  assert.equal(
    validateDemoBooking({ ...validBooking(), preferred_time: '18:00' }, TEST_NOW),
...
test('loads only the server-computed rolling availability window', async () => {
  ...
  assert.deepEqual(await listDemoBookingSlots(client as never), [
