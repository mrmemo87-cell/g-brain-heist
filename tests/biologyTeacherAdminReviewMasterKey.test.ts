...
function collectProductionSources(relativeDir: string): string[] {
  const dir = resolve(root, relativeDir);
  return readdirSync(dir).flatMap((entry) => {
    const relativePath = `${relativeDir}/${entry}`;
    const fullPath = resolve(root, relativePath);
    if (statSync(fullPath).isDirectory()) return collectProductionSources(relativePath);
    if (!/\.(ts|tsx)$/.test(entry) || entry.endsWith('.bak')) return [];
    return [relativePath];
  });
}
