...
import {
  toIeltsLocalDateTimeInput,
  validateIeltsExamSchedule,
} from '../src/lib/ieltsExamSchedule.js';
...
test('IELTS create and reschedule flows share the same four schedule rules', () => {
  ...
  assert.equal(
    validateIeltsExamSchedule({ ...validSchedule, startsAt: '' }),
    'Start and end time are required.',
  );
  assert.equal(
    validateIeltsExamSchedule({ ...validSchedule, endsAt: '2026-08-04T08:59' }),
    'End time must be after start time.',
  );
  assert.equal(
    validateIeltsExamSchedule({ ...validSchedule, durationMinutes: 0 }),
    'Duration must be greater than zero.',
  );
  assert.equal(
    validateIeltsExamSchedule({ ...validSchedule, durationMinutes: 121 }),
    'Duration must fit within start and end times.',
  );
});
...
test('IELTS exam detail date conversion fails safely for missing and malformed values', () => {
  ...
  assert.match(
    toIeltsLocalDateTimeInput('2026-08-04T09:00:00.000Z') ?? '',
