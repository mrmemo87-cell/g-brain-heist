...
import {
  ...
  resolveNextIncompleteAssignmentItem,
} from '../services/ieltsAssignmentUx.js';
import type { IeltsPracticeAssignmentProgress } from '../services/ieltsPracticeAssignmentService.js';
...
test('next incomplete assignment item selection prefers same skill then order index', () => {
  const progress: IeltsPracticeAssignmentProgress = {
  ...
  assert.equal(
    buildNextAssignmentItemRoute(progress, 'reading-1', { assignmentTitle: 'Week 1', assignmentDueAt: '2026-05-20T10:00:00.000Z' }),
    '/ielts/reading/33?assignment_id=assignment-1&assignment_item_id=reading-2&assignment_item_count=4&assignment_title=Week+1&assignment_due_at=2026-05-20T10%3A00%3A00.000Z',
  );
});
