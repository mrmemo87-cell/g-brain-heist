...
const form = (id: string, blueprint_id: string, form_code: string, created_at: string, questions = [cleanQuestion(form_code.includes('6') ? 6 : 5)]): AdmTestForm => ({
  id,
  blueprint_id,
  school_id: 'school',
  form_code,
...
test('historical old attempts can remain visible outside default send cards', () => {
  ...
  const attempt: AdmAttempt = { id: 'attempt-old', candidate_id: 'cand', form_id: oldForm.id, school_id: 'school', started_at: '2026-02-01T00:00:00Z', submitted_at: null, expires_at: '2026-02-02T00:00:00Z', status: 'in_progress', total_score: null, max_score: null, percentage: null, anti_cheat_flags: {}, created_at: '2026-02-01T00:00:00Z' };
...
test('forms containing legacy or unmanaged questions are excluded from sendable forms', () => {
  ...
  const unmanaged = form('unmanaged', 'bp-g5-mat', 'MAT5-2026-OLD', '2026-07-08T00:00:00Z', [{ ...cleanQuestion(5), external_id: null as any }]);
