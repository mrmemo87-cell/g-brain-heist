...
} from '../src/lib/brains_heist/writingIntegrity.js';
...
test('practice mode records substantial paste and recommends teacher review', () => {
  ...
  const finalized = finalizeWritingCompositionTelemetry(
    telemetry,
    submission,
    '',
...
test('independent mode records a blocked paste without counting it as composed text', () => {
  const telemetry = recordWritingPaste(
    createWritingCompositionTelemetry('independent'),
    240,
    true,
  ...
  const finalized = finalizeWritingCompositionTelemetry(
    telemetry,
    'This short response was typed in the editor.',
    '',
    new Date('2026-07-26T10:01:00.000Z')
  );
...
test('supervised mode flags leaving the page and revisions preserve change evidence', () => {
  ...
  telemetry = recordWritingInput(
    telemetry,
    '',
    'Schools should teach writing with clear examples.',
    new Date('2026-07-26T10:00:00.000Z')
  );
  telemetry = recordWritingVisibilityHidden(telemetry);
  const finalized = finalizeWritingCompositionTelemetry(
    telemetry,
    'International schools should teach writing with clear examples and purposeful revision.',
    'Schools should teach writing with clear examples.',
