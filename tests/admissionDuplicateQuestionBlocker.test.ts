...
test('official bank validator hard-fails duplicate normalized stems under different external ids', () => {
  ...
  for (const subject of ['maths', 'science']) writeFileSync(path.join(root, subject, 'grade_5.json'), JSON.stringify({ pools: [], questions: [] }));
  writeFileSync(path.join(root, 'english', 'grade_5.json'), JSON.stringify({
    pools: [{ external_id: 'pool', subject: 'english', grade_level: 5, stage_level: 'primary', placement_band: 'target', name: 'Pool', content_version: 'test', source_label: 'Brain Heist Official Admission Bank', is_official: true, is_locked: true, content_owner: 'brain_heist' }],
    questions: [1, 2].map((n) => ({ external_id: `q${n}`, pool_external_id: 'pool', subject: 'english', grade_level: 5, stage_level: 'primary', placement_band: 'target', diagnostic_skill: 'Grammar', strand: 'grammar', subskill: 'sentence control', difficulty: 'medium', question_type: 'mcq', prompt: n === 1 ? 'Question 1: Choose the correctly punctuated sentence.' : 'Choose the correctly punctuated sentence!', explanation: 'Because it is correct.', marks: 1, estimated_seconds: 60, content_version: 'test', source_label: 'Brain Heist Official Admission Bank', is_official: true, is_locked: true, content_owner: 'brain_heist', options: ['A', 'B', 'C', 'D'], correct_answer: 'A' })),
  }));
...
test('current official bank duplicate branch ignores legacy import and unmanaged Grade 7/8 rows', () => {
  const branch = inspection.slice(
    inspection.indexOf('current_official_bank_stem_duplicates AS'),
    inspection.indexOf('legacy_official_bank_missing_external_id_or_unmanaged AS'),
  );
