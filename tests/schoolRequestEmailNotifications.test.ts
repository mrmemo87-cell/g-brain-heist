...
const migration = readFileSync(
  'supabase/migrations/20260812203000_school_request_email_notifications.sql',
  'utf8'
);
const edgeFunction = readFileSync(
  'supabase/functions/school_request_email/index.ts',
  'utf8'
);
const professionalEmailMigration = readFileSync(
  'supabase/migrations/20260815151437_professional_email_communications_v1.sql',
  'utf8'
);
...
test('school request email ledger is service-only and idempotent', () => {
  ...
  assert.match(
    migration,
    /revoke all on table public\.school_request_email_deliveries from public, anon, authenticated/i
  );
  assert.match(
    migration,
    /grant select, insert, update on table public\.school_request_email_deliveries to service_role/i
  );
});
...
test('submission and superadmin updates require the correct caller', () => {
  assert.match(
    edgeFunction,
    /request\.requested_by !== user\.id \|\| request\.status !== "pending"/
  );
