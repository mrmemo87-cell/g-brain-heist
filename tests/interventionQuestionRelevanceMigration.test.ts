...
const migration = readFileSync(
  'supabase/migrations/20260901031554_refine_intervention_targeted_practice_relevance.sql',
  'utf8',
);
...
test('automatic recommendations fail closed to exact matches only', () => {
  assert.match(
    migration,
    /recommended as \([\s\S]*?from matched[\s\S]*?where match_tier = 1[\s\S]*?limit 6/,
  );
