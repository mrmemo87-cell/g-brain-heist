...
import {
  ...
  generatedMultipleChoiceAnswerPositionCounts,
  hasBalancedGeneratedMultipleChoiceAnswers,
} from '../supabase/functions/_shared/teacherQuestionOptionBalance.js';
...
const generatedMcq = (index: number, optionCount = 4): TestQuestion => {
  const options = Array.from({ length: optionCount }, (_, optionIndex) =>
    optionIndex === 0 ? `Correct ${index}` : `Distractor ${index}-${optionIndex}`,
  );
