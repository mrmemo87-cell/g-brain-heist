...
import {
  ...
  resolveIeltsExamModeAdminAccess,
} from '../services/ieltsExamAccessService.js';

test('IELTS Exam Mode access trusts canonical school capability and platform administrators', () => {
  assert.deepEqual(
    resolveIeltsExamModeAdminAccess({ isAuthenticated: true, role: 'school_admin' }),
    { allowed: false, reason: 'denied' },
  );
  assert.deepEqual(
    resolveIeltsExamModeAdminAccess({ isAuthenticated: true, role: 'admin' }),
    { allowed: true, reason: 'platform_admin_role' },
  );
  assert.deepEqual(
    resolveIeltsExamModeAdminAccess({ isAuthenticated: true, isAdmin: true, role: 'student' }),
    { allowed: true, reason: 'platform_admin' },
  );
  assert.deepEqual(
    resolveIeltsExamModeAdminAccess({ isAuthenticated: true, role: 'school_admin', canAdministerSchool: true }),
    { allowed: true, reason: 'school_admin_capability' },
  );
});
...
test('IELTS Exam Mode access denies students and isolated legacy IELTS admins', () => {
  assert.deepEqual(
    resolveIeltsExamModeAdminAccess({ isAuthenticated: true, role: 'student', manageableExamListSucceeded: true, manageableExamCount: 0 }),
    { allowed: false, reason: 'denied' },
  );
  assert.deepEqual(
    resolveIeltsExamModeAdminAccess({ isAuthenticated: true, role: 'ielts_admin', manageableExamListSucceeded: true, manageableExamCount: 0 }),
    { allowed: false, reason: 'denied' },
  );
  assert.deepEqual(
    resolveIeltsExamModeAdminAccess({ isAuthenticated: false, role: 'superadmin' }),
    { allowed: false, reason: 'not_authenticated' },
  );
});

test('IELTS Exam Mode access fails closed when authentication cannot be verified', async () => {
...
test('IELTS Exam Mode guard hides previously authorized content during every recheck', () => {
  ...
  assert.match(guard, /const requestIdRef = useRef\(0\)/, 'guard must version overlapping authorization checks');
  assert.match(guard, /const requestId = \+\+requestIdRef\.current;\s*setState\('loading'\)/, 'every recheck must hide protected children first');
...
test('School Admin Portal exposes Phase 1 IELTS tabs without removing Cambridge', () => {
  ...
  for (const tab of ['ielts-exams', 'ielts-practice', 'ielts-results', 'ielts-student-progress', 'ielts-reviews']) {
    assert.match(portal, new RegExp(`'${tab}'`), `School Admin Portal should include ${tab}`);
  }

  assert.match(portal, /id: 'cambridge'[\s\S]*?label: 'Cambridge Assessments'/, 'school-friendly Cambridge navigation label must remain present');
...
test('IELTS routes isolate legacy admin pages from school-scoped Exam Mode guard', () => {
  ...
  assert.match(
    routes,
    /path:\s*'\/ielts\/admin',[\s\S]*?<IeltsAdminGuard>[\s\S]*?<IeltsAdminDashboard\s*\/>[\s\S]*?<\/IeltsAdminGuard>/i,
    'legacy IELTS admin route must keep IeltsAdminGuard',
  );
  assert.match(
    routes,
    /path:\s*'\/ielts\/exams\/manage',[\s\S]*?<IeltsExamModeAdminGuard>[\s\S]*?<IeltsExamManager\s*\/>[\s\S]*?<\/IeltsExamModeAdminGuard>/i,
    'IELTS exam manager route must use school-scoped Exam Mode guard',
  );
  assert.match(
    routes,
    /path:\s*'\/ielts\/exam\/:examEventId\/monitor',[\s\S]*?<IeltsExamModeAdminGuard>[\s\S]*?<IeltsExamMonitor\s*\/>[\s\S]*?<\/IeltsExamModeAdminGuard>/i,
    'IELTS exam monitor route must use school-scoped Exam Mode guard',
  );
});
...
test('Phase 0 IELTS Exam Mode SQL keeps managers school-scoped and teachers monitor-only', () => {
  const migration = fs.readFileSync(
    path.join(process.cwd(), 'supabase/migrations/20260516120000_ielts_exam_mode_school_scoped_permissions.sql'),
    'utf8',
  );

  assert.match(
    migration,
    /u\.school_id\s*=\s*e\.school_id[\s\S]*coalesce\(u\.role, ''\)\s*=\s*'school_admin'/i,
    'school_admin role must be scoped to the exam school',
  );
  assert.match(
    migration,
    /sm\.role_in_school\s+in\s*\('school_admin',\s*'admin',\s*'superadmin'\)/i,
    'school membership managers must be explicit admin roles only',
  );
  assert.doesNotMatch(
    migration,
    /role_in_school\s+in\s*\([^\)]*'teacher'/i,
    'teachers must not be included in create/manage roles',
  );
  assert.match(
    migration,
    /case\s+when\s+v_is_manager\s+then\s+to_jsonb\(f\)\s+else\s+to_jsonb\(f\)\s+-\s+'answer_key'\s+end/i,
    'monitor-only detail responses must omit answer_key',
  );
  assert.match(
    migration,
