...
test('IELTS review RPCs are school-scoped and admin-only', () => {
  ...
  assert.match(
    sql,
    /if not (?:public\.can_review_ielts_productive_submission\(v_school_id, v_class_id, v_student_id\)|public\.can_manage_ielts_practice_school\(v_school_id\)) then[\s\S]*raise exception 'forbidden'/i,
    'submit must reject out-of-scope actors',
  );
