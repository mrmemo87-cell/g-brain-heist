...
const buildPlanInput = (
  ...
  response: string,
  studentId: string,
  memoryDate = '2026-03-01T10:00:00.000Z'
) => {
...
test('Grade 6 language-focused week uses high support and language drills early', () => {
  const { memory, assessment, plan } = buildPlanInput(
    6,
    'email',
    'Dear teacher i is go yesterday to event. it mattered because teamwork. i recommend do again',
    'g6'
  );
...
test('Grade 8 organisation-focused week includes sequencing/linking tasks', () => {
  const { memory, assessment, plan } = buildPlanInput(
    8,
    'article',
    'The event was a school clean-up and it mattered because students helped and suggestion is continue',
    'g8'
  );
...
test('Grade 10 communicative-focused week includes genre convention work', () => {
  const { memory, assessment, plan } = buildPlanInput(
    10,
    'report',
    'Hey guys this report was awesome and cool. I describe event, explain it mattered and suggest to do again.',
    'g10'
  );
...
test('end-of-week task is exam-style', () => {
  const { memory, assessment, plan } = buildPlanInput(
    11,
    'essay',
    'This essay describes the event, explains why it mattered, and gives one suggestion for future improvement.',
    'g11'
  );
