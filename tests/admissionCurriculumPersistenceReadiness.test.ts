...
test('additive migration adds nullable curriculum columns and audit indexes', () => {
  ...
  for (const col of ['curriculum_linkage_status','curriculum_map_id','curriculum_map_version','curriculum_programme','curriculum_subject_code','curriculum_source_version','curriculum_review_status','curriculum_objective_id','curriculum_source_reference']) assert.match(sql, new RegExp(`add column if not exists ${col}`));
  for (const idx of ['idx_adm_qpool_curriculum_map','idx_adm_qpool_curriculum_linkage_status','idx_adm_q_curriculum_objective']) assert.match(sql, new RegExp(idx));
});
...
test('readiness matrix contains all 30 slots and legacy banks never ship', async () => {
  ...
  assert.ok(result.rows.filter((r:any) => r.curriculum_linkage_status === 'legacy_review_required').every((r:any) => r.final_readiness_status !== 'SHIP_READY'));
});
...
test('readiness blockers and synthetic SHIP_READY path', async () => {
  ...
  row = auditAdmissionBankShipReadiness({ seedDir:root, grade:1, subject:'english', validatorOverride:{ ok:true, errors:[] } }).rows[0]; assert.equal(row.final_readiness_status, 'MAP_REQUIRED');
  writeJson(path.join(root,'curriculum-maps/english/grade_1.json'), approvedMap());
  row = auditAdmissionBankShipReadiness({ seedDir:root, grade:1, subject:'english', validatorOverride:{ ok:false, errors:['bad'] } }).rows[0]; assert.equal(row.final_readiness_status, 'VALIDATION_FAILED');
  row = auditAdmissionBankShipReadiness({ seedDir:root, grade:1, subject:'english', validatorOverride:{ ok:true, errors:[] } }).rows[0]; assert.equal(row.final_readiness_status, 'SHIP_READY');
  ...
  row = auditAdmissionBankShipReadiness({ seedDir:root, grade:1, subject:'english', validatorOverride:{ ok:true, errors:[] } }).rows[0]; assert.equal(row.final_readiness_status, 'ACADEMIC_REVIEW_REQUIRED');
});

test('readiness auditor reports programme and source basis for Brain Heist International maps', async () => {
  const { auditAdmissionBankShipReadiness } = await auditor();
  const root = baseRoot();
  writeJson(path.join(root,'english/grade_1.json'), { curriculum_linkage_status:'linked', curriculum_map_id:'map-1', curriculum_map_version:'2026.1', curriculum_review_status:'approved', release_manifest:{ content_review_status:'approved', department_head_approval_status:'approved', staging_verification_status:'verified' }, pools:[pool()], questions:[question({ curriculum_objective_id:'obj-1' })] });
