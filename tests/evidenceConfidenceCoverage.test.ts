...
const migration = readFileSync(
  'supabase/migrations/20260810210000_evidence_confidence_and_coverage.sql',
  'utf8',
);
...
test('client-scored Cambridge observations never qualify', () => {
  ...
  assert.doesNotMatch(
    migration.match(/student_learning_observation_is_qualified[\s\S]+?\$\$;/i)?.[0] ?? '',
    /stored_client_result/i,
  );
