...
import {
  AdmissionReportPartialAttemptNotice,
  resolveAdmissionReportPartialAttempt,
  resolveAdmissionReportVisiblePartialAttempt,
} from '../components/admissionReportPartialAttempt.js';
...
test('Admission Hub report modal path derives SCI6 partial notice from the visible score and answer-count values', () => {
  ...
  assert.ok(
    hub.indexOf('<AdmissionReportPartialAttemptNotice') < hub.indexOf('Placement Recommendation'),
    'partial-attempt notice must render before Placement Recommendation in the report modal',
  );
});

test('Admission Hub report modal partial-attempt notice renders SCI6 visible fallback values before placement details', () => {
  const previousMetrics = resolveAdmissionReportPartialAttempt(sci6Report as any);
