...
function latestManagedVersion(versions: string[], grade: number, subject: string): string | undefined {
  return versions
    .map((version) => ({ version, match: version.match(managedVersion) }))
    .filter((item): item is { version: string; match: RegExpMatchArray } =>
      item.match !== null && Number(item.match[2]) === grade && item.match[3] === subject,
    )
...
test('Grade 6 Maths generation is pinned to v2 without weakening managed-content safety', () => {
  ...
  const selected = Object.entries(required).flatMap(([difficulty, count]) =>
    grade6Maths.questions.filter((question: { difficulty: string }) => question.difficulty === difficulty).slice(0, count),
  );
  const ids = new Set(selected.map((question: { external_id: string }) => question.external_id));
  const stems = new Set(selected.map((question: { prompt: string }) => question.prompt.trim().toLowerCase()));
  const byDifficulty = selected.reduce<Record<string, number>>((counts, question: { difficulty: string }) => {
