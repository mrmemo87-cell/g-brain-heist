...
import { startPracticeBattle, applyPracticeTurn } from '../supabase/functions/commander_practice/engine';
import { readCommanderSession, saveCommanderSession } from '../src/features/cursedCommander/commanderSessionCache';
import { getCommanderPresentationPoint, commanderProjectileAngle } from '../src/features/cursedCommander/commanderPresentationLayout';
...
test('refresh restores all signed combat values and pending input without healing or consuming a turn', () => {
  const store = storage(); const confirmed = session();
  ...
  const pending = { move: 'guard' as const, targetId: null };
...
test('projectile headings point toward targets in rendered pixels across narrow and wide boards', () => {
  for (const width of [320,375,390,430,768,1024,1440]) for (const dx of [-50,50]) for (const dy of [-30,0,30]) {
