...
test('Grade 6 Maths international bank v2 has required question count and linked MCQ shape', () => {
  ...
  const objectiveIds = new Set(map.objectives.map((objective: { objective_id: string }) => objective.objective_id));
...
test('Grade 6 Maths international bank v2 fixes the greatest-number answer regression', () => {
  const question = grade6.questions.find((item: { external_id: string }) => item.external_id === 'adm-g6-maths-v2-q001');
  assert.ok(question);
