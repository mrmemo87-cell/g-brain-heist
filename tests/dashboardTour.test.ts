...
} from '../src/features/onboarding/dashboardTour.js';
...
const learnerState = (metadata: Record<string, unknown> = {}): OnboardingState => ({
  ...
  completed_steps: ['intent', 'school_confirm', 'goal', 'identity', 'mission_brief', 'reward_reveal'],
...
test('dashboard tour metadata resumes the saved step and records mission completion intent', () => {
  ...
  assert.deepEqual(
    appendDashboardTourCompletedStep(metadata.completed_steps, 'first_mission'),
    ['base_unlocked', 'profile_progress', 'xp_rewards', 'first_mission'],
  );
});
