...
async function loadValidator() {
  ...
  return await import(moduleUrl) as {
    validateAdmissionOfficialBank: (seedDir?: string) => ValidationResult;
    normalizeAdmissionQuestionStem: (value: string) => string;
  };
}
...
test('normalized stem detection strips artificial answer hints', async () => {
  ...
  assert.equal(
    normalizeAdmissionQuestionStem('In a sports report, which word best completes the sentence: The instructions were clear and ______. Focus on precise.'),
    normalizeAdmissionQuestionStem('In a library conversation, which word best completes the sentence: The instructions were clear and ______. Focus on precise.'),
  );
});
