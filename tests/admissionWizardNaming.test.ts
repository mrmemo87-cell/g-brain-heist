...
import {
  buildAdmissionWizardBlueprintName,
  buildAdmissionWizardDefaultName,
  getAdmissionWizardSubjectLabel,
} from '../src/lib/admissionWizardNaming.js';
...
test('blueprint payload name preserves custom names and adds wizard prefix once', () => {
  assert.equal(
    buildAdmissionWizardBlueprintName('Custom Grade 5 Maths Screen'),
    'Admission Test Wizard — Custom Grade 5 Maths Screen',
  );
  assert.equal(
    buildAdmissionWizardBlueprintName('Admission Test Wizard — Grade 5 Science Admission Test'),
    'Admission Test Wizard — Grade 5 Science Admission Test',
  );
});
