...
test('empty-state student experience render', () => {
  ...
  assert.ok(html.includes('No writing state yet'));
});
...
test('no-active-task state render', () => {
  ...
  assert.ok(html.includes('All tasks submitted for now'));
});
...
test('teacher dashboard stalled/improving indicators render', () => {
  ...
  submitInitialWritingAssessment({
    ...
    target_word_count: 120,
    student_response:
      'This final response clearly describes the event, explains why it mattered to the school community, and offers a specific practical suggestion in a well-organized structure.',
    attempted_at: '2026-03-03T10:00:00.000Z',
  });
...
test('progress visual render with monthly data', () => {
  ...
  assert.ok(html.includes('Progress at a glance'));
  assert.ok(html.includes('Target score range'));
});
...
test('admin calibration review renders full decision chain', () => {
  ...
  submitInitialWritingAssessment({
    ...
    student_response:
      'This report describes the event, explains why it mattered for students, and gives one practical recommendation.',
...
test('admin calibration review missing-data fallback', () => {
  ...
  const emptyHtml = renderToStaticMarkup(
    React.createElement(WritingCalibrationReview, { studentId: 'unknown-student', month: '2026-03' })
  );
  assert.ok(emptyHtml.includes('No calibration data found'));

  const loadingHtml = renderToStaticMarkup(
    React.createElement(WritingCalibrationReview, { studentId: 'cal-1', isLoading: true })
  );
  assert.ok(loadingHtml.includes('Loading calibration review'));

  const errorHtml = renderToStaticMarkup(
    React.createElement(WritingCalibrationReview, { studentId: 'cal-1', errorMessage: 'timeout' })
  );
...
test('writing analytics dashboard renders aggregated views with drill-down links', () => {
  ...
  const html = renderToStaticMarkup(
    React.createElement(WritingAnalyticsDashboard, {
      gradeFilter: 9,
      genreFilter: 'essay',
...
test('writing analytics dashboard loading/error/empty and warning badge states', () => {
  ...
  const errorHtml = renderToStaticMarkup(
    React.createElement(WritingAnalyticsDashboard, { errorMessage: 'service unavailable' })
  );
  assert.ok(errorHtml.includes('Unable to load analytics'));

  const emptyHtml = renderToStaticMarkup(
    React.createElement(WritingAnalyticsDashboard, { gradeFilter: 12, genreFilter: 'story' })
  );
...
test('cross-page admin filter query normalization behavior', () => {
  ...
  const monitoringHtml = renderToStaticMarkup(
    React.createElement(WritingMonitoringView, { filterQuery: '?status=stalled&grade=9' })
  );
  assert.ok(
    monitoringHtml.includes('Teacher/Admin Writing Monitor') || monitoringHtml.includes('No monitoring matches')
  );

  const promptHtml = renderToStaticMarkup(
    React.createElement(WritingPromptBankManager, { filterQuery: `?prompt_id=${promptRecord.data!.id}&active=true` })
  );
  assert.ok(promptHtml.includes('Filter Prompt'));

  const calibrationHtml = renderToStaticMarkup(
    React.createElement(WritingCalibrationReview, { studentId: 'flt-1', filterQuery: '?weakness_tag=under_length' })
  );
...
test('admin help content and filtered empty-state copy render', () => {
  __resetWritingIntegrationStoreForTests();
  const monitoringEmpty = renderToStaticMarkup(
    React.createElement(WritingMonitoringView, { filterQuery: '?status=stalled&grade=9' })
  );
  assert.ok(
  ...
  const calibrationHelp = renderToStaticMarkup(
    React.createElement(WritingCalibrationReview, { studentId: 'help-1', month: '2026-03' })
  );
...
test('admin feedback flags display and filter behavior in views', () => {
  ...
  const calibrationFlagged = renderToStaticMarkup(
    React.createElement(WritingCalibrationReview, { studentId: 'flag-1', month: '2026-03' })
  );
