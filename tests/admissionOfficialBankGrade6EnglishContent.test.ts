...
const grade6Passages = passages.passages.filter((passage: any) => passage.grade_level === 6 && passage.content_version === 'adm-bank-v1-g6-english');
const grade6Rubrics = rubrics.rubrics.filter((rubric: any) => rubric.grade_level === 6 && rubric.content_version === 'adm-bank-v1-g6-english');
...
test('Grade 6 English reading questions and writing prompts are linked', () => {
  const passageIds = new Set(grade6Passages.map((passage: any) => passage.external_id));
  const rubricIds = new Set(grade6Rubrics.map((rubric: any) => rubric.external_id));
  const reading = grade6.questions.filter((question: any) => question.question_type === 'reading_comprehension');
  assert.equal(new Set(reading.map((question: any) => question.passage_external_id)).size, 5);
  for (const question of reading) assert.ok(passageIds.has(question.passage_external_id), `${question.external_id} must link to a Grade 6 passage`);
  const writing = grade6.questions.filter((question: any) => question.question_type === 'writing_prompt');
  for (const question of writing) assert.ok(rubricIds.has(question.rubric_external_id), `${question.external_id} must link to the Grade 6 rubric`);
});
