...
const supplement = readFileSync(
  'supabase/migrations/20260901093000_add_grade4_future_tense_verified_supplement.sql',
  'utf8',
);
const taxonomy = readFileSync(
  'supabase/migrations/20260901093500_add_grade4_future_tense_diagnostic_taxonomy.sql',
  'utf8',
);

const externalIds = Array.from(
  supplement.matchAll(/"externalId": "(bh-g4-eng-future-2026\.12-\d{3})"/g),
  (match) => match[1],
);
const atomicSubskills = Array.from(
  taxonomy.matchAll(/'english\.future-tense\.([a-z0-9-]+)'/g),
  (match) => match[1],
);
