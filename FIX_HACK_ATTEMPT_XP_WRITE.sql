-- ============================================================
-- FIX: rpc_hack_attempt — set app.allow_xp_level_write
-- ============================================================
-- ROOT CAUSE: The block_direct_xp_level_updates trigger rejects
-- any UPDATE on public.users that modifies xp or level unless
-- current_setting('app.allow_xp_level_write', true) = '1'.
--
-- rpc_hack_attempt (SECURITY DEFINER) does:
--   UPDATE public.users SET xp = xp + xp_delta ...
-- but NEVER set the config flag, so the trigger blocks every
-- PvP battle with: "Direct XP/Level updates are not allowed."
--
-- FIX: Add PERFORM set_config('app.allow_xp_level_write','1',true)
-- at the start of the function body (after auth checks, before
-- any UPDATE that touches xp/level).
-- ============================================================
